...
- **One mute for the whole estate** — any page that makes sound reads/writes the single shared key **`ws:pref:muted`** (via `WS`), so a visitor mutes *once* and it holds everywhere (Sound Garden, the front-door ambient layer, the Survey's discovery melodies, any audio piece). Never invent a per-page mute key. Default: respect it on load; honor the browser's autoplay gate (sound waits for a click). The same convention governs every shared sound preference — the estate's other `ws:pref:*` sound keys are **`ws:pref:air`** (the ambient score's arm choice) and **`ws:pref:air-bg`** (the visitor's opt-in to let the air keep playing while the tab sits behind another; absent means the estate stills itself when you look away, which is its manners). Each ships a getter, a notifying setter, a subscribe function, and a cross-tab `storage` listener in `tools/ws/ws.js` — a preference flipped on one estate tab must take effect live in the others.
