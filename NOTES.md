...
**⏳ IN PROGRESS (2026-06-13 cont., Opus 4.8) — THE LETTERER + map self-labelling.** Brandon saw the live map: the hand-placed POI labels OVERLAP (the "↳ within" pills, "Undercroft · beneath", Music/Map rooms, the Workshop placard crowding the Gardens). His steer: *"build a label placement algorithm specimen … and use it for your map too."* So instead of nudging coords I'm building a real **point-feature label-placement engine** (`tools/label/label.js` — candidate-slot model + simulated annealing, à la Christensen–Marks–Shieber 1995; deterministic/seeded; module-guarded for forge+Node) with a Node self-test proving **0 label-label & 0 label-obstacle overlaps**, shipping it as a **provable specimen — "The Letterer"** (Workbench → Toys & benches; forge page using WS, `ws:seen:letterer`), then wiring the **front door to consume that same engine** to place its own labels (runtime, deterministic → stable; replaces hand `lx/ly/label`; "append a POI" stops needing hand label coords). Deputy #1 = engine+specimen+test (worktree); deputy #2 = map integration (depends on #1; visual-iterate). Resume: if a worktree branch `wt-letterer*` exists, deputy #1 ran — check its commit, integrate via `git checkout <branch> -- <files>` + fresh commit, then dispatch #2. THEN the big creative wave (below).

**Last shipped (2026-06-13, Opus 4.8): THE BIG REFRESH — the front door is now an ESTATE MAP, the `ws:` unlock system is forge-inlined with an in-the-moment unlock cue, + two new bench pieces (Harmonograph, Scytale). All pushed & live (HEAD `3d9dbba`); tree clean, synced with origin, `forge --check --all` green, no leftover worktrees.**
...
**Next-steps menu — THE BIG CREATIVE WAVE (user-requested; begin after the context compaction):**
- **A NEW reusable engine (Lantern-class)** — declarative data-driven instances + a self-verifying solver/AI, forge-inlined like Lantern's engine. Candidates (Claude's call): an **abstract-strategy / board-game engine** (game-defs as data → playable vs a *provable* AI; a "mate-in-N" proven by exhaustive search; small solved games — Nine Men's Morris / Hex / Konane / m,n,k-games), OR a **solitaire engine** ("every deal *provably winnable*" + watch-it-solve-itself — mirrors Lantern's winnable-proof & Daedalus's self-solving). New genre for the workshop.
- **A NEW cross-page METAGAME** on the WS/forge infra (distinct from the Undercroft), whose **visible surface is the estate map itself**. Seeds: a *living estate* (day/night or seasons that tint the map + time-gated events), a **wandering familiar** (a creature roaming pages that remembers you and leads somewhere), a **menagerie/collection** (pages drop specimens into a cabinet). Forge-share the metagame module exactly like WS.
- **More growth POIs** — a **Chladni/cymatics bench** (pairs w/ Ripple), an **arcade cabinet** (Defender/Scramble side-scroller — the last classic gap; bumps the games count → **also bump the map's Arcade `tag` in `index.src.html` PLACES**), a **kaleidoscope**, a **sundial** (celestial-instrument kin). New benches go on the **Workbench** (reached via the Workshop POI), born as **forge pages using WS** (`WS.seen('<id>')`).
- **Cipher vein** (public: Volvelle + Scytale; hidden: Enigma) — still open: Playfair/Polybius, one-time-pad, Hagelin M-209, or a **bombe / known-plaintext attack** (the codebreaker side). Don't rebuild a rotor-Enigma, a shift/Vigenère disk, or another transposition rod.
- Wire the deferred **Night Shift** cue (above). Grow **Lantern** (a 3rd tale; a wired `llmPlayer`).
