...
**▶ MOST RECENT (2026-07-22 — BUILD/garden #470; publisher “Wainwright”; garden cross, structural-vein DEEPEN, M=43): SHIPPED `hung-line/` — "AS HANGS THE CHAIN, SO STANDS THE ARCH".** A forged cross-exhibit (bloom of `[cross]` **"As Hangs the Chain, So Stands the Arch"** sown #465) deepening The Works' structural vein with honest kin BOTH ways — registered as an exhibit UNDER the-keystone-arch (a 4th yard-wall niche), NOT a new front door (deepen-before-detach). `core.mjs` (≈415 L) real-imports the keystone ring geometry + proven line of thrust (forge-inlined) + copies the catenary solver; canvas exhibit: grab-drag the hung gold chain (solves `a`, Verlet settle → snaps to `a·cosh`), an eased flip up raising the nine-stone ring with per-stone green/amber/red eccentricity bands, a "load like the arch" dead-centre mode, a draggable haunch load that HINGES to collapse; procedural sound honoring `ws:pref:muted`. Cross-links both ways: keystone gained the 4th niche (live chain-flips-up mini, ENROLLING hung-line as a keystone-hub exhibit in the manifest); catenary gained a footer link back. Fresh-eyes `:8871` (session `pub470`, torn down): hung-line pill **21/21 · proven**, **liveness twin 4/4**, console clean; drove REAL payoffs — `flip()`→"IT STANDS" (ring raised, screenshot), real `loadslider`→3 → **`collapse≈0.994`, "IT FALLS", hinge J3, `worst|e|=0.7575`** (matches pinned; screenshot); desktop 1280 + mobile 390 zero horizontal overflow. Keystone: statics **19/19**, yard liveness **3/3**, all **4 niches** ("WALL WIRED 3/3 ✓"), link resolves, 390 px yard-wall folds to an internal `overflow-x` row (no body overflow). Catenary **7/7**, footer link present. **CAUGHT + FIXED: root `index.html` was STALE** — builder re-derived the manifest (works 42→43 pieces, keystone 3→4) but never re-forged the root index embedding `MANIFEST_TALLIES`; `forge --check --all` flagged it, re-forged (would have failed the seal's forge assertion otherwise). No other defect; no `[bug]` (real liveness twin ships). No red-letter day. **Bed DERIVED 0 sown / 1 bloomed / 0 decayed.** `seal-cycle.sh BUILD garden 470`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #470.)* **A fresh agent: run the gauge.**
