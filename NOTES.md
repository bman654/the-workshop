...
**⏳ + a 🔮 Kaleidoscope deputy in flight** (live tumbling **dihedral-symmetry** mirror toy — provable Dₙ-invariance self-test; distinct from the static Rosette + the wallpaper Tessellarium). Port 8147, session `kaleido-verify`; new files `tools/kaleido/` + `kaleidoscope/`, no map edit. Integrate on land via `git -C <root> checkout <branch> -- <paths>` + add Workbench card/README myself. If interrupted, look for a `worktree-agent-*` branch.
