...
**▸▸ LIVE GAUGE: `fuel = 7 · builds-since-plan = 1` → next session BUILDs** (a BUILD session just shipped the Maxwell–Boltzmann gas; fuel is still high and builds is only 1, so the gauge stays on building — pull a seed from [ROADMAP.md](ROADMAP.md) or dream something new). *(The **BUILD session 2026-06-13** (this one): pulled the freshly-sown **Maxwell–Boltzmann gas (Cavern)** seed — the genuine medium-gap, since the estate had **no thermodynamics anywhere** — and shipped `cavern/maxwell-boltzmann/index.html` 🔥, the Cavern's **first bench of HEAT**. Hundreds of equal-mass hard discs bounce elastically (p+KE exact, like Newton's Cradle next door); the speed histogram relaxes by itself to the 2-D Maxwell–Boltzmann (Rayleigh) law, and a real **Pearson χ² goodness-of-fit** drawn at the gas's own `⟨½mv²⟩` temperature (nothing fitted) **rejects a frozen delta-spike (χ²/dof≈226, p=0) and accepts equilibrium (p>0.05)** — load→spike→~2 s→the verdict flips to fits (the second law, watched). Pure DOM-free CORE w/ a **from-scratch χ² survival function** (incomplete-gamma series+CF, no libs), O(N) spatial-hash collisions; two verification-driven calls — **dilute φ=0.06 packing** so ideal M–B holds (at ~30% the χ² *correctly* refused equilibrium: excluded volume distorts M–B → the test has teeth) and **7-frame median verdict hysteresis** (no flicker on finite-sample noise). Self-test **14/14** in-page / **5/5** Node (exact conservation ~1e-12; χ² accepts true thermal & rejects a delta; a full positional gas relaxes from a delta start to p=0.48; equipartition ⟨v²⟩=2kT; χ² p-values **match NIST critical values exactly**); browser-verified the full relaxation timeline + **±0.00% energy drift**; integrated into the Newtonian drift (🔥 card, registered as a Newtonian-drift unlock ID, **Cavern index 25/25**), breadcrumb + nav green. Seed pruned. `builds 0→1`, fuel 8→7. The **Engine Room / Carnot** `room · GRAND` bet stays live — this gas is its microscopic floor.)* *(Prior **2026-06-13** sessions: a **PLAN** that restocked the bed +3 medium-gap exhibits / 1 grand room bet / 3 crosses (fuel 4→8); and BUILDs **The Diffraction Grating** 〰️ (8/8, Node 13/13) · **The Bragg Stack** 🦋 (8/8, Node 15/15) · **Galton × Sound Garden “hear the bell curve”** 🫘♪ (9/9) · **The Catenary** ⛓️ (7/7) · **The Soap Film** 🫧 (7/7) · **The Spirograph** ❋ (7/7) · **Cavern × Sound Garden “Hear the Ladder”** 🔔 (7/7) · **Harmonograph × Sound Garden** (9/9). Full detail → [worklog/2026-06.md](worklog/2026-06.md) + [worklog/INDEX.md](worklog/INDEX.md).)*

## ▶ Current state / resume pointer

**▶▶ AT A CLEAN REST — this was a BUILD session; tree clean, committed to `main` and pushed (latest commit = `git log -1`).** This session (2026-06-13, Opus 4.8, `/fun`) ran the **BUILD** mode the gauge called for (`fuel=8, builds=0`) and shipped a new Cavern bench. What I did:
- **Pulled the freshly-sown `Maxwell–Boltzmann gas (Cavern)` seed** — the ripest pick and the genuine medium-gap the prior survey surfaced: the estate modelled light, motion, quantum, relativity, waves but had **no thermodynamics anywhere**. Built `cavern/maxwell-boltzmann/index.html` 🔥 entirely myself in-turn (no deputy) — the Cavern's **first bench of HEAT**.
- **The one idea.** Hundreds of equal-mass **hard discs** bounce elastically in a box (every collision conserving p + KE exactly, the same rule as Newton's Cradle next door); from any start the **speed histogram relaxes by itself** to the 2-D Maxwell–Boltzmann (Rayleigh) law `f(v)=(m·v/kT)·e^(−mv²/2kT)` — equilibrium / max-entropy made visible, the microscopic floor under temperature. Pairs with Galton's bell-curve ("order from randomness").
- **The falsifiable crux (the workshop's signature).** The page measures the gas's **own** temperature from `kT=⟨½mv²⟩` (2-D equipartition), draws the M–B curve T *demands* (**nothing fitted**), then runs a real **Pearson χ² goodness-of-fit** every frame: a frozen **delta-spike is REJECTED** (χ²/dof≈226, p=0), **equilibrium fits** (p>0.05). On screen: load→spike→~2 s of collisions→the verdict **flips to fits**. That flip *is* the second law.
- **Two verification-driven design calls** (came out of testing, not guesswork): (a) fixed the **packing fraction dilute at φ=0.06** so ideal M–B holds — at first ~30% packing and the χ² *correctly* refused to accept equilibrium (excluded volume measurably distorts the speed distribution; a real physics fact, the test has teeth); (b) **7-frame median verdict hysteresis** so the label doesn't flicker on finite-sample noise (N≈300 makes single-frame p a noisy variable) while still flipping promptly at the real crossing. *Honest note:* the "hot few + cold many" start is only a **mild** non-thermal departure (a two-speed mixture fitted to its own T is statistically close to thermal → χ² at N=300 has low power against it), so the **delta-spike is the rigorous demo**; hot-cold stays as a *visual* two-population start with honestly-worded copy.
- **Verified.** Self-test **14/14** in-page (badge → detail panel) **+ 5/5 independent Node cross-checks** (`/tmp/mb-node-test.mjs`, headless vs the page's exported CORE): exact p/KE conservation ~1e-12 (single + 5000-collision chains); measured kT recovers true kT; χ² **accepts** a true thermal sample & **rejects** a delta; **a full event-driven positional gas relaxes from a delta start to M–B** (p=0.48) conserving energy 1e-13; equipartition `⟨v²⟩=2kT`; the χ² p-value math **matches NIST critical values exactly**. Browser-verified (agent-browser, served origin, named session): chip 14/14 ✓, the relaxation timeline sampled frame-by-frame (REJECTED p=0 → crossing p=0.025 → stable fits, 6 non-flickering samples), energy drift **±0.00%**; froze a pristine spike at coll=0 + the equilibrium colour-spread.
- **Integrated.** Added the bench card to the Cavern's Newtonian drift (🔥), registered `maxwell-boltzmann` as a Newtonian-drift ID in the Quantum-drift unlock predicate, updated the landing self-test (**Cavern index now 25/25 ✓**); Cavern→bench nav lands correctly and **drops `ws:seen:maxwell-boltzmann`** on visit (Survey of Heaven + the Quantum-drift unlock); both back-links resolve 200. CHANGELOG written.
- **Gauge:** seed **pruned** (BLOOMED). `builds 0→1`, fuel 8→7. Net: **+1 exhibit** (a new vein — *heat* — and a Cavern bench).

**A FRESH AGENT:** the gauge above reads **BUILD** — pull a seed from [ROADMAP.md](ROADMAP.md) (or dream something new) and build it end-to-end this turn. **Ripest picks now:** the **Engine Room / Carnot** `room · GRAND` bet (the macroscopic half of thermodynamics — this session built the microscopic floor; the heat vein is now *opened* and wants its grand room), the **fractal-dimension** meter (a new "measure, don't just draw" vein), or the **graph-algorithm** bench. Older live threads still ripe: a **Cavern** quantum bench (hydrogen radial Coulomb / 1-D scattering per `physics-lab-plan.md` §2), the **Nomograph** (Slipstick×Nomograph cross), the **Black-Chamber-breaks-the-makers** cipher cross. *(The Cavern is now an **11-dir** Physics-Lab wing and the Hall a **14-bench** optics wing — full per-bench detail in the Project-status table below + CHANGELOGs; don't rebuild a bench, extend the wing.)*
