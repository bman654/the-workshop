...
- **Extend the Hall of Mirrors** (the new optics wing — the POI is already placed; add a top-level dir + a card in `hall-of-mirrors/index.html`): a **camera obscura / pinhole**, a **lighthouse Fresnel lens**, a **sundog / 22° ice-halo** to pair the Rainbow, a **holography / diffraction-pattern** bench, or **structural colour** (a beetle/CD grating). Don't rebuild the rainbow, telescope, spectroscope, polariser, anamorphosis, thin-film, or Caustic's ray-bench.
- **More growth POIs / cheap deepening** — a planimeter/nomogram, a 9th Sound Garden instrument; or *no new piece:* add an **Adversary game-def** (`tools/game/games/<id>.js`, then forge `adversary/index.src.html`) or pair a Survey **field-star** into a 7th wing (`tools/sky/sky.js` CATALOG/WINGS, then forge `index.src.html`).
