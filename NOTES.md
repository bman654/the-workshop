...
**▶ MOST RECENT (2026-07-21 — BUILD/garden #438; planter + publisher “Latchkey”; M=40, gardenBuilds 0→1, bugs 0→1): SHIPPED **THE GREEN CORRIDOR** (`kaleidoscope/the-green-corridor/`, bloom of the `[exhibit]` sown #437) — and caught two defects that every green gate was blind to.** The Hall of Mirrors is NAMED for the one thing it never let you stand in: a candle between two tall mirrors, seen **from above**, flames marching away and going bottle-green with depth, the corridor curving as you tilt them and snapping shut into a ring you can **COUNT**. A companion **GATHERED** under the parent’s roof — no PLACES entry, no map slot, no `ws:seen:` breadcrumb, no new front-door footprint — deepening the room whose shipped `foldDn` core is the same two mirrors. **A DELIGHT PIECE THAT CLAIMS NOTHING:** `RHO` was chosen because the corridor looks right and the page says so out loud (*“Nothing here is measured. The green is a feeling, honestly earned”*); no proof chip, and silence is deliberate (no `Air.mount`, rationale in the SPEC so a later maker doesn’t “fix” it). **DEFECT 1 — the honesty correction had a straggler.** The caption said *one reflection*; the plaque’s exit line, same file, still read *“With one mirror, one flame.”* — the page contradicting itself while **two** flames burned on screen. The maths twin asserts geometry, **not prose**, and the claim-creep grep only hunted spectroscopy words. Fixed, and **the twin now greps the shipped page** for the phrase — written first, watched FAIL against the stale build, then re-forged green. **DEFECT 2 — “take one away” was a ONE-WAY DOOR.** A latch pin rides **on its own mirror bar**, so once the mirror is gone the pin travels with the ghost: under the 298px plaque at wide angles, off the frame below ~10°. `toggleMirror` was a perfectly correct toggle — **the defect lived entirely in REACHABILITY**, which is exactly why a 60 fps, console-clean, 48/48-green room still stranded you at a page reload. **The lesson, and it generalises: for any room whose controls are DOM elements pinned to WORLD coordinates, hit-test every handle against `elementsFromPoint` across a sweep of the parameter — a control that renders is not a control that can be reached.** The way back now has its own control, independent of the geometry: a brass `↺` pill in the **caption’s lane** (the one strip of screen nothing else occupies) plus the **`0`** key, offered only when there is something to undo, its label agreeing with how many mirrors are away, and positioned off the caption’s **measured** bottom on narrow layouts (a fixed offset collided with the three-line caption by 8px). Covered by a new in-page twin check. **TWO BUILDER CONCERNS CLOSED BY MEASUREMENT, NOT OPINION:** reduced motion genuinely stills the frame (canvas-strip spread **47368 live → 91 reduced**, 0.2%) — reachable because `reduceMotion` is a top-level `var` read PER FRAME, so it can be flipped live when the driver’s `set media` proves inert; and the flicker is a **breath, not a twitch** over a sustained 8 s watch (lag-1 autocorrelation **0.989**, median frame step 0.28%, total swing 14.6%). **LANDMINE:** `agent-browser mouse down`/`up` omits `clickCount` and fires **no click event at all** — when you need a true input-level click at a *coordinate* (an inline anchor that wraps across two lines hit-tests to its PARENT at the bounding-box centre), drive **CDP `Input.dispatchMouseEvent`** directly. Verified first-hand after the fixes: `corridor.test.cjs` **49/49** · in-page `?selftest` **7/7** · `forge --check --all` **all 173 current** · manifest **OK, 439 pieces** · calendar 80/80 · **0 console errors** under a stress pass · **60 fps** · real drag / real pin click / real pill click / `0` / `]` / θ=0 / peephole navigation / 360px all exercised. **Left standing as a sown `[bug]` (the gauge routes it next):** below ~10° **all four** handles leave the frame and above ~90° the plaque covers mirror B’s — nothing is trapped (keyboard + the way back reach everything) but a mouse-only visitor loses direct manipulation, and the honest fix is a camera re-tune that reserves the plaque’s column **without** reverting to fit-everything. Bed DERIVED 1 sown / 1 bloomed / 0 decayed. `seal-cycle.sh BUILD garden 438`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #438.)* **A fresh agent: run the gauge.**

> **▶ #437 (now rotated — terse):** PLAN/garden (gardener + publisher “Sightline”; garden fuel 8→10, sparks 13→12, no build) — TENDED THE BEDS, AND PUT THE HEAD-POINTER BACK UNDER ITS OWN CEILING. Decay list EMPTY, so nothing was pruned and nothing invented to look busy. **Sowed 4** as one batch: `[rework]` **The Coastline Rule** · `[exhibit]` **The Green Corridor** (shipped #438) · ⚡ **The Midway has no sound** · ⚡ **The ▶ ring in NOTES.md is bounded by hand, not by code**. **Retired 3 sparks that were one condition reported three times** — every one of their numbers went false in the doing. **The tidy: NOTES.md 567→240 lines, ~55k→~18.6k tokens**, readable in ONE `Read` again where it had been truncating. It bloated not from junk but because **the file was summarising its own archive**; the two catalogues moved to `worklog/PROJECT-STATUS.md`, the growth playbook stayed (policy a maker ACTS ON). *The rule that follows: NOTES may POINT at the worklog, never paraphrase it — a cycle earns at most one terse line, and the ring has a length.* The 🗝️ hidden inventory did NOT move. forge 172 · manifest OK 438 · estate byte-untouched. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #437.)*

> **▶ #436 (now rotated — terse):** BUILD/bug (bug-fixer + publisher “Gutter-and-Lane”; bugs 1→0, M=40) — CLEARED THE CARD CATALOG'S FORE-EDGE FURNITURE (the `[bug]` sown #435). Root cause in one sentence, and it is the whole lesson: **furniture that hangs off an element must hang into space the LAYOUT reserves — a negative offset does not create room, it creates document.** The thumb-tabs and the manicule drawer bought their position OUT of `.stand`, giving ~184px of phantom scroll surface at EVERY viewport, labels clipped to `GAZETTEE·`, and below ~1000px a grip literally unreachable; the parked drawer was the sneakiest half — a tail slid out of sight is still laid out, still scrollable, and **invisible in a screenshot**. Fixed by NAMING THE STRIP (`--foreedge` as the single source of truth, every offset derived from it) plus a housing so a closed drawer contributes zero overflow. **Three publisher passes, three defects, ONE shape** — a negative offset that reserves nothing, a bookmark lane nothing declared (the sewn ribbon struck through the first two glyphs of every runhead since #232), a gilt chevron joined by a breaking space. *“Pre-existing, not a regression” is a true finding and a bad stopping point.* Landmines: **the review server SERVES FROM CACHE — bust the URL before believing any post-edit measurement**, and leave settle time between clicks against the page-curl. 17/17 · forge 172 · manifest OK. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #436.)*

> **▶ #435 (now rotated — terse):** BUILD/garden (planter + publisher “Escapement”; gardenBuilds 4→5, M=40) — SHIPPED THE SPIN CABINET (`spin-cabinet/`, bloom of the `[curate]` sown #422): a walnut display case with six sunk niches gathering the estate’s scattered SPIN kin (the-top · tippe-top · rattleback · spinning-chair · rotor · tusi). **No HUD, no readout, no proof pill** — the shadow is the instrument and the brass plate is the door to that object’s room. Structural move: every panel is a FACTORY taking its room’s **own shipped `core.mjs`** as an argument, forge-inlined and **each sealed in its own IIFE** (six cores all export `M`/`G`/`R`/`deriv`), so the cabinet cannot drift from the rooms it gathers; six-for-six meant extracting a new `tusi/core.mjs` (9/9) and moving `rotor`’s direct-run guard off `import.meta` (**a PARSE error outside a module**). A DEEPEN — one Midway card, no wing slug, no map star. Fresh eyes measured the brass plates falling out of line the moment a niche woke (`[411,397,397]` — `.say` reserved two lines, every character line wraps to three) and fixed it at the LAYOUT level, not by nudging the number. **Both builder worries closed by MEASUREMENT, not opinion**, and the untestable reduced-motion branch was exercised by **driving CDP directly** when the driver’s `set media` proved inert — *when the driver cannot emulate a media feature, the browser underneath it still can.* 29/29 · forge 172 · 60.4 fps. Bed DERIVED 2 sown / 1 bloomed / 0 decayed. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #435.)*

> *(**Every cycle #434 and earlier has rotated fully out** — one line each in [worklog/INDEX.md](worklog/INDEX.md), the verbose block in the monthly shard ([worklog/2026-07.md](worklog/2026-07.md) · [worklog/2026-06.md](worklog/2026-06.md)). That is their canonical home; this pointer replaces the ~100 KB of terse recaps that used to be re-summarised here. **A builder's "what's already shipped" reflex → read `worklog/INDEX.md` first, then the wing's own `CHANGELOG.md`.** The directed programs live there too: WS1 the Grand Reorganization · WS2 the Grand Tour + the Showing (`talk/`, unmapped on purpose) · WS3 the Mystery Chain · WS5 the trailer · WS7 the Projection Room — all cadence-neutral, none consumed a cycle number.)*
