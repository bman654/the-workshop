...
> **An overnight `/fun` autorun (Brandon asleep — "set a heartbeat, keep at it until ~7am") shipped 13 pieces, each browser-verified, committed & pushed live; every shared surface QA'd clean (front door = 9 cards / 7 companion pills / "16 games"; all 93 live `.html` pages return 200; front door + Undercroft re-verified with 0 console errors).** In ship order:
...
>
> **Every piece carries a built-in self-test (the workshop's verifiable-crux tradition) and ships green.** Net growth: **+2 companion wings → 7 total (the companion axis is now FULL** — every eligible front-door card has one); **Arcade → 16**; a brand-new **logic-puzzle** family; the front-door **footer** is now an "extras" row (`puzzles · weave · light · reckon · colophon`); and **4 mediums the workshop lacked** — fluid-ink, tactile fiber, geometric optics, and working instruments/ciphers. **Front door deliberately UNCHANGED — still the curated 9 cards.** The **Undercroft** also grew (hidden — see its SECRETS manifest; *spoiler etiquette: don't name its trails/contents when summarizing to Brandon*).
>
...
> **To resume / continue:** read this block + the per-piece detail below. Clean growth axes left: **deepen the Arcade** (free), **add a hidden Undercroft secret/trophy** (many `ws:best:`/`ws:seen:` breadcrumbs now ship and are un-trophied), **grow a new vein's family** (instruments: astrolabe/abacus/sector · ciphers: Vigenère/M-209/bombe · tactile: kaleidoscope/ripple-tank), or the **bigger swing** (a 10th front-door standalone + the flat-grid redesign it implies). See the 🧹 **Curation items** lower down (back-link consistency on 6 card pages; footer "extras" grouping) — flagged for Brandon's daylight call, deliberately NOT changed overnight (outward-facing UX on core pages). **Guard context: delegate builds to self-verifying subagents; commit + push after each unit; verify live 200.**

> **▶ CURRENT STATE / RESUME POINTER (2026-06-12, `/fun` — 🔐 ENIGMA shipped → a BRAND-NEW VEIN [a working CRYPTOGRAPHIC DEVICE] AND a new SUBJECT [ciphers/codes the workshop had nowhere], placed as the Undercroft's 9th secret — NOT a 10th front-door card, NOT a new footer link).**
