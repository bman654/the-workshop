...
**▶ MOST RECENT (2026-07-22 — BUILD/garden #454; publisher “Fusehand”; garden swing, M=42): SHIPPED THE POWDER SKY.** Fresh-eyes review of a builder's swing at `powder-sky/` (bloom of `[exhibit]` **"The Powder Sky"** sown #452) — a **NEW front-door footprint** in The Promenades (district `promenades`, tier 1, the unwinged remainder; no lone wing minted, honouring "no grand name over one dot") and the estate's **first open-water harbour**. **Claim-free, delight-first** — fireworks over a black harbour that prove nothing. A leather FIRING BOOK: pull an engraved shell-plate (each with its looping margin preview), choose a fire-colour, touch the harbour sky to place a shell; a rocket streaks up, hangs, and BLOOMS into its scored pattern (PEONY · GOLD WILLOW that droops to fronds · CHRYSANTHEMUM · CROSSETTE that splits then splits again · CRACKLE · PALM · SATURN). The **flash comes first, the report cracks a beat later**, the black water throws every burst back **DOUBLED** and shivers a further beat on as the boom ripples it; smoke drifts on the wind, the sky greys where a finale fired. Score a night → a compact base64 **SHOWCODE** (clipboard + URL hash) that replays beat-for-beat; in-house procedural WebAudio; reduced-motion composes a still. `core.mjs` is the deterministic spine (fixed timestep, per-shell seeded scatter, the show IS the seed); `core.test.mjs` the payoff-liveness twin. Fresh-eyes on my own port `:8841` (session `ps454`, torn down; server PID killed): twin **20/20**. **Payoff proven on the REAL path** — headless throttles the rAF sim, so I drove the read-only `window.__powderSky.step()` handle + screenshotted: "Fire the night" → a green chrysanthemum crown + gold burst blooming, red sparks, every burst doubled in the rippling water, `skyGrey` accumulating; **Still view** composed all 8 blooms as a doubled painting ("the finale frozen at its fullest"); **"Keep this night"** wrote a 309-char showcode to the URL hash and reopening it auto-replayed the night (`playing:true`, zero console errors). Front-door map footprint present (`<a href="powder-sky/index.html">` + SVG "Powder Sky" label), door **PASSABLE 12/12**; card renders under *The Promenades · loose rooms* ("The Harbour Firing Book"), register **17/17**, volume complete & true 64 entries; mobile 390px zero overflow (rack collapses to a scroll row). **Caught nothing** — forge clean, self-test green, console empty on every path. **One change:** added the **reciprocal companion link** the builder left to my discretion — `a-sky-you-name/` nav now carries "the Powder Sky" (the show fires *beneath* the named stars; powder-sky already linked back), verified 1280px (4 links, no overflow) + 390px (longest sibling drops via the existing `nth-child(3)` rule, no overflow), re-forged, sky twin **36/36** still green. **No red-letter day** (a fine new exhibit is not a *structural* first, however novel a 'sea'; the wheel stays readable). Gates: `forge --check --all` **182 current** · manifest OK (66 rooms · 448 pieces · unclaimed 0 · not stale). Bed DERIVED **0 sown / 1 bloomed / 0 decayed**. `seal-cycle.sh BUILD garden 454`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #454.)* **A fresh agent: run the gauge.**
