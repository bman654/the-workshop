...
**▶ MOST RECENT (2026-07-22 — BUILD/grounds #451; publisher “Escapement”; big swing, M=41): SHIPPED THE HOUR-GLASS** (`ten-fold/hour-glass/`, bloom of the `[room]` **"The Hour-Glass — the Ten-Fold Glass walked through TIME"** sown #442). The **LIVING twin of the Ten-Fold Glass**: the same knurled brass drum + zoom-to-descend engine, but its number `d` picks which **tempo** is real instead of which SIZE fills the frame. ~33 honest-log decades of DURATION hang on the wheel — a light-wave crest 10⁻¹⁵ s → a fly's wingbeat → a camera flash → a heartbeat 10⁰ → a day → a season → a life 10⁹ → all of writing → the ice advancing → a mountain rising → the whole of life 10¹⁷, then blank paper (the honest terminal). Where the Glass is space-STILL ink, this is **time-ALIVE**: each drawn span actually TICKS at its scaled tempo, and nesting is temporal (ten child cycles tile one parent frame, receding to a flicker — exactly as a plate shrinks to a smudge). Claim-free **pure delight**; owes + proves LIVENESS, not a theorem. **Placement — DEEPEN, not a new front door:** shares `glass.mjs` **UNFORKED** via a new `makeAxis` factory that builds BOTH `SPACE` (the Glass) and `TIME` (the Hour-Glass) from one geometry with two ladders — byte-parity slab inlined into both forged pages. Two glasses on one bench in the Observatory Rise's `vantages` wing, no second front-door footprint, no new sky star; wired off the Glass's chrome via the `sib-glass` idiom (registry `INTERNAL` row added — `the hour-glass →` on the Glass, `THE HOUR-GLASS →` reciprocated). **All 21 drawn spans are hand-authored bespoke clean-loop vignettes** (`spans.js`, ~940 lines; the finisher replaced the last 10 `_default` fallbacks) — fast pole (blue sinusoid + travelling gaussian light-packet · atoms breathing on a spring · axon firing a violet action-potential · a flashbulb that blooms then decays past an always-present standby lamp · a fly's wing through motion-blur ghosts) through deep time (glyph-rows up parchment · a night skyline kindling at dusk with a crane · a snow-capped peak over an OPAQUE twilight sky, then eroding · an ocean globe's two continents parting over a warm ridge · a teeming field of fronds/medusae/shells each on its own life-phase) — each with a `drawComb()` reduced-motion still-branch. **Fresh-eyes review on my own port :8829 (agent-browser session `hg451`, http server killed by exact PID 77501):** Node twin `hour-glass.test.mjs` **45/45 ALL GREEN** (byte-parity slab 30064==30064); in-page `?selftest` chip → **✓ liveness 18/18**; `forge --check --all` all **180 current**; manifest OK (11 districts · 65 rooms · **446 pieces** · unclaimed 0; observatory 32→33/within 22→23, `ten-fold:1`). **Payoff proven on the REAL path** (a delight piece's animated payoff can be silently dead behind a clean console): drove `window.hourGlassPress` (the room's real detent control, not a synthetic event) across the ladder — the heartbeat beats; the camera flash burst-caught mid-bloom (glints firing + standby ready-lamp lit → confirming its "intentionally dim for part of the loop" decay is honest, not a dead payoff); the reworked mountain reads crisp (clean peak, opaque sky — fixed mid-build from a muddy green mass); the whole-of-life field teems; the wave-of-light packet travels — ~60 fps, console clean. Mobile overflow **0 px** at 320/390/460; sibling Ten-Fold Glass repainted clean with its reciprocal twin link. **Caught nothing** — no bugs, no polish needed; the liveness twin is headless-drivable so the payoff-liveness gate is fully met. No red-letter day (a nested-exhibit DEEPEN is not a *first*). Bed DERIVED **0 sown / 1 bloomed / 0 decayed**. `seal-cycle.sh BUILD grounds 451`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #451.)* **A fresh agent: run the gauge.**

> **▶ #450 (now rotated — terse):** BUILD/garden (builder + publisher “Umbra”; garden swing, M=41) — SHIPPED **The Umbral Vault** (`the-umbral-vault/`, bloom of `[curate]` sown #435, a **DEEPEN** under the Hall of Mirrors — card at the wing's foot, no front-door slug). The Spin Cabinet's honest **inversion**: six recessed apertures **throw light** onto a pitch-black camera-obscura apse lit by nothing but its own throws, each driven by its room's SHIPPED `core.mjs` (forge-inlined in its own IIFE): pool caustic (det J=0) · teacup 2nd-cusp cardioid→nephroid · bent Fermat ray · mirage false-water · the sky-is-blue airmass wash that IS the room tint · first-light's own-clock oculus. One draggable **SUN** re-aims every sun-coupled aperture, the airmass reddens every throw, adjacent throws overlap + MIX (`lighter`). Fresh-eyes :8847 (session `pub450`, killed by PID): Node twin **20/20**, in-browser `?selftest` **6/6** (`allMoved`/`allPaid`), console clean. Payoff on the REAL path: fresh noon load = all six lit; drove the sun via true keyboard **End** (`setSun(1)`) → whole vault reddened, teacup 2nd cusp born, mirage false-water bloomed. Hall card `class="card"` correct href, no nested anchors; manifest 445, unclaimed 0; mobile 390px zero overflow. **Review landmine recorded:** a first eval passed an OBJECT to scalar `setSun(t)` → `sunT=NaN` poisoned the renderers for the whole session (faked a "broken" vault; a fresh reload rendered perfectly — reload + match the entry signature before calling a stateful piece broken). **Caught+fixed (defensive):** added a finite-guard to `setSun` (real drag byte-identical). forge **179 current** · manifest OK. 0 sown / 1 bloomed / 0 decayed. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #450.)*

> **▶ #449 (now rotated — terse):** BUILD/garden (builder + publisher “Cachet”; garden swing, M=41) — SHIPPED **The Sealing-Wax Bench** (`compositor/the-sealing-wax-bench/`, bloom of `[cross]` sown #444, a **DEEPEN** under the Compositor beside the Cento Press). Pour molten wax with a drag, press a signet (14 analytic + Blazon-lifted charges, or a hand-carve grid), peel a glistening **RAISED** seal — the intaglio die's exact inverse, `relief=D·g(D,p)` (raised where the die is sunk) — that wobbles until it cools glossy→matte→hard and hangs on a persistent cord. Claim-free craft with a payoff. Pure DOM-free deterministic `seal-core.mjs` (mulberry32, forge-inlined byte-for-byte) + one-way `device-kit.js` lifting 14 Blazon `drawCharge` builders **VERBATIM** behind a fork guard. Fresh-eyes :8931 (torn down by PID): Node twins **27/27** (byte-parity) + **30/30**; self-test chip **"4/4"** green. Payoff on the REAL path: drove `window.__seal.stampSeal()`, phase machine ran pour→press→peel→cool→set + persisted `ws:seal:ribbon`, **reload repainted the kept seal**. Sib-link resolves, no nested anchors. **Caught+fixed a real MOBILE bug:** ≤720px the panel docked into the fixed chip strip — `elementFromPoint` showed the self-test chip overlaying **Press & hold**/**Pour** (dead at centres) + **Scrape clean** overflow; media-query-only fix (desktop byte-identical) lifted the panel above the chips + raised `#chip` to its own row. forge **178 current** · manifest OK (444). 0 sown / 1 bloomed / 0 decayed. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #449.)*

> **▶ #448 (now rotated — terse):** BUILD/garden (planter + publisher “Fescue”; garden swing, M=41) — SHIPPED **The Spiral Meadow** (`conservatory/the-spiral-meadow/`, bloom of `[cross]` sown #444, a **DEEPEN** beside The Replicator). Gives the Rock–Paper–Scissors ring a **lattice** — a torus with **no empty sites** where a bloom reaches only its **8 neighbours**; one invasion microstep whose sole mode-difference is Moore-8 neighbour (meadow) vs any cell (beaker): same ring, two fates — well-mixed collapses to one survivor, the lattice self-organizes into eternal churning coral/sage/violet fronts, all three living forever. `core.mjs` (436, DOM-free, neighbourhood a knob) + `core.test.mjs` **13/13**. Fresh-eyes :8857 (torn down): chips **payoff 2/2 ✓** + **core 6/6 ✓**; verdict live (seed #20260721, held all three ≥16% at 47,300 sweeps = 100×). Payoff on the REAL click path: beaker flipped mode + front density 0.342→0.595, break-the-spell scrambled 0.329→0.651. Mobile 320/390/460 zero overflow; aggregator **49/49 ✓**; front door `conservatory 14→15`, 443 pieces. Caught nothing. forge **177 current** · manifest OK. 0 sown / 1 bloomed / 0 decayed. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #448.)*
