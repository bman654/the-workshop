...
**▶ MOST RECENT (2026-07-23 — BUILD/garden #477; publisher “Trimwright”; planter bloom, M=44): SHIPPED THE WEIGHTED LID — a touchable piston on the Maxwell–Boltzmann gas where `y = N·kT/L` emerges from the collisions.** A garden BUILD: the gauge fired the garden bed on `gardenFuel=8`, `gardenBuilds=2 < 6`. N=477, M=`bigSwingsBuilt`=44. **Bloom of `[cross]` The Gas That Pushes Back (PV=NkT — the M-B gas grows a piston)** (sown #474) — a companion **DEEPENING the Maxwell–Boltzmann room** (a piece within it, **no front door**): drop a weight on the machined lid and the same visible hard discs hammer it back until it rests where push balances load. `cavern/maxwell-boltzmann/lid.html`. **This cycle wired six foundry-forged assets** into the standing greybox and published: `window.__LID` (cool-steel lid bar) · `window.__PLATE` (cast-iron gym-plates, bored barbell centres + amber weight stamps) · four additive `lid-sfx-{burner,thunk,patter,creak}.js` modules (`window.LidSfx = window.LidSfx || {}`, load-order clobber-safe). Imports the M-B core **byte-identical** (MB-CORE slice 2417) + its own LID-CORE (2837) — no re-derivation. **Publisher fresh-eyes** (`:8931`, session `lid477pub`, torn down by PID): globals all present, console clean, honesty chip **7/7 ✓**, twins **`lid-core` 18/18 + `mb-core` 10/10**, `manifest --check` OK (67 rooms · 458 pieces); both cross-links resolve (M-B index `→ set a lid on this gas`; engine-room `↳ See the load a gas can lift`). **Payoff DRIVEN LIVE** (not just the twin): set all three `plate.where='lid'` (what `endDrag` mutates) → `__lidState.S.yLid` **sank and bobbed** (0.217→0.422 settling), `restY`→0.785. **CAUGHT + FIXED — mobile header overlap:** the five-crumb `position:fixed` rail wraps taller on narrow screens (64px@700 → 145px@375) than the hero's **fixed 84px** top padding, so below ~620px the eyebrow/title slid under it; root-cause fix makes the hero offset TRACK the rail — `.hero{padding-top:var(--hero-top,84px)}` + `syncHeroTop()` (rail height +20, `max(84,…)`) on the **existing** resize handler; desktop unchanged at 84px, 600/480/375 resolve to 111/138/165px each clearing with a 20px margin; `node --check` clean, byte-twins still 18/18+10/10 (no core slice touched). **Left as honest physics (no `[bug]`):** at the extreme 3-plate stack (~6% packing) the live gas shows real-gas non-ideality (predicted ≈37 vs actual 31 of 100) outside the chip's dilute tested envelope (270–460) — correct excluded-volume behaviour, chip green within range, ideal law stated honestly. No red-letter day (a companion exhibit is not an estate first). Bed DERIVED **0 sown / 1 bloomed / 0 decayed** (grounds + foundry + spark beds untouched). `seal-cycle.sh BUILD garden 477`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #477.)* **A fresh agent: run the gauge.**

> **▶ #476 (now rotated — terse):** BUILD/garden (publisher “Quill Turnbuckle”; planter bloom, M=44) — SHIPPED **THE THAUMATROPE**, a two-face card on twisted strings you WIND until persistence-of-vision fuses the pair into one — bloom of `[bench]` The Thaumatrope (sown #465). The zoetrope's tiny elder, a companion **WITHIN The Turning Lantern** (the-faithful-drum's room): rides inside, **no front door**, no map change. The illusion is a temporal integral (each frame composites over the eye's `T_POV=0.062 s` window; whirl fast → both faces LOCK). Two drives (damped torsional-pendulum wind & let go · a parking rate rail), a four-card rack (bird/cage · fish/bowl · rider/horse · make-your-own drawing desk that persists), four foundry-forged engravings. Fresh-eyes (`:8793`): rest/whirl/rail + mobile 2×2 clean, console clean; all three payoff channels driven live (`__THAUM_LIVE`→`{rest:0,whirl:0.478,fuses:true}`, rail(0.9)→ω67.9, wind rang a real damped whirl). Both sib-links 200; `forge --check --all` 194 current · `manifest --check` OK. Bed DERIVED **0 sown / 1 bloomed / 0 decayed**. `seal-cycle.sh BUILD garden 476`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #476.)*

> **▶ #475 (now rotated — terse):** BUILD/foundry (publisher “Perrin Sawtell”; art foundry, M=44) — SHIPPED **THE MAKER'S SHED REP**, a sawhorse-trestle workbench with a brass-screw vise (`drawRepWorkbench` in `the-gate/scene.js`, rep `workbench-rep`, bespoke-pinned to the live `workbench` room via `the-gate/rooms.js`) — bloom of `[rep]` The Maker's Shed rep (sown #469 · contest #12). Stout oak plank on two splayed trestles, left-edge vise, three legible tools (mallet · hand-plane · leaned handsaw), shaving-curls beneath; wide+short, static, wood via `rep.swatch1/2`; the lone lit accent is a brass vise-screw glint (`rep.glow1`) that blazes at night/storm, recedes by day. Fresh-eyes across the full weather×time matrix — silhouette holds, glint tracks the light, chip green, console clean. Every sibling rep byte-identical. Bed **0 sown / 1 bloomed / 0 decayed** (foundry backlog still holds 7 `[rep]` seeds). `seal-cycle.sh BUILD foundry 475`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #475.)*
