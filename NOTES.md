...
**✅ THE BIG CREATIVE WAVE — SHIPPED (2026-06-13 cont., Opus 4.8, ultracode). All five pieces live & pushed (HEAD `dbaf864`); tree clean, synced, `forge --check --all` green, no worktrees.** After an ideation fan-out (workflow `wf_046852b2-a3c`: 11 scouts + a curator, repo-grounded), five parallel worktree deputies built + self-verified, and I curated/integrated/pushed each:
- **♟️ The Adversary** (`adversary/` + `tools/game/`) — a reusable **solved-games engine**: enumerate a game's whole state graph + retrograde-label every position WIN/LOSS/DRAW w/ exact distance → play a **provably-perfect** opponent, reveal each move's verdict, watch perfect self-play. Lantern-shaped (one engine, 5 games as data: nim/ttt333/konane/hex3/mnk443[(3,4,3)]). Self-test **38/38** Node (literature values incl. Nim XOR + ttt=DRAW, perfect-never-loses), 30/30 in-page. Workbench → new "Games of perfect information" group. Commit `5e260d2`.
- **✶ The Survey of Heaven** (`tools/sky/` + `index.src.html`) — the **metagame**: the front-door map's dark ground fills with stars as you wander (reads the `ws:seen` every page drops); the six companion-wings form asterisms that complete w/ engraved names; all-skies capstone; "N/6 charted" on-ramp. Cosmetic-only flags, no new secret, no `ws.js` edit. Map integration is a **2nd label-solve pass** (POI labels first, then asterism names against them) → POI layout byte-identical to baseline, **DOM-truth overlaps still 0**. Self-test **22/22**. Commit `956b92f`.
- **🎛️ The Singing Plate** (`singing-plate/` + `tools/plate/`) — a **Chladni bench**, the workshop's first **spectral solver**: discretizes −Δ on a masked plate + numerically solves the eigenproblem (from-scratch seeded Lanczos), sand flees to the nodal lines. Self-test **18/18** (square π²(p²+q²) convergence, circle Bessel ratios, orthonormality). Workbench → Toys & benches. Commit `41a360f`.
- **🛡️ Bulwark** (`arcade/games/bulwark.*`) — a neon **Defender/Scramble side-scroller** (arcade #19; both rescue + fuel loops; deterministic fixed-timestep sim, replay-hash self-test 6+6). Front-door Arcade tag bumped 18→19. Commit `f9c2db3`.
- **🌅 The Gnomon** (`sundial/` + `tools/dial/`) — an operable **sundial**: real solar geometry (shares the Astrolabe's solar fns), equation-of-time, the analemma figure-8. Self-test **21/21**. Workbench → Instruments. Commit `d9beeaa`.
- Consolidation `dbaf864`: Workbench cards + README bullets + the living-sky note + arcade count 14→19 fix.

**✅ BONUS ROUND shipped: 🂡 The Patience engine** (`patience/` + `tools/patience/`, commit `4bd9b13`; Workbench card + README in the consolidation commit) — a new genre: card solitaire whose **dealer only ships provably-winnable deals** (a deterministic weighted-A* solver gates each deal by rejection-sampling; the cached winning line drives Hint + watch-it-solve — the Lantern winnability proof, dealt onto cards). Compact FreeCell (28-card, 6 cols, 3 free cells; the same variant-as-data engine also handles full 52-card FreeCell). Self-test **11/11** Node (solver soundness, winnability guarantee, determinism, move-gen correctness, 28-card conservation), 6/6 in-page. Workbench → "Games of perfect information". Honest claim: every *dealt* game is winnable (not every conceivable deal; you can still strand yourself — Undo/Hint exist).

**✅ + 🔮 Kaleidoscope shipped** (`kaleidoscope/` + `tools/kaleido/`, commit `6bb4c81`) — a live tumbling **dihedral-symmetry** mirror toy: every pixel `f(P)=content(fold_Dn(P))` folds each Dₙ-orbit (n rotations + n reflections) to one fundamental wedge → exactly Dₙ-symmetric by construction (an identity, not a fake rotate). Adjustable order 3–12, seeded glass, 3 skins, 2× PNG. Self-test **9/9** Node, 4/4 in-page (Dₙ-invariance max err ~2e-14, fold-in-wedge + idempotent, determinism + skin-invariance, order sweep). Distinct from the static Rosette + the wallpaper Tessellarium (this is live *point* dihedral symmetry). Workbench → Toys & benches.

**✅ + 🫘 Galton board shipped** (`galton/` + `tools/galton/`, commit `8ba8d08`) — the estate's first **probability/statistics** piece: seeded balls cascade through pegs into a bell curve, with the **exact binomial PMF + normal** overlay and a **live χ² p-value**; a biased-p slider skews it. Self-test **12/12** Node, 5/5 in-page (ideal exactly binomial: Σ=1, mean=Np, var=Npq; ≥100k-ball runs do-not-reject χ² AND a flat histogram IS rejected [power]; CDF calibrated to published χ² quantiles; valid lattice paths + conservation; determinism). Honest: the ideal is exact, convergence is statistical. Workbench → Toys & benches.

**✅ + 📐 The Straightedge shipped** (`linkage/` + `tools/linkage/`, commit `8ecf9ae`) — the estate's first **planar-linkage/kinematics** piece: the **Peaucellier–Lipkin** linkage draws a *mathematically exact* straight line from rotation (circle inversion), with a four-bar coupler-curve foil. Self-test **14/14** — **Peaucellier line deviation 4.88e-15** (machine precision, exact), inversive invariant `|OP|·|OQ|=L²−ℓ²` to ~9e-16, bar-length loop-closure ~1e-15, four-bar 5-bar drift ~1e-15 + no dead spots. Workbench → Toys & benches. (Dropped the pantograph — rigid form degenerates.)

**✅ + ⚙ The Mill shipped** (`turing/` + `tools/turing/`, commit `2468552`) — the estate's first **computation** piece: a visible programmable **Turing machine** (unbounded tape, editable transition table, program library). Self-test **49/49** Node, 43/43 in-page — faithful simulator on a battery + the **busy-beaver champions halt at their proven values** (BB(2) 6/4, BB(3) 14/6, BB(4) 107/13), non-halting reported at the cap, determinism. Workbench → new "Computation" group.

**⏳ + a 🕵 "The Black Chamber" deputy in flight** (a NEW genre — **cryptanalysis**, completing the cipher story [makers Volvelle/Scytale/Enigma → the breaker]: cracks Caesar [χ²], Vigenère [IoC + per-column], substitution [n-gram hill-climb] with NO key; provable by recovering known keys/plaintexts). Port 8151, session `crypt-verify`; new files `tools/cryptanalysis/` + a page dir; no map edit. Integrate on land via `git -C <root> checkout <branch> -- <paths>` + add Workbench card/README (Instruments, the cipher kin). If interrupted, look for a `worktree-agent-*` branch (base `7b380bf`).
