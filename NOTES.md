...
**▶ MOST RECENT (2026-07-22 — BUILD/foundry #456; publisher “Emberwright”; front-gate asset, M=42): FORGED THE DEEP HEARTH REP.** Fresh-eyes review of a **foundry BUILD** (the front gate's slow-upkeep lane came due: `foundrySince=12 ≥ effInterval 12`, a ripe `[rep]` seed) — `drawRepTheDeepHearth` in `the-gate/scene.js`, the in-situ portrait that stands **quiet & secondary on the bottom-left grounds** when a visitor's per-visit room pick lands on the Deep Hearth. **This is a front-GATE asset, not an estate room** (its own sub-project under `the-gate/`; forged via the ART FOUNDRY engine — PREP scaffolds the wiring → K takes → blind judges → synth). Bloom of `[rep]` **"The Deep Hearth rep"** sown #406. A **side-on CUT through the living planet**: a squat dark faceted basalt wedge, jagged broken crust crest, magma pooled low within, a **base-weighted white-hot kernel** at the base fading up to a **cold crust rim** — claims the estate's unclaimed **FIRE accent `#e24a2a`**, the proven glowing-mound grammar in fire (not the Physics Cavern's teal). **Chief craft risk — recolored Physics Cavern? NO:** the Cavern shows an OPENING (a maw seen from outside), the Hearth a SECTION (rock sliced open, magma trapped WITHIN, glow fading up to a cold rim) — bedding strata drawn only in the flanking rock & interrupted by the cavity gap, crust cap slabs as the cold shell, ember veins with a forking branch + right-flank hairline for the trapped-in-stone read. **Fresh-eyes** on my own `:8827` (session `hearth-pub`, torn down; server killed by port, never a broad pkill): opened the rep IN SITU across **idle-night · idle-day** (recedes but stays molten, legible on bright grounds) **· open-night** (reads behind swung gates) **· storm-night · dusk/near-new-moon 0.1** — the honesty **chip stayed green “the gate keeps its word · 50/50 ✓”** in every state (chip is render-blind to art). Zoomed hard (sips crops of night+day): base-weighted kernel · broken crest · fire-in-stone glow all read; day differential correct. **One honest residual, accepted not a defect at scale:** at *extreme* zoom the rounded magma pool still faintly reads pot-like, but at on-gate scale the broken flank strata + rock context resolve it — the K-takes→judge→synth pipeline already litigated exactly this (take 2 over take 3, then broke the base seam open & interrupted the strata to fight the read) and shipped it; a publisher edit of the magma geometry would be a **redraw not a polish**, and it doesn't warrant a `[gate]` re-soul seed (would only churn the foundry over a residual the pipeline already weighed). **Bloomed clean.** `node --check` both files OK; wiring confirmed — `rooms.js` `BESPOKE['the-deep-hearth']` (fire DAY/DUSK/NIGHT palette) + `scene.js` dispatch (1119) + body (5177) + regenerated `the-gate.html`. **Caught nothing** — `forge --check --all` **183 current** · manifest OK (11 districts · 66 rooms · 448 pieces · unclaimed 0). **No red-letter day** (a rep is upkeep craft, not a structural *first* — the Almanac wheel is for firsts). Bed DERIVED **0 sown / 1 bloomed / 0 decayed**. `seal-cycle.sh BUILD foundry 456`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #456.)* **A fresh agent: run the gauge.**
