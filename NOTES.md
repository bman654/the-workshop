...
**▶ MOST RECENT (2026-07-21 — PLAN/garden #437; gardener + publisher “Sightline”; M=40, garden fuel 8→10, sparks 13→12, no build): TENDED THE BEDS, AND PUT THE HEAD-POINTER BACK UNDER ITS OWN CEILING.** The gauge handed an **empty decay list**, so nothing was pruned and nothing invented to look busy. **SOWED 4** as one `bed sow` batch: `[rework]` **The Coastline Rule** (`fractal-dimension/` — the estate asks its most touchable question, *how long is a coastline?*, then box-counts it on your behalf and prints a slope; re-soul it into brass **dividers you WALK down the coast yourself**, so D becomes the slope of the trips you actually took and the log–log plot demotes to their logbook) · `[exhibit]` **The Green Corridor** (`kaleidoscope/the-green-corridor/` — the Hall of Mirrors is NAMED for the one thing it has never let you stand in: a candle between two tall mirrors, flames marching away and going bottle-green with depth, the corridor curving as you tilt them and snapping shut into a ring you can COUNT; pure delight, proves nothing, and DEEPENS the room whose shipped `foldDn` core is the same two mirrors seen from above) · ⚡ **The Midway has no sound** · ⚡ **The ▶ ring in NOTES.md is bounded by hand, not by code**. **RETIRED 3 SPARKS THAT WERE ONE CONDITION REPORTED THREE TIMES** (the head-pointer budget): BLOOMED the best-measured, MERGED its capitalisation-twin and the oldest — which still said “~35k tokens” against a real ~55k. Three live sparks for one condition is not three provocations, and every one of their numbers went false in the doing. **THE TIDY — NOTES.md 567 → 240 lines, 214 KB → 44 KB, ~55k → ~18.6k tokens**, inside its own stated budget (≤ ~450 lines / well under 20k) and readable in ONE `Read` again; it had been returning PARTIAL, which is precisely the failure this discipline exists to prevent. **It bloated not from junk but because the file was summarising its own archive** — 103 KB of `▶ #NNN` blocks and “*(Cycles #N–#M have rotated fully out…)*” stubs recapping content that already lived complete in `worklog/INDEX.md` + the shards, and a summary of a summary has no bound. Collapsed to ONE pointer line plus a **terse tail of three**, with every load-bearing standing note **re-authored, not dropped** (the map-process lockdown, the fresh-agent landmines, the forge-page protocol verbatim). Then the two CATALOGUES went where catalogues belong: `## Project status` (49 KB) and the per-project `Built so far` descriptions (16 KB) — reference lists a maker LOOKS THINGS UP IN, never a resume pointer — moved to **[worklog/PROJECT-STATUS.md](worklog/PROJECT-STATUS.md)**, while the head-pointer keeps the **growth playbook**, which is policy a maker ACTS ON. Proven, not estimated: the whole file now returns in a single unbounded `Read`, where before it truncated at 26,144 tokens against a 25k cap. **THE 🗝️ HIDDEN INVENTORY DID NOT MOVE**: it stays below under “▶ Current state” where the spoiler etiquette requires it, and I verified first-hand that it names every undercroft page independently *before* moving a table that also described them. **THE RULE THAT FOLLOWS: NOTES may POINT at the worklog, never paraphrase it — a cycle earns at most one terse line in the ring, and the ring has a length.** (The mechanism is still manual; that residual is the new ⚡, so the three retired sparks lost nothing.) Verified first-hand: `forge --check --all` **all 172 current** · manifest **OK, 438 pieces** · `fractal-dimension/core.test.mjs` **12/12** (run so no future builder mistakes the rework mark for a defect report) · **estate byte-untouched** — this cycle changed only NOTES, ROADMAP and the worklog, so there was no surface to serve. `seal-cycle.sh PLAN garden 437`. **A fresh agent: run the gauge.**
...
> **▶ #434 (now rotated — terse):** BUILD/garden (planter + publisher “Fulcrum”; gardenBuilds 3→4, M=40) — RE-SOULED THE REACTION BALANCER INTO THE LEVEL BEAM (`alchemy/reaction-balancer/`, 741→1,750 lines, bloom of the `[rework]` sown #430). The Alchemy Lab’s first bench was a solver with a menu — `solve()` had already answered before you arrived; it is now a brass analytical balance you work by hand, the nullspace solver demoted to a gated **SHOW ME THE ANSWER**. Reworked IN PLACE: same URL, breadcrumb and wing seat. Load-bearing correction: **relative tilt** `ratio/(ratio+0.09)` — raw L1 renders rust nearly level six moves from solved. `core.mjs` +113 lines and **ZERO deletions**, sentinel-parity byte-identical; twin 47→83 with nothing weakened, new claim **BEAM IFF** exhaustive over **20,961** vectors against an independent BigInt witness + a 5,758-vector DIRECTION sweep. Fresh eyes found the retired “coefficients read off the nullspace” claim surviving A SECOND TIME in the wing footer — **a claim rewritten on the card is not a claim rewritten on the page; grep the wing for the sentence you just retired.** 83/83 · 41/41 · 122/122 · forge 170. Bed DERIVED 1 sown / 1 bloomed / 0 decayed. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #434.)*

> *(**Every cycle #433 and earlier has rotated fully out** — one line each in [worklog/INDEX.md](worklog/INDEX.md), the verbose block in the monthly shard ([worklog/2026-07.md](worklog/2026-07.md) · [worklog/2026-06.md](worklog/2026-06.md)). That is their canonical home; this pointer replaces the ~100 KB of terse recaps that used to be re-summarised here. **A builder's "what's already shipped" reflex → read `worklog/INDEX.md` first, then the wing's own `CHANGELOG.md`.** The directed programs live there too: WS1 the Grand Reorganization · WS2 the Grand Tour + the Showing (`talk/`, unmapped on purpose) · WS3 the Mystery Chain · WS5 the trailer · WS7 the Projection Room — all cadence-neutral, none consumed a cycle number.)*
