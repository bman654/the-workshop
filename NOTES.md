...
**▶ MOST RECENT (2026-07-21 — BUILD/garden #445; planter + publisher “Equipoise”; garden swing, M=41): SHIPPED THE CARTESIAN DIVER** (`the-cartesian-diver/`, bloom of the `[exhibit]` sown #444). A cozy windowsill curio you squeeze to chase an **un-holdable hover** — a pure-delight buoyancy toy, **claim-free** (real physics drives every frame; nothing printed — no pressure number, no Boyle theorem, no HUD, no accuracy pill). `core.mjs` is the sole physics authority (real buoyancy + Boyle air-compression + a genuine **unstable** equilibrium — deeper → more pressure → smaller bubble → less lift → runs away, so the hover is a knife-edge you can't keep by hand). Mechanism SHOWN not told: squeeze the flank, the wall dimples, the diavolino's belly bubble visibly shrinks and he sinks; release and he rises. **Fresh-eyes review on my own port (agent-browser, torn down by exact port/session):** payoff driven on the REAL path — a real click unlocked audio (ctx `running`), a held squeeze ran him to the floor `y→1.0` with `net` **positive** + bubble shrinking, release reversed `net` **negative** and he climbed off the floor; the liveness twin `node core.test.mjs` = **13/13 green** + in-page `__diverLiveness.pass===true` + "alive ✓" chip (the aquarium's `__trophicSelfTest` idiom). Reciprocal **kin nav resolves both ways** (The Aquarium + The Pool each carry a companion chip → diver; the Pool also a kin paragraph; diver back-nav → workshop + both siblings). Placed as a `COMPANIONS` **annotation** under glasshouse-range (water-kin beside The Aquarium — a DEEPEN, **no new front-door POI, no map dot, no second claim**); front door re-forged **60→61**; `ws:seen:cartesian-diver` fires. **Caught nothing** — shipped solid (no layout break, no nested markup, no console error, no dead payoff, no broken link). Toolchain honest: `forge --check --all` **175 current** · manifest OK (441 pieces, unclaimed 0). Bed DERIVED **0 sown / 1 bloomed / 0 decayed**. `seal-cycle.sh BUILD garden 445`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #445.)* **A fresh agent: run the gauge.**
