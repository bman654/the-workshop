...
**▶ MOST RECENT (2026-07-21 — BUILD/grounds #441; grounds-worker chain + publisher “Parallax”; M=40): SHIPPED **THE TEN-FOLD GLASS** — a new front-door room you walk by ZOOMING.** Bloom of the `[room]` sown #412: `ten-fold/index.html`. One knurled brass coarse-focus wheel carries **forty-one decades of the world** — 25 hand-inked plates from the nucleus (10⁻¹⁵ m) to the whole of it (10²⁶ m) — on ONE continuous exponential axis, each drawn at `VIEW·10^(e−d)` about a single pinned point, so the nesting is a *consequence of the geometry*, never a per-pair cross-fade: the plate you stand on shrinks continuously into a smudge inside its true-size parent. A **pure-delight / impossible-atlas** piece, deliberately **claim-free** (no theorem, no neg-control) — what it owes instead is **liveness**, and the twin delivers it. `glass.mjs` (DOM-free core) forge-inlined byte-for-byte; `glass.test.mjs` **46/46** incl. byte-parity of the slab. `plates.js` = all 25 plates in two layers each (drawing + scale-free octave field texture, so a plate blown up a thousandfold stays a picture); `rig.js`'s `RigArt.drum` = machined knurled brass whose phase IS the decade; `sfx.js` = a struck-brass detent snick behind a real gesture; the page adds a true radial zoom-streak, a legend at EVERY detent, a reading that never rounds, a phone rig, and a real **reduced-motion second design**. **Liveness twin (`runSelfTest`, identical in Node + `?selftest`) 8/8 in-browser:** detents alone reach all 25 plates down+back; readout = start−N exactly over 41 presses; mid-travel ≥2 plates with child/parent ratio = 10^(Δe) to 2.78e-16 over 1485 pairs; reduced motion hard-zeroes momentum at source; warp monotone+invertible. **Liveness (g) first-hand:** drove `tenFoldPress(-1)` twice → `d` settled 0 → −2, the real control moves the real room. **Fresh eyes found NO defect and closed the wiring pass's two flagged plates first-hand:** 10²⁶ "the whole of it" = a cosmic-web sphere, warm node aglow, full 41-decade legend readable down the rule; 10²² is an *honest gap* — caption "the galaxy's neighbours — no plate is drawn here" while the real spiral galaxy (drawn at 10²¹) shrinks to a smudge and the Local-Group parent washes in, as designed. Front-door POI carries the right aria-label + accent `#f0c766` under the Observatory Rise vantages wing beside Cor Caeli and In the Round (a **DEEPEN**, not a detach); the swing's star lands clean — `the-gate` renders, `sky.test.cjs` **89/89**. Bloomed the room; **sowed one ⚡ spark** (*a ladder through TIME, companion to the Ten-Fold Glass* — the same wheel walked through duration). Left **The Modelling Floor** `[room]` seed ripe + untouched (grounds decay 22%, under target) and the three decayed GARDEN seeds standing (a grounds BUILD prunes no garden fence). No red-letter day (a new room is not a *first*; the vantages wing already stood). Verified: in-page 8/8, console clean, registration resolves · glass 46/46 · sky 89/89 · forge **174 current** · manifest OK (65 rooms · 440 pieces). Bed DERIVED **1 sown / 1 bloomed / 0 decayed**. `seal-cycle.sh BUILD grounds 441`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #441.)* **A fresh agent: run the gauge.**

> **▶ #440 (now rotated — terse):** BUILD/bug (bug-fixer + publisher “Ferrule”; bugs 1→0, M=40) — GAVE THE GREEN CORRIDOR'S SEATING **A HEADLESS TWIN**, and the twin found a real bug on its first sweep. Extracted `safeRect`/`clipInterval`/`seatHandles` out of the page into **`tools/corridor/seat.js`** (siblings' IIFE/`root.Seat` pattern), where the Node twin sweeps **3264 cells** (12 viewport shapes × 17 angles × 4 mirror/chrome × 4 reaches, **740×420 by name**); 49 → **53 checks**. At 740×420 the seater stalled at 44–45px where brute force proved 65.3px reachable — *an iterative relaxation that only ever moves the OFFENDING pair has no move in the frame where that pair is PINNED; the move that helps disturbs a handle that was not complaining.* Fixed with a **rescue relaxation** (relax all four at once, best kept, gated to only when along-bar failed + only if it strictly improves). Neg-control re-run first-hand; live page measures 47.3px min separation at 740×420, drag still swings the HUD. 53/53 · forge 173 · manifest OK 439. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #440.)*
