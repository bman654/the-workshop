...
**▶ MOST RECENT (2026-07-22 — BUILD/garden #457; publisher “Straightwright”; garden bench DEEPEN, M=42): SHIPPED HART'S INVERSOR.** Fresh-eyes review of a builder's **new bench in The Drawing Room** (bloom of `[bench]` **"The Straight Line with One Bar Fewer — Hart's Inversor"** sown #452) — `the-drawing-room/hart-inversor/`, a garden DEEPEN sibling to `mechanism-bench/` + `pantograph/` (the manifest auto-claims the bench dir under the drawing-room court — **no new front-door POI**). Hart's crossed four-bar **antiparallelogram** inverts a circle into a **mathematically EXACT straight line using ONE fewer link than Peaucellier** (four bars, not six): turn the weighted flywheel crank, the scissors sweep P across a closeable arc, and pen Q lays a ruler-true walnut-ink line; a blue-chalk least-squares GHOST + a DEVIATION-FROM-STRAIGHT gauge + a live `OP·OQ` readout make the exactness legible. A TRUE⇄OFF-SPEC detune thumbwheel bows the line into a visible arc; a CROSS⇄UNCROSS fold toggle folds it to a genuine **parallelogram twin** (shared pole O + crank G, its locus NOT straight — the neg-control); an engraved **HART·4 / PEAUCELLIER·6** plaque carries the "one fewer bar" wonder statically (a second live linkage was deliberately dropped for byte/overflow safety). Drafting/Blueprint/Brass skins + 2× PNG export; **silent by design** (no air chip — matches the Drawing Room's quiet). Math authority: `core.mjs` (~432 lines) copies mechanism-bench's Solver (Gauss–Newton + orientation-sign branch guards) verbatim between HART-INVERSOR CORE byte-sentinels + adds the Hart plate assembly (a=3.2/b=1.7/s=½/d0=1.05, O & P on rigid gusset triangles → locus at machine-ε), the parallelogram twin, an auto-detecting closeable-arc finder, `runSelfTest`; shipped `index.html` inlines that slab byte-for-byte. **Fresh-eyes** on my own `:8831` (session `hart457pub`, torn down by port/session, never a broad pkill): Node twin `core.test.mjs` **9/9** (exact line lineMaxDev 1.24e-13 span 1.638 ⟂OG · OP·OQ=(a²−b²)s(1−s)=1.8375 drift 2.3e-14 collinear 1.3e-13 · every bar rigid 7.3e-14 · crossed fold 0 flips/guard rejects · detune monotone bow + parallelogram dev 4.1e-1 · byte-parity 27585==27585); in-page pill green **"Hart's inversor verified — 5/5 ✓"**. **Payoff proven on the REAL crank path** (headless can't click canvas → drove the piece's own `window.__hartLiveness()`): **trueDev 1.49e-13 (straight FIRES) · bowDev 0.227 (detune break FIRES) · foldDev 0.411 (fold-flip break FIRES)** — all three payoffs live, not merely rendered. Screenshot: brass linkage draws, gauge **DEAD STRAIGHT "0 (≤ ε)"**, OP·OQ 1.8375, collinear **yes·0**; console clean over 2.5 s. Drawing Room's formerly-dark Hart chip lit → **live link + reciprocal**; back-links resolve (`../index.html` · `../../linkage/index.html` Peaucellier · `../pantograph/`). Front-door tallies re-forged (Number district 48→49, drawing-room 9→10); `manifest --check` **OK — 11 districts · 66 rooms · 449 pieces · unclaimed 0**. Zero horizontal overflow at 320/390/500/768/1280. **Caught nothing** — build shipped solid, no polish needed; `forge --check --all` **all 184 current**. **No red-letter day** (a bench is a DEEPEN, not a structural *first*). No spark (no scaling strain). Bed DERIVED **0 sown / 1 bloomed / 0 decayed**. `seal-cycle.sh BUILD garden 457`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #457.)* **A fresh agent: run the gauge.**
