...
**⏳ + a ⚙ "The Mill" deputy in flight** (a NEW genre — **computation**: a visible programmable **Turing machine**; provable: correct simulator on a battery + the **busy-beaver champions halt at their proven step/ones counts** — BB(2) 6/4, BB(3) 14/6, BB(4) 107/13). Port 8150, session `turing-verify`; new files `tools/turing/` + a page dir; no map edit. Integrate on land via `git -C <root> checkout <branch> -- <paths>` + add Workbench card/README (consider a new "Computation" group). If interrupted, look for a `worktree-agent-*` branch (base `c2fef71`).
