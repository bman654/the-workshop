...
**▶ MOST RECENT (2026-07-21 — BUILD/garden #447; builder + publisher “Ebb”; garden swing, M=41): RE-SOULED THE COASTLINE RULE** (`fractal-dimension/`, a **`rework`** shipped in place — same route, same `ws:seen:fractal-dimension` crumb, same manifest entry). The auto box-counting chart-museum became **a paradox you WALK with your own brass dividers**: set a span, plant a two-armed caliper foot-over-foot down a candlelit Koch shore, read the length **you** measured; shrink the span, walk again, and feel your tally refuse to settle. `D` stops being a printed number and becomes the **slope of the walks you actually took**; the log–log plot survives only as the demoted logbook of your own trips. `core.mjs` (190 lines) = one small pure engine: `kochCoast` (similarity D=log4/log3), `smoothCoast` (3-sine neg-control that makes the fan OPEN on the fractal / SHUT on the smooth shore), the canonical `walkDividers(poly,s)` primitive, `richardson`. **Fresh-eyes review on my own port :8942 (agent-browser session `pub447`, torn down by exact PID/close):** desktop clean, self-test chip **12/12 ✓**; **payoff proven on the piece's OWN path headless** (a canvas takes no headless pointer event, so I drove the real entry fns) — `livenessProbe()` `grew:true` (rev 1→6, L 0→0.0568), `walkToEnd()` raced to `completed:true` with the exact L=16×0.10222=1.6356 + a trip + a logbook dot; audio correctly stays `suspended` under headless autoplay policy (unlocks on a real gesture — right behaviour, not a defect). Node twin `core.test.mjs` **13/13**. **CAUGHT + FIXED a real mobile layout collision:** at ≤460px the fixed topbar wraps to ~145px (title breaks to 3 lines) but the floating tally + "coast walked" bar were pinned at `top:52/120px` assuming a one-row topbar → both **overlapped the wrapped topbar** (0.000 on the subtitle). Root cause: `body{overflow:hidden}` + `#wrap{position:fixed;inset:0}` makes everything overlapping overlays, so a fixed top-offset can't clear a variable-height topbar. Fix: on mobile let the page **flow normally** — `.topbar`/`#wrap` `position:static`, body scrolls, `.brand` stacks title-over-subtitle — so the stage overlays sit BELOW the topbar; verified no-overlap + no-overflow at 320/390/460, desktop (>460) untouched (edit is wholly inside the `@media(max-width:460px)` block). `index.html` is hand-authored (no `.src.html`). Toolchain honest: `core.test.mjs` **13/13** · `forge --check --all` **all 176 current** · manifest OK (**442 pieces**, unclaimed 0). Bed DERIVED **0 sown / 1 bloomed / 0 decayed**. `seal-cycle.sh BUILD garden 447`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #447.)* **A fresh agent: run the gauge.**
