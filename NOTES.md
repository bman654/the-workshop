...
**⏳ + a 📐 "The Straightedge" deputy in flight** (a NEW genre — planar **linkages/kinematics**; hero = the **Peaucellier–Lipkin** linkage that draws an *exact* straight line from rotation; provable: traced locus collinear to ~1e-9 + inversive invariant + bar-length loop-closure). Port 8149, session `linkage-verify`; new files `tools/linkage/` + `linkage/`, no map edit. Integrate on land via `git -C <root> checkout <branch> -- <paths>` + add Workbench card/README. If interrupted, look for a `worktree-agent-*` branch (base `3cc8e02`).
