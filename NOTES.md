...
**▶▶ AT A CLEAN, COMPLETE REST — tree clean, synced with origin, `forge --check --all` green (29 files), no worktrees (latest commit = `git log -1`).** Latest = a **legibility fix to the Survey-of-Heaven constellations on the front-door map** (2026-06-13 `/fun`, Opus 4.8): Brandon read the map's completed asterisms ("The Scribe"/"The Gardener", each a brass line + an engraved name) as **bugs — "a line that points to nowhere"** + unlabeled dots. They aren't bugs (it's the metagame), but they were *too subtle* to read as intentional. Fix (rendering only, no metagame-logic change): completed-asterism member stars now draw **brighter/larger in brass** (`.sky-star.charted`, tagged in `tools/sky/sky.js`), and each engraved name is tied to its nearest charted star by a **faint dashed leader** (`.an-leader`, built in `buildSkyNames`/`applySkyName` in `index.src.html`, reusing `LabelPlacer.geom`) — the same leader idiom every ROOM label already uses; the stub is suppressed (<14px) when the name abuts its star. Verified in-browser at 2/6 and 6/6 charted states; **0 label collisions** (6 names vs 40 POI labels); sky self-test **22/22**. Field-star dots left as-is by design (they're the metagame's quiet breadcrumbs; Brandon's happy to "play to discover"). Detail → `worklog/2026-06.md` (newest-first).

**The estate is large and well-finished.** The 2026-06-13 `/fun` marathon (Opus 4.8, ultracode) shipped **13 new pieces + the estate-map front-door refresh** across ~16 media/genres (map redesign + the Letterer label engine the map uses on itself; the Adversary solved-games engine; the Survey-of-Heaven metagame; spectral physics / sundials / a side-scroller / solitaire / dihedral symmetry / probability / kinematics / a Turing machine / cryptanalysis / a new Lantern tale). Every piece carries a self-test proving its claim. Full per-piece detail → the **Project-status table** below + `worklog/2026-06.md`. **A FRESH AGENT (or a heartbeat tick):** rest is fine; to keep making, pick from the **"Next-steps menu"** below (genuinely fresh ideas — the Workbench is dense; weigh a substantial *new front-door room* over another small bench). No heartbeat cron running — re-create one with `CronCreate` for a long unattended run.
