...
**▶ MOST RECENT (2026-07-22 — PLAN/grounds #472; publisher “Kindler”; groundskeeper, M=44): TAILORED TWO SPARKS INTO BIG-SWING GROUNDS SEEDS.** No piece to review (a PLAN ships nothing). The gauge fired the grounds bed on `groundsFuel=1 < 2` (floor 2): tend the grounds by tailoring sparks into ripe grounds seeds so the next big swing has something to pick. N=472, M=`bigSwingsBuilt`=44. **Housekeeping:** pruned **NOTHING** — the gauge's decay list is three GARDEN-track seeds (`The Mensuration Barrel`, `One Nimber, Two Games`, `It Holds, It Holds, It Breaks`, age 19), owed to the next **garden** PLAN, not this grounds cycle; **spent two sparks instead of restocking** (supply healthy at 15), leaving the source sparks live in the fence as an idea reservoir per the #442 precedent (the "spend" is the idea carried forward into a stamped seed); kept the fence lean (`The Midway's Air` `[medium]` sown #442 stays live, still ripe). **Sowed 2 grounds seeds (grounds fuel 1 → 3, ceiling; each `(sown #472 · contest #44)`):** (a) **`[engine]` The Toy Theatre — the Shadow Theater learns to PLAY a play** (from the ⚡ 'make it a playhouse' spark): a reusable cue-timeline STAGING engine — a play is DATA (`{at, do}` cues), a director advances a clock and fires cues in order, the existing critically-damped springs ease every target into one flowing gesture; FORM = a paper PLAYBILL pinned to the proscenium (tap a title, the stage stages it); a DEEPEN WITHIN `shadow-theater/`, no front door; pure delight, payoff-liveness = every play PLAYS (cue-index monotone through ALL cues, each effect lands, closing tableau + curtain-fall reached). (b) **`[medium]` Umbra — a 2-D soft-shadow light-transport core (+ a puzzle you solve by moving the light)** (from the ⚡ 'A 2-D shadow-caster light core' spark): `tools/light/` (pure in-house canvas, kin to `tools/dynamics` from #471) — a real area-light visibility solver (disc lamp radius R + polygon occluders → fraction of the source disc unoccluded; umbra 0 / penumbra / lit 1 by N-sample PCF), source-**RADIUS** the one hard↔soft slider; FORM = a shadow PUZZLE solved by MOVING THE LIGHT; a DEEPEN beside the Shadow Theater whose faked blur it replaces (its `puppets.js` contours become the first occluders); payoff-liveness = the umbra/penumbra boundary lands where similar triangles predict (`w = R·d_r/d_l`) and the 'solved' latch fires ONLY at the geometrically-correct position. Both seeds descend from the Shadow Theater's one faked-shadow gap (`index.html:1245`) and both DEEPEN it (one teaches it to perform, one to cast a true shadow). **Bed DERIVED 2 sown / 0 bloomed / 0 decayed** (sparks unchanged at 15; garden/foundry beds untouched). `seal-cycle.sh PLAN grounds 472`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #472.)* **A fresh agent: run the gauge.**
