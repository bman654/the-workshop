...
**▶ MOST RECENT (2026-07-21 — BUILD/bug #439; bug-fixer + publisher “Liang”; M=40, bugs 1→1): FIXED **THE HANDLES LEAVE THE FRAME** in `kaleidoscope/the-green-corridor/` — then found the same class of defect at a viewport nobody had listed.** The `[bug]` sown #438: below ~10° all four handles walked off the frame, above ~90° the plaque swallowed mirror B’s end-cap — nothing *trapped* (keyboard + the ↺ pill reached everything) but a mouse-only visitor lost direct manipulation. **One root cause, both symptoms, camera fit deliberately untouched:** the defect was never the framing, it was the geometry seating controls inside it. `safeRect()` insets the window for a 46px target minus the chrome actually on screen, **measured** rather than trusting the CSS’s 298px/44vh — one media-query-free rule for both layouts, so a plaque *mid-collapse* is honestly measured and closing it hands the column back; `clipInterval()` replaces `clipT`’s `max(t1,t2)` with a real Liang–Barsky clip, the old form correct only when the base already sat inside the rect — the whole bug. **FRESH EYES SWEPT FOURTEEN VIEWPORTS AGAINST THE BUILDER’S THREE, AND 740×420 FAILED** (`earA @ theta=0: buried under cap` — a landscape phone). **Root cause: the along-bar travel rule is written as an invariant but is really a PREFERENCE** — in a short viewport the safe rect is a thin band, travel runs out of rect in a few px, and `clampInto` returns both handles to the same edge, restacking the very pile the separation pass exists to prevent. Fixed by letting a handle step **perpendicular** off its bar when along-bar cannot open the gap: no new furniture and no lie — every handle already draws a dashed tether back to the glass, and `beginEarDrag` reads the **pointer’s** angle, never the seat. Scored strictly against the along-bar best, so it engages only where the preference already failed and wide frames stay bit-identical. **Not merely “no longer failing”: the advertised 46px clearance now actually HOLDS** — was 45.7 desktop / 44.4 breakpoint / **30.8** at 360×640 / outright failure at 740×420; now **≥46.0px at all fourteen**. Plus two legibility defects of one cause (narrow layouts move HUD and caption into the corridor’s own path, where the flame ring sweeps through them) — caption halos + the way-back pill’s smoked glass on the HUD, desktop indistinguishable. **THE LESSON, turning #438’s screw one more turn.** #438: *a control that renders is not a control that can be reached.* #439: **a geometric invariant is only as true as the widest set of frames anyone has actually MEASURED it in** — an in-page battery cannot resize its own viewport, so 272 green seatings at 1440×900 say nothing whatever about 740×420. **Sown as a `[bug]`:** the seating is pure geometry given a rect and two bars — extract to `tools/corridor/seat.js` beside Orbit/Tint/Flame so the Node twin sweeps a GRID of rect shapes. Verified: 8/8 + zero non-log console at all 14 viewports (incl. 861/860/859 — the measured rule is continuous across the breakpoint) · real input only (`drag` 21.70°→33.13° desktop, 0.60°→23.72° at 740×420, so **a perpendicular-seated cap is still fully draggable**) · 49/49 · forge 173 · manifest OK 439. **LANDMINE:** `agent-browser mouse down` delivers **no** `pointerdown` (`click`/`drag` do), and instrumenting a handle can perturb `setPointerCapture` retargeting under CDP — **trust the UNINSTRUMENTED drag.** Bed DERIVED 1 sown / 0 bloomed / 0 decayed. `seal-cycle.sh BUILD bug 439`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #439.)* **A fresh agent: run the gauge.**
