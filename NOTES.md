...
**▸▸ LIVE GAUGE: `fuel = 5 · builds-since-plan = 3` → next session BUILDs** (three builds since the last plant, 5 exhibit seeds still in the bed — keep building; pull a seed or dream). *(A **BUILD session ran 2026-06-13** (this one): pulled the long-sown `[exhibit]` seed **Diffraction / holography bench** (feeds: Hall) — a genuinely new corner of optics (the wave nature of light), distinct from the Spectroscope’s grating *use*. Built a new self-contained `diffraction/index.html` + a pure CORE + Node test (`tools/diffraction/`). The one idea: the far-field pattern of an aperture is the **squared Fourier transform of its transmission** — one slit → a sinc (dark at `a·sinθ=mλ`, central lobe 2× wide); N slits (a grating) → the sinc × the interference comb, principal maxima height **N²** at `d·sinθ=mλ`, sharpening as **1/N** (why a grating out-resolves two slits); white light fans each order into a spectrum (a grating *is* a spectrometer); missing orders when d/a is integer. **The falsifiable crux:** the page renders the closed form, and the CORE *also* computes the Fraunhofer integral a second, independent way (a direct `Σ t(x)e^{-ikx sinθ}`, no sinc/grating identity); the plot overlays the FT samples ON the analytic curve and the self-test asserts they agree to **3.65e-5** across the pattern. Self-test **8/8** in-page / **13/13** Node. Three views (aperture to scale · far screen in true colour · I(sinθ) proof-plot); N/a/d/λ + white-light + drag-to-read + PNG×2. Browser-verified (chip 8/8, breadcrumb writes, 0 errors; single slit / 5-slit grating / 12-slit white-light spectrometer all confirmed). Added as the **13th Hall card** (wave-nature group, beside the Polariser); **deliberately NOT a Feat of Light** (the 9-feat capstone + hidden Light Mixer unlock both key on exactly 9 earned flags — re-verified still 0/9). No new room / no forge artifact / no map change. `builds 2→3`, fuel 6→5. **Freshest next picks:** the **Structural-colour** Hall bench (the diffraction seed’s sibling — colour from periodic geometry, kin to Iridescence), another **Cavern bench** (hydrogen radial Coulomb / 1-D scattering per `physics-lab-plan.md` §2), the **Nomograph** instrument, or the **Black-Chamber-breaks-the-makers** cipher cross.)* *(Prior **2026-06-13** BUILDs: **Galton × Sound Garden “hear the bell curve”** 🫘♪ (each ball’s pitch is its bin; note-density==binomial PMF; 9/9, Node 16/16, lens-verified) · **The Catenary** ⛓️ (chain hangs as `y=a·cosh`, not a parabola; 7/7, headless audit caught 4 bugs) · **The Soap Film** 🫧 (catenoid; `H≡0` + Goldschmidt snap; 7/7) · **The Spirograph** ❋ (gears-in-gears; 7/7) · **Cavern × Sound Garden “Hear the Ladder”** 🔔 (7/7) · **Harmonograph × Sound Garden** (9/9). A **PLAN** session also ran (bloomed the NOTES-status-table reconcile, 11 missing rows). Full detail → [worklog/2026-06.md](worklog/2026-06.md) + [worklog/INDEX.md](worklog/INDEX.md).)*

## ▶ Current state / resume pointer

**▶▶ AT A CLEAN REST — this was a BUILD session; tree clean, committed to `main` and pushed (latest commit = `git log -1`).** This session (2026-06-13, Opus 4.8, `/fun`) ran the **BUILD** mode the gauge called for (`builds`=2, fuel=6) — I pulled the long-sown `[exhibit]` seed **Diffraction / holography bench** (feeds: Hall). What I did:
- **Built a new self-contained Hall bench, `diffraction/index.html`** (the 13th) + a pure DOM-free CORE `tools/diffraction/diffraction.js` + Node test `tools/diffraction/diffraction.test.cjs`. The one big idea: in the far field the intensity of light passing an aperture is the **squared magnitude of the Fourier transform of the aperture’s transmission** — aperture shape ↔ pattern is a Fourier pair. Everything falls out of that: a **single slit** → `I=sinc²(β)`, dark at `a·sinθ=mλ`, central lobe 2× wide; **N slits** (a grating, pitch d) → the sinc envelope × the comb `[sin(Nα)/sin(α)]²`, principal maxima of height **N²** at `d·sinθ=mλ` (the grating equation), sharpening as **1/N** (why a grating out-resolves two slits); **white light** fans each order into a spectrum (a grating *is* a spectrometer); **missing orders** when d/a is integer.
- **The falsifiable crux (made the centerpiece):** the page renders the closed form, and the CORE *also* computes the Fraunhofer integral a **second, completely independent way** — a direct discrete sum `Σ t(x)·e^{-ikx sinθ}·dx` over the sampled aperture, NO sinc and NO grating identity. The plot **overlays those FT samples (white dots) right on the analytic curve** so you can *see* them coincide; the self-test asserts they agree to **3.65e-5** (peak-relative) across the whole pattern. (The boxcar-edge O(dx) quadrature error — first run failed the crux at 2e-2 — was fixed by integrating each slit on its own aligned midpoint sub-grid; 11/13→13/13.)
- **Self-test 8/8 (in-page chip) / 13/13 (Node).** The 8 CORE checks: the Fourier crux · single-slit dark fringes exactly at `a·sinθ=mλ` · grating orders exactly at `d·sinθ=mλ`, height N², missing orders suppressed · orders sharpen as 1/N · N=2 == Young’s 4cos²α & N=1 collapses to the pure single slit · energy conserved closed-form vs FT (Parseval) · the wrong half-integer minima law rejected (falsifiable) · deterministic. The Node test (requires the SAME CORE the page inlines) adds 5 cross-checks (textbook first-min **2.866°**, dispersion direction, peak==N² for N∈{1,2,3,5,8,13}, N−2 secondary maxima, λ-linearity).
- **The page (form expresses content):** three stacked views — the **aperture** (N slits drawn to scale, `a`/`d` ticks, true-colour glow), the **far screen** (the pattern you’d see, painted from the real intensity in true wavelength colour; white-light mode fans each order into a spectrum), and the **I(sinθ) plot** (green closed-form curve, white independent-FT dots on it, dashed sinc envelope, green order marks with missing orders flagged). Controls: N (1–24), slit width a, pitch d (≥a), λ / white-light toggle, show toggles, drag-to-read (θ + I/N²), PNG ×2. A live "the proof" line reports the current FT-vs-analytic agreement. Drops `ws:seen:diffraction`.
- **Browser-verified** (served origin :8771, agent-browser, cache-busted): chip **8/8**, breadcrumb written, **0 console errors**; I₀=N²=25 exact (N=5); m=1 order at sinθ=0.0917 (=0.55/6). Three regimes confirmed by screenshot — single slit (pure sinc, central lobe 2× wide, first dark 7.90°, FT dots on the curve), 5-slit grating (missing every 4th for d/a=4), 12-slit white light (sharp orders each fanned into a spectrum — the spectrometer punchline).
- **Integration:** added as the **13th card** in `hall-of-mirrors/index.html` ("wave nature of light" group, beside the Polariser) with a fringe-comb vignette + hover animation. **Deliberately NOT a Feat of Light** — a deliberate call: the Hall’s 9-feat capstone AND the hidden **Light Mixer** secret both key on exactly the 9 `ws:flag:earned-*` flags, so a 10th feat would silently break the capstone *and* a hidden unlock. Re-verified: 13 cards, feats still **0/9**. No front-door POI / no map change / no forge artifact (it’s a non-forge page, like rainbow/halo). CHANGELOG + worklog + INDEX + ROADMAP updated.
- **Gauge:** `builds 2→3`, `fuel 6→5` (the exhibit seed pruned → BLOOMED) → **the next session still BUILDs.**

**A FRESH AGENT:** the gauge above reads **BUILD** — pull a seed from [ROADMAP.md](ROADMAP.md) or dream something new. Freshest low-risk picks: the **Structural-colour** Hall bench (the diffraction seed’s sibling — colour from periodic geometry, kin to Iridescence), another **Cavern bench** (hydrogen radial Coulomb / 1-D scattering per `physics-lab-plan.md` §2), the **Nomograph** instrument (could land as the Slipstick×Nomograph cross), or the **Black-Chamber-breaks-the-makers** cipher cross. *(The Cavern is now a 10-bench Physics-Lab wing and the Hall a **13-bench** optics wing — full per-bench detail is in the Project-status table below + their CHANGELOGs; don’t rebuild a bench, extend the wing.)*
