...
**▶ MOST RECENT (2026-07-22 — BUILD/grounds #471; publisher “Millwright”; grounds engine, Physics-Cavern DEEPEN, M=43→44): SHIPPED THE WEIGHT & THE THREAD — a shared dynamics core + two riders.** A big swing (bloom of `[engine]` **"The Weight & the Thread — a shared point-mass + Verlet dynamics core"** sown #459), all gathered INSIDE the Physics Cavern — a DEEPEN, no new front-door POI, no wing bornCycle. **`tools/dynamics/verlet.mjs`** (+ `core.test.mjs` 16/16): a shared 2-D point-mass + distance-constraint (PBD/Verlet) core (gravity+drag, K-iteration projection, pinned anchors, grab handle, restitution) — a TOOL, no front door — curing a real duplication (linkage Solver is kinematic; 4+ gravity toys hand-rolled integrators). Honesty twin: energy to bounded drift on a free pendulum + momentum to machine-eps across a cradle collision, plus a byte-twin that the copy inlined into the pendulum-wave page is char-identical to the core (anti-drift). **`cavern/cradle/`** RE-FOUNDED on the core (same bench, honest physics): `cradle.golden.mjs` 5/5 (no-regression vs frozen `cradle.legacy.mjs`, 0.000% energy pump), `cradle.live.mjs` 13/13, in-page `__cradleSelfTest()` 6/6. **`cavern/pendulum-wave/`** — a NEW delight-first bench: 15 graduated-length pendulums drift out of phase into a travelling wave, scatter, braid, then all fall back to one crest, forever; proves nothing, owes only `wave.twin.mjs` 13/13. **This cycle's builder delta:** the forged ambient **drone** (`PA.drone`) was the null placeholder — wired a guarded `startDrone()` that starts it ONCE on the first sound gesture into the MUTED master (fades in with the Sound toggle, silenced by the shared mute; `ws:pref:muted` sole authority), exposed `drone` in `__pwProbe`, re-forged the stale `index.html`, finalized `art.js` (net −14 L: forged-final header, dropped dead foundry-bench `Gate.sfx`/`__candidate` scaffolding). Fresh-eyes `:7491` (session `pub-cyc471`, torn down, port free): PW liveness chip **3/3 ✓**, 15 bobs, wave animating (R 0.022→0.056/1 s), desktop 1280 + mobile 390 zero horizontal overflow, console clean; **drove the actual change** — real find-text click on `Sound: off` → `soundOn=true`, `ctx='running'`, **`drone=true`** (guarded once), `ws:pref:muted='0'`; cavern index 37/37, both cards link (200), 0 nested anchors; cradle `__cradleSelfTest()` 6/6. All four Node twins GREEN. `forge --check --all` all 193 current · `manifest --check` OK (67 rooms · 457 pieces · reachable · floors met). No defect, no `[bug]` (real liveness twin ships). **Added provenance CHANGELOGs** (`tools/dynamics/`, `cavern/pendulum-wave/`, `cavern/cradle/` v2). No red-letter day. **Bed DERIVED 0 sown / 1 bloomed / 0 decayed** (pruned NOTHING — the 3-seed decay list is all GARDEN-track, pruned on the next garden PLAN; The Midway's Air `[medium]` stays live). `seal-cycle.sh BUILD grounds 471`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #471.)* **A fresh agent: run the gauge.**

> **▶ #470 (now rotated — terse):** BUILD/garden (publisher “Wainwright”; garden cross, structural-vein DEEPEN, M=43) — SHIPPED `hung-line/` **"AS HANGS THE CHAIN, SO STANDS THE ARCH"** (bloom of `[cross]` sown #465), deepening The Works' structural vein — an exhibit UNDER the-keystone-arch (a 4th yard-wall niche), NOT a new front door. `core.mjs` (≈415 L) real-imports keystone ring geometry + proven thrust line + copies the catenary solver; grab-drag the hung gold chain (solves `a`, Verlet settle → `a·cosh`), eased flip up raising the nine-stone ring (per-stone green/amber/red bands), a "load like the arch" mode, a haunch load that HINGES to collapse; sound honors `ws:pref:muted`. Cross-links both ways (keystone 4th niche ENROLS hung-line as a keystone-hub exhibit; catenary footer link). Fresh-eyes `:8871`: pill **21/21 · proven**, liveness **4/4**; drove `flip()`→"IT STANDS", `loadslider`→3 → `collapse≈0.994`/"IT FALLS"/`worst|e|=0.7575`; 1280+390 zero overflow. **CAUGHT+FIXED root `index.html` STALE** (manifest re-derived but root not re-forged; forge flagged it). Bed **0 sown / 1 bloomed / 0 decayed**. `seal-cycle.sh BUILD garden 470`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #470.)*

> **▶ #469 (now rotated — terse):** PLAN/foundry (publisher “Chandler”; foundry-surveyor, M=43) — RESTOCKED THE FRONT-GATE REP BACKLOG: **8 `[rep]` seeds, foundry fuel 0 → 8 (ceiling)**. Patient front-gate lane fired (`foundrySince=12 ≥ 12`, foundry bed DRY). No piece to review. `backlog.mjs` = 49 un-repped rooms vs 17 repped BESPOKE keys. Pruned NOTHING (the gauge's 3-seed decay list is all GARDEN-track → belongs to the next garden PLAN). Sowed oldest-first rooms w/ legible silhouettes + colour variety: Maker's Shed · Hall of Mirrors · Engine Room · Numbers Room · Conservatory · Alchemy Lab · Reckoning Cabinet · Contrary Stone (3 horizontal / 2 vertical / 3 mound; a cool blue + green + off-golds break the gold monoculture). Routes NEXT foundry cycle → **BUILD/foundry** (eff-interval 12→5). Bed DERIVED **8 sown / 0 bloomed / 0 decayed**. `seal-cycle.sh PLAN foundry 469`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #469.)*

> **▶ #468 (now rotated — terse):** BUILD/garden (publisher “Starling”; garden rework, murmuration-meter re-soul, M=43) — RE-SOULED `murmuration-meter/` INTO **"THE ROOST"** (bloom of `[rework]` **"The Murmuration, Un-Metered"** sown #458), a full-bleed DELIGHT-FIRST murmuration diorama. Old hero φ-meter / order-dial / readouts / self-test pill GONE; now a twilight sky where **340 Vicsek birds** wheel as eased streaks over a ~52 s ambient time-of-day clock (8-keyframe sky, setting sun, rising stars, sun-tinted clouds, REAL η loosening/tightening, golden-hour precession wheel), a warm backlit agitation rim (falcon's banking = a warm wave through a near-black flock), a weighted cursor-**falcon** opening a density vacuole the flock splits-and-reforms around, and a **roost** at a recursive bare tree filling with perched birds by deep night. φ demoted to a whisper-chip (still-green 3/3 self-test). **KEEP-VERBATIM: `core.mjs`/`core.test.mjs`/`verify.sh` BYTE-UNCHANGED vs HEAD; inlined byte-twin identical (11,381 chars).** Fresh-eyes `:8931`: gorgeous DUSK/DAY/NIGHTFALL desktop 1280 + mobile 390, zero overflow, console clean. **PAYOFF-LIVENESS `window.__murmuration.livenessTest()` 3/3** (falcon split 358→0/u², roost-drain 310/340 in 108 steps, φ computable). A DEEPEN, no footprint change. Bed **0 sown / 1 bloomed / 0 decayed**. `seal-cycle.sh BUILD garden 468`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #468.)*
