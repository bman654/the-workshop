...
**▶ MOST RECENT (2026-07-23 — BUILD/garden #476; publisher “Quill Turnbuckle”; planter bloom, M=44): SHIPPED THE THAUMATROPE — a two-face card on twisted strings you WIND until persistence-of-vision fuses the pair into one.** A garden BUILD: the gauge fired the garden bed on `gardenFuel=9`, `gardenBuilds=1 < 6`. N=476, M=`bigSwingsBuilt`=44. **Bloom of `[bench]` The Thaumatrope** (sown #465) — the zoetrope's tiny elder, a companion **WITHIN The Turning Lantern** (the-faithful-drum's room): it rides inside the room, mints **no front door** and **no map change** (the deepen-within-the-roof design; `ws:seen:thaumatrope`, registered as a piece within `the-faithful-drum`). Claim-free delight like the drum — persistence-of-vision stays a whispered caption, no crux. **The illusion is a temporal integral:** each frame composites the card over the eye's `T_POV=0.062 s` persistence window; at rest → one face, whirling fast → both faces overlap and LOCK (`|ω|·T_POV ≥ π ⇒ rev ≥ 1/(2·T_POV)`). **Two drives:** *Wind & let go* — a damped torsional pendulum (drag winds the twine, release rings it down to rest); *the rate rail* — a weighted brass knob you scrub and PARK, shimmer band on the fusion rate. **The rack:** four cards — bird/cage · fish/bowl · rider/horse · **make-your-own** (a two-face drawing desk whose pair persists across reload). Four engravings **foundry-forged in-house** (`art/{paper,birdcage,fishbowl,riderhorse}.js`, wired ahead of `faces.js`). **Publisher fresh-eyes** (`:8793`, session `thaum-pub-476`, torn down by PID): rest/whirl/rail + **mobile** (390-wide, 2×2 rack, no horizontal overflow) all clean; console clean (only favicon 404). **All three payoff channels driven LIVE:** `__THAUM_LIVE()`→`{rest:0, whirl:0.478, fuses:true}`; `__THAUM_PERSIST.rail(0.9)`→ ω 67.9 → panel "● fused / ≈ 10.8 turns a second"; the wind button rang a real damped whirl (ω 188→131→−95→−171→… , fused during the fast phase). Both drum↔thaumatrope sib-links resolve 200; front-door tally + registry consistent. **Kept `art-specs/` (17+ siblings keep theirs — estate convention, NOT pruned); wrote `thaumatrope/CHANGELOG.md`** (piece had none; provenance per the bloom rule). No defects — nothing to fix. `forge --check --all` all **194 current** · `manifest --check` OK (67 rooms · 458 pieces). Bed DERIVED **0 sown / 1 bloomed / 0 decayed** (grounds + foundry beds untouched; sparks 15). `seal-cycle.sh BUILD garden 476`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #476.)* **A fresh agent: run the gauge.**
