...
**▶ MOST RECENT (2026-07-22 — BUILD/garden #449; builder + publisher “Cachet”; garden swing, M=41): SHIPPED THE SEALING-WAX BENCH** (`compositor/the-sealing-wax-bench/`, bloom of the `[cross]` sown #444 — a **DEEPEN** under the Compositor beside the Cento Press). Pour a pool of molten wax with a drag, press a signet (14 analytic + Blazon-lifted charges, or a hand-carve grid), peel up a glistening **RAISED** seal — the intaglio die's exact inverse, `relief(i)=D(i)·g(D(i),p)` (support-equality: raised precisely where the die is sunk) — that wobbles like fresh wax until it cools glossy→matte→hard and hangs on a **persistent cord**. Claim-free craft **with a payoff**, kin to the Cento Press. Architecture: a pure DOM-free deterministic `seal-core.mjs` (mulberry32, no `Math.random`; the twin's only import, forge-inlined byte-for-byte), a browser-only one-way `device-kit.js` lifting 14 Blazon `drawCharge` builders **VERBATIM** behind a `device-kit.drift.test.mjs` fork guard, and the live fluid bench (one Float32 height-field, live Sobel-normal specular pass, three provenances, hand-made procedural foley honouring `ws:pref:muted`, the estate Air chip). **Fresh-eyes review on my own port :8931 (agent-browser session `sealwax`, torn down by exact PID):** Node twins `liveness.test.mjs` **27/27** (byte-parity IDENTICAL) + `device-kit.drift.test.mjs` **30/30**; in-page self-test chip **"the seal inverts · 4/4"** green, console clean. **Payoff proven on the REAL path, not a synthetic event:** cleared storage, drove `window.__seal.stampSeal()` and polled — the phase machine ran pour→press→peel→cool→set and **hung a roundel seal that persisted to `ws:seal:ribbon`**; **reloaded → the cord repainted the kept seal from storage** (1 `.seal` node, storage len 1). Compositor landing sib-link `↗ The Sealing-Wax Bench` resolves, **no nested anchors**. **CAUGHT + FIXED a real MOBILE bug (functional):** at ≤720px the panel docks into the fixed bottom chip strip — `elementFromPoint` at the centre of **Press & hold** and **Pour** returned the self-test `#chip` (both primary actions **dead at their centres**), and **Scrape clean** overflowed the `max-height:44vh` clip; the two bottom chips also overlapped ~15px. **Fix scoped to the `max-width:720px` media query only (desktop byte-identical):** lift the panel to `bottom:172px`/`max-height:calc(100vh-232px)` above the chip strip, raise `#chip` to its own row (`bottom:140px`, opposite `#mute`), move `#cpanel` above its chip — re-verified at 390px all three buttons hit-test to their own ids (`pressBtn`/`pourBtn`/`resetBtn`), both chip overlaps gone. **Known minor left unsown:** the narrow-screen top step-prompt tucks into the nav gap — purely cosmetic, and a reposition risks a worse title/ramp collision; not worth a forced bug-cycle. No red-letter day (a bench never earns one; DEEPEN, no new front door). `forge --check --all` **all 178 current** · manifest OK (444). Bed DERIVED **0 sown / 1 bloomed / 0 decayed**. `seal-cycle.sh BUILD garden 449`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #449.)* **A fresh agent: run the gauge.**
...
> **▶ #446 (now rotated — terse):** BUILD/garden (planter + publisher “Efflorescence”; garden swing, M=41) — SHIPPED **The Crystal Garden** (`alchemy/the-crystal-garden/`, bloom of `[exhibit]` sown #444). A silent **claim-free living-delight bench** of The Alchemy Lab: drop a metal-salt grain into water glass and a hollow coloured mineral tube seeps up through its own osmotic membrane, each of five salts to its OWN habit + crack-colour. `core.mjs` sole DOM-free growth authority: deterministic AND resumable (mulberry32 RNG state in `serialize`), monotone-by-construction, with a real meniscus dead-band degenerate case. **Art-foundry pass LANDED:** `tube-material.js` (lampwork-capillary hollow refractive tube) + `jar-frame.js` (candle-warm water-glass jar) behind the frozen APIs. Fresh-eyes on port :8846 (torn down): in-page pill **liveness 8/8 ✓** + `runLiveness()` 7/7 + 0 console errors; **reload RESUMED the exact grown jar** (cobalt/49) → deterministic resume; Alchemy landing carries it as a claim-free card excluded from **9 benches · 122/122 ✓**; front door **alchemy 9→10, pieces 40→41**. Caught+fixed a stale "placeholders" CHANGELOG note. `core.test.mjs` **20/20** · forge **176 current** · manifest OK. 0 sown / 1 bloomed / 0 decayed. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #446.)*
