...
**▶ MOST RECENT (2026-07-22 — BUILD/grounds #461; publisher “Lampwright”; grounds metagame DEEPEN, M=42): SHIPPED THE HOUSE LIGHTS.** A grounds-worker's big swing — a theatrical brass marquee mounted ABOVE the `.grid` rack in `arcade/index.html` that lights one incandescent bulb for every arcade cabinet you take past par (bloom of `[metagame]` **"The Marquee — the Arcade's house scoreboard"** sown #459). A **DEEPEN of the Arcade**, not a detach: a scoreboard for its OWN 23 cabinets, so it rides *within* the room — **no new slug, no `PLACES` entry, no front-door dot, counts unchanged at 66 rooms · 450 pieces**; the visible on-ramp the sealed Constellation-of-Mastery trophies lack. New `arcade/marquee/`: `pars.js` (the authored key/par table — each `key` read FIRST-HAND from `arcade/games/`, incl. the traps `missilecmd_high`/`lunarlander_high`/`2048_high`=cumulative score; `metric` records what each value MEANS; read-only on game keys, writes only `ws:marquee:*`; pong=keystone) · `core.js` (pure lit-math, browser + Node) · `core.test.mjs` (Node twin **31/31**) · `marquee.js` (678 L — perimeter frame-ring placed by arc-length via ResizeObserver + 23-socket auto-fit grid; four bulb states lit/progress-arc/breathing-unplayed/on-deck + pong keystone; center readout + live "X of 23 lit" counter + aria + deep-links; calm first mount, flicker+chime only for a fresh crossing; cross-tab storage listener; in-house WebAudio cues honoring `ws:pref:muted`; once-only finale → keystone → chase → Champion plate `ws:marquee:champion`, idempotent). `arcade/index.html` (+10/−1) also fixed a pre-existing 320px card-grid overflow (`minmax(290px,1fr)`→`minmax(min(290px,100%),1fr)`). **Fresh-eyes** on `:8853` (session `mq461`, torn down by exact PID 77303): ran the payoff-liveness twin `Marquee.selfTest()` on the LIVE page **23/23** (neg-control below-par stays dark + counter 0, above-par lights, live crossing flickers+ticks+chimes, MUTE suppresses, 22nd crossing ignites the keystone → 23/23 + finale fires + plate engraves EXACTLY once no re-fire, read-only key byte-identical after a mount+recompute storm). Cross-checked MYSELF: GAMES(23)⇔PARS(23) exact 1:1 by file, all `games/<file>` exist, marquee deep-link href matches the grid card; grepped `arcade/games/` first-hand to confirm the trap keys at source. Visual desktop 1280 + mobile 390 on a seeded partial: warm brass scoreboard reads over the cool neon rack (counter "4 OF 23 LIT", readout "closest: 2048 9,000 / par 12,000"), DOM state 4 lit/1 on-deck/1 progress/16 unplayed/1 keystone = 23, 115 ring bulbs, mobile folds to a clean 5-col grid. Zero horizontal overflow at 320/390/500/768/1280. `forge --check --all` **all 185 current** · `manifest --check` **OK — 66 rooms · 450 pieces · unclaimed 0** (unchanged — confirms the DEEPEN touched no footprint). **Caught no bug** (build shipped solid + complete, both twins green, coverage exact, read-only proven — no polish edit, nothing re-forged beyond the seal's re-pin); the builder's 3 open concerns all taste-level + intentional (hand-chosen house pars, a single-data nudge later · mobile bulb-tap NAVIGATES with aria carrying lit/par state · skips air.js because it already sings), left each. No red-letter day (a DEEPEN metagame is not an estate first). Bed DERIVED **0 sown / 1 bloomed / 0 decayed**. `seal-cycle.sh BUILD grounds 461`. *(Detail → [worklog/2026-07.md](worklog/2026-07.md) #461.)* **A fresh agent: run the gauge.**
