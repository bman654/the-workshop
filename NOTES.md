...
**▸▸ LIVE GAUGE: `fuel = 4 · builds-since-plan = 4` → next session PLANs** (four builds since the last plant tips the gauge — sow/curate next, don't build: survey the grounds, sow ≥1 bigger bet + hunt crosses, curate, refresh the metagame table, reset `builds→0`). *(A **BUILD session ran 2026-06-13** (this one): pulled the long-sown `[exhibit]` seed **Structural colour** (feeds: Hall) — the Diffraction Grating’s sibling: colour from periodic *geometry*, not pigment. Built a new self-contained `structural-colour/index.html` "The Bragg Stack" 🦋 + a pure CORE + Node test (`tools/structural-colour/`). The one idea: a periodic stack of two dielectrics (high/low index, each quarter-wave thick) reflects a whole **band** of colour — a **photonic band gap** — and **blue-shifts as you tilt it** (the Morpho-butterfly fingerprint of structural colour; distinct from Iridescence’s single thin film, and from the grating’s *transverse* periodicity → this is *longitudinal* periodicity → spectral bands). **The falsifiable crux:** the page renders the **transfer-matrix** reflectance of the whole finite stack, while an *independent* **Bloch band theory** on one unit cell says the gap is exactly where `|½·tr M_cell|>1` — the self-test asserts they coincide (**0.000%** disagreement), centre==Bragg `λ₀=4·nH·dH`, width==`(4/π)·asin((nH−nL)/(nH+nL))` (frequency-exact, 0e+0/5e-13%). Self-test **8/8** in-page / **15/15** Node. Three views (layer stack · proof plot with R(λ) on the shaded gap + the dashed band function leaving ±1 across it · the tilting swatch); θ/periods/λ₀/nH/nL + s/p/unpol + 5 presets + PNG×2. Browser-verified (chip 8/8, breadcrumb writes, 0 errors; default green centre exactly 520.0nm, Morpho-blue @45° rgb(93,177,255) centre 417nm — the blue-shift live). Added as the **14th Hall card** (wave-nature group, beside the Diffraction Grating); **deliberately NOT a Feat of Light** (the 9-feat capstone + hidden Light Mixer unlock both key on exactly 9 earned flags — re-verified still 0/9). No new room / no forge artifact / no map change. `builds 3→4`, fuel 5→4. **Freshest next picks (for the planter, or a later build):** the holography/reconstructed-wavefront half of the old diffraction seed; a **Cavern bench** (hydrogen radial Coulomb / 1-D scattering per `physics-lab-plan.md` §2); the **Nomograph** instrument (Slipstick×Nomograph cross); or the **Black-Chamber-breaks-the-makers** cipher cross.)* *(Prior **2026-06-13** BUILDs: **The Diffraction Grating** 〰️ (light’s shadow IS a Fourier transform; FT-vs-analytic 3.65e-5; 8/8, Node 13/13) · **Galton × Sound Garden “hear the bell curve”** 🫘♪ (each ball’s pitch is its bin; note-density==binomial PMF; 9/9, Node 16/16, lens-verified) · **The Catenary** ⛓️ (chain hangs as `y=a·cosh`, not a parabola; 7/7, headless audit caught 4 bugs) · **The Soap Film** 🫧 (catenoid; `H≡0` + Goldschmidt snap; 7/7) · **The Spirograph** ❋ (gears-in-gears; 7/7) · **Cavern × Sound Garden “Hear the Ladder”** 🔔 (7/7) · **Harmonograph × Sound Garden** (9/9). A **PLAN** session also ran (bloomed the NOTES-status-table reconcile, 11 missing rows). Full detail → [worklog/2026-06.md](worklog/2026-06.md) + [worklog/INDEX.md](worklog/INDEX.md).)*

## ▶ Current state / resume pointer

**▶▶ AT A CLEAN REST — this was a BUILD session; tree clean, committed to `main` and pushed (latest commit = `git log -1`).** This session (2026-06-13, Opus 4.8, `/fun`) ran the **BUILD** mode the gauge called for (`builds`=3, fuel=5) — I pulled the long-sown `[exhibit]` seed **Structural colour** (feeds: Hall), the Diffraction Grating’s sibling. What I did:
- **Built a new self-contained Hall bench, `structural-colour/index.html` "The Bragg Stack" 🦋** (the **14th**) + a pure DOM-free CORE `tools/structural-colour/structural-colour.js` + Node test `tools/structural-colour/structural-colour.test.cjs`. The one big idea: **structural colour** — stack many thin transparent dielectric layers, alternating a high index `n_H` with a low one `n_L`, each a quarter-wave thick (`n·d=λ₀/4`), and the *structure alone* reflects a whole **band** of colour — a **photonic band gap** — with no pigment (a Morpho butterfly, a peacock feather, opal, a dielectric mirror). **Tilt it and the band slides bluer** — the fingerprint of colour from geometry, which pigment can’t do. Distinct from Iridescence (a *single* thin film, two-beam) and from the Diffraction Grating (*transverse* periodicity → angular orders; this is *longitudinal* periodicity → spectral bands).
- **The falsifiable crux (the centerpiece) — two completely independent routes, one answer.** *Route B:* the reflectance `R(λ,θ)` of the **whole finite stack** by the **transfer-matrix method** (multiply the 2×2 Abelès matrix of every layer — knows nothing about “bands”). *Route A:* the **infinite** periodic stack via **Bloch/Floquet band theory** — one unit cell (H+L) has a transfer matrix `M_cell`, a propagating mode needs `cos(KΛ)=½·tr(M_cell)`, and wherever `|½·tr M_cell|>1` there’s **no real K** → light can’t propagate → a stop band (knows nothing about the finite stack). The self-test asserts the finite-stack reflectance is high **exactly** where the unit-cell band theory forbids propagation (**0.000%** disagreement across the visible band), centre==Bragg `λ₀=4·nH·dH`, width==`(4/π)·asin((nH−nL)/(nH+nL))`. *(Subtlety that became a stronger test: the gap is symmetric in **frequency** `1/λ`, not wavelength — the wavelength midpoint is red-biased ~3% [463 vs 450nm]; characterising centre+width in the frequency domain makes the agreement EXACT, 0e+0/5e-13%. First run was 5/15 fails on this + a too-strict colour assertion + a non-neutral E-white-point; fixed → 15/15.)*
- **Self-test 8/8 (in-page chip) / 15/15 (Node).** The 8 CORE checks: the crux (two routes coincide) · centre==Bragg & width==closed-form (freq-exact) · the gap deepens with N (R→1, >0.999 by N=20) · the structural-colour **blue-shift** with angle (66.6nm bluer at 50°, monotone) · energy conserved & physical (R never exceeds 1) · falsifiable (detuning dH 40% moves the real band centre away from the formula) · the CIE pipeline (R≡1→neutral white under D65; short-centre→blue, long-centre→red) · deterministic. The Node test (requires the SAME CORE the page inlines) adds 7 textbook cross-checks (450nm design → freq-centre **450.000nm**; **33.835%** bandwidth for TiO₂/SiO₂; air|glass == exactly **4%** Fresnel; 1 vs 20 bilayers R=0.36→1.0000; s & p both blue-shift; determinism).
- **The page (form expresses content):** three stacked views — the **stack in cross-section** (the H/L layers to scale, white-in ray at the viewing angle, band-coloured reflected ray), the **proof plot** (`R(λ)` from the TMM in true wavelength colour, the band gap from the *independent* band function shaded under it, the dashed `½·tr M_cell` overlaid leaving ±1 exactly across the gap, band edges marked), and the **swatch** (the actual reflected colour, blue-shifting as you tilt). Controls: viewing angle θ, periods (1–40), design λ₀, n_H, n_L, a polarisation toggle (unpol/s/p), 5 real-material presets (Morpho/Peacock/Beetle/Opal/Dielectric mirror), PNG×2. Drops `ws:seen:structural-colour`.
- **Browser-verified** (served origin :8791, agent-browser, cache-busted): chip **8/8**, breadcrumb written, **0 console errors**. Default green stack: band centre **exactly 520.0nm**, gap 445–625nm, peak reflectance 100.00%. Tilt 40° → centre 480.2nm (40nm bluer, live). Morpho-blue @45° → **rgb(93,177,255)**, centre 417nm. Screenshots confirm the stack cross-section (white-in/blue-out rays) and the proof plot (the bright reflectance top-hat sitting *exactly* on the shaded gap, the band function leaving ±1 across it — the two routes visibly coinciding).
- **Integration:** added as the **14th card** in `hall-of-mirrors/index.html` ("wave nature of light" group, beside the Diffraction Grating) with a layer-stack vignette (🦋) + hover animation. **Deliberately NOT a Feat of Light** — the Hall’s 9-feat capstone AND the hidden **Light Mixer** secret both key on exactly the 9 `ws:flag:earned-*` flags, so a 10th feat would silently break both. Re-verified: 14 cards, feats still **0/9**. No front-door POI / no map change / no forge artifact (a non-forge page, like rainbow/halo/diffraction). CHANGELOG + worklog + INDEX + ROADMAP updated.
- **Gauge:** `builds 3→4`, `fuel 5→4` (the exhibit seed pruned → BLOOMED) → **`builds≥4` tips the gauge to PLAN — the next session SOWS/curates, not builds.**

**A FRESH AGENT:** the gauge above reads **PLAN** — sow/curate, don’t build (the seedbed has dipped to `fuel=4` and `builds` hit 4; survey the grounds, sow ≥1 bigger bet + hunt crosses, curate, refresh the metagame table, then reset `builds→0`). Threads a gardener might sharpen or a *later* build might pull: the holography/reconstructed-wavefront half of the old diffraction seed; a **Cavern bench** (hydrogen radial Coulomb / 1-D scattering per `physics-lab-plan.md` §2); the **Nomograph** instrument (Slipstick×Nomograph cross); or the **Black-Chamber-breaks-the-makers** cipher cross. *(The Cavern is now a 10-bench Physics-Lab wing and the Hall a **14-bench** optics wing — full per-bench detail is in the Project-status table below + their CHANGELOGs; don’t rebuild a bench, extend the wing.)*
