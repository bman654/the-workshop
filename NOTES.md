...
**▸▸ LIVE GAUGE: `fuel ≈ 9 · builds-since-plan = 4` → next session PLANs** (PLAN triggers at `builds ≥ 4`). *(A **BUILD session ran 2026-06-13** (this one): took the Quantum Drift's next step **up in scale** — from one trapped atom (the just-finished bound-state trilogy) to a **crystal** — and shipped **`cavern/lattice/index.html`**, the **Kronig–Penney lattice** (the 6th Quantum-drift bench, 9th in the wing). Line the finite wells up periodically and the atom's sharp levels **smear into bands** of allowed energy separated by **forbidden gaps**: the dispersion `cos(qa) = cos(ka) + P·sin(ka)/(ka) ≡ f(E)` makes an energy allowed only where `|f| ≤ 1`. Two views (the `f(E)` dispersion graph + the textbook `E(q)` band diagram). `P → ∞` narrows each band onto the isolated atomic level `Eₙ=n²π²/(2a²)` (the bridge back to the Finite Well); `P → 0` slams the gaps shut to the free electron — and the prose lands the punchline (half-filled band → metal, filled-to-a-gap → insulator, thin gap → semiconductor). Self-test **8/8** (headless-Node against the embedded code + live browser; the band edges are cross-checked TWO independent ways — `f(E) == ½·tr M(E)` from a from-scratch **transfer matrix** to 4.4e-16 with `det M = 1`, and a **from-scratch cyclic-tridiagonal ring eigensolve** confirming exactly **N states per band** for N atoms). Integrated → Cavern landing self-test **23/23 → 25/25** (verified locked + unlocked). No seed consumed beyond the already-live GRAND Cavern wing seed (which stays live — more benches remain). `builds 3→4`; fuel unchanged. **Next session PLANs** — survey the grounds, sow/curate seeds in [ROADMAP.md](ROADMAP.md), check metagame health (per [DESIGNING.md](DESIGNING.md)); don't reflexively build. If BUILD returns, freshest threads: **more Cavern benches** (the bound-state trilogy is done and the lattice is built — natural nexts are a **hydrogen radial Coulomb** ladder, a **1-D scattering / wave-packet** bench, or a 3rd Newtonian/Einsteinian per `physics-lab-plan.md` §2), or any clean exhibit/cross in the bed (Harmonograph, Catenary, structural colour, diffraction, the cipher-loop cross…). Reset `builds`→0 when the PLAN session runs; update this line every session.)*

## ▶ Current state / resume pointer

**▶▶ AT A CLEAN REST — this was a BUILD session (The Lattice); tree clean, committed to `main` and pushed (latest commit = `git log -1`).** This session (2026-06-13, Opus 4.8, `/fun`) ran the BUILD mode the gauge called for and took the Quantum Drift's next step **up in scale** — from one trapped particle (the just-completed bound-state trilogy) to a **crystal of them**. Shipped **`cavern/lattice/index.html`** entirely myself in-turn (no deputy/worktree), ~640 lines: the **Kronig–Penney lattice**, the 6th Quantum-drift bench and 9th in the wing. Line the finite wells up in a row **repeated forever** (`V(x)=U·Σδ(x−n·a)`, period `a`, strength `P=U·a`) and the atom's sharp levels can no longer stay sharp — each **smears into a BAND** of allowed energies with **forbidden GAPS** between them. Bloch's theorem + δ-matching give the **dispersion** `cos(qa) = cos(ka) + P·sin(ka)/(ka) ≡ f(E)`, `k=√(2E)`; since `cos(qa)∈[−1,1]` an energy is **allowed only where `|f(E)| ≤ 1`** (a band), the rest forbidden (a gap), with edges exactly at `f=±1` ⇔ `qa=nπ`. Two views: the **dispersion `f(E)` graph** (violet curve threading the green `|f|≤1` corridor, amber gap strips) and the textbook **band diagram `E(q)`** over the reduced zone `q:0→π/a`. The **two limits are the soul:** `P→∞` (atoms far apart) **narrows each band onto the isolated atomic level** `Eₙ=n²π²/(2a²)` — the bridge back to the Finite Well next door (band-1 width 1.74→0.049, top edge → `π²/(2a²)` rel 0.00% as `P:8→400`); `P→0` (free electron) **slams the gaps shut** to `E=q²/2`. The prose lands the punchline — **half-filled band → metal, filled-to-a-gap → insulator, thin gap → semiconductor.** UI: band chips (one per band — the count is the live headline), `P`+`a` sliders, a dispersion/band-diagram toggle, a periodic-potential overlay sketch. **The spine = TWO independent cross-checks** (Cavern discipline: closed form vs a different algebra): (1) a from-scratch **transfer matrix** `M=F·D` (free-drift × δ-kick) gives `½·tr M(E) == f(E)` to **4.4e-16** over 4000 energies with `det M=1` (symplectic) to 8.9e-16; (2) a **from-scratch eigensolve of a finite ring of N cells** (deflated inverse-power on a *cyclic-tridiagonal Sherman–Morrison* solve) puts **exactly N states in each band** — the one-state-per-atom counting law behind metals vs insulators. **Self-test 8/8**, proven two ways: headless-Node against the *actual embedded `runSelfTest()`* (extracted the inline script, stubbed document/localStorage/window via a Proxy) AND live in-browser (agent-browser, served origin :8743, clean console, badge ✓ 8/8, both views + a 390×844 mobile layout reviewed). The 8 checks: dispersion `f(E) == ½·tr M(E)` & `det M=1` (4.4e-16 / 8.9e-16) · band edges land on `|f|=1` (8.5e-14) · gaps truly forbidden `|f|>1` · free-limit gap → 0 (8.28→1.81→0.04) · **atomic-limit band 1 → `π²/(2a²)` rel 0.00%** · **a ring of N=8 atoms holds exactly 8 states per band** (independent eigensolve) · `E(q)` spans the band, `f(E)=cos(qa)` round-trips (1.1e-9) · deterministic. **Integration:** 6th Quantum-drift card in `cavern/index.html` (⛓️) + 2 symmetric self-test checks → Cavern landing **23/23 → 25/25** (verified live in BOTH locked and unlocked states); drops `ws:seen:lattice`; `forge --check --all` clean (29 files), `--audit-seen` still 13/13 (lattice breadcrumb is bench-internal, not a front-door PLACES id). **A FRESH AGENT:** rest is fine; the gauge above now reads **PLAN next** (`builds` hit 4) — survey/sow/curate in [ROADMAP.md](ROADMAP.md), don't reflexively build. ───── **The Cavern** ⚛️ (`cavern/`) — the grand **Physics Lab** WING: a cave on the grounds, **three drifts off a central shaft** — warm Newtonian / cold Einsteinian / and the **Quantum Drift** (cold electric-violet), which *opens spatially in-page* once a visitor has walked both open drifts (`walkedBothDrifts` reads the `ws:seen:<id>` breadcrumbs; this is a PUBLIC in-cave reveal, distinct from the Undercroft's hidden inventory). Index self-test **25/25**, drops `ws:seen:physics-lab`. **The nine benches** (served-origin verified, clean console): **The Light Clock** ⏱️ `cavern/light-clock/` (SR — γ-from-photon-slant == closed form 2.38e-13; interval invariant 3.11e-13; **7/7**) · **Newton's Cradle** ⚙️ `cavern/cradle/` (elastic — conserves p AND KE; lift-k ⇒ k-out all cases exact; honest dissipation; **6/6**) · **Mercury's Precession** ☿ `cavern/precession/` (GR — RK4 Binet orbit → **42.98″/century**, Newton control precesses 0; **6/6**) · **The Double Slit** 🎯 `cavern/double-slit/` (Born-rule fringes; λL/d exact; which-path erases interference; **8/8**) · **Quantum Tunnelling** ⛰️ `cavern/tunnelling/` (closed-form T == transfer-matrix solve 1.9e−14; R+T=1; resonant transparency; **7/7**) · **Particle in a Box** 📦 `cavern/box/` (quantized ladder `n²` == from-scratch eigensolve; hard walls; **8/8**) · **The Harmonic Oscillator** 🌀 `cavern/oscillator/` (the Box's foil — even ladder `ω(n+½)` == from-scratch FD eigensolve, leaking tails, coherent state; **8/8**) · **The Finite Well** 🕳️ `cavern/finite-well/` (the realistic well — finite ladder from a transcendental match == from-scratch FD eigensolve of the stepped V, exponential leak out the walls, box-recovery limit; **8/8**) · **The Lattice** ⛓️ `cavern/lattice/` (this session; one atom → a CRYSTAL — Kronig–Penney bands & gaps; `f(E)==½·tr M` transfer-matrix 4.4e-16 + N-states-per-band from a ring eigensolve; metal/insulator/semiconductor; **8/8**, above). The **Box · Oscillator · Finite Well** are the matched **bound-state trilogy** (one trapped particle, three model shapes); **The Lattice** is the step up to a periodic solid. Per-unit detail in `worklog/2026-06.md` + `cavern/CHANGELOG.md`. ───── *Prior run (2026-06-13, concluded before this session) — the planning system + 5 builds + a PLAN session:* **designed the gardener/builder planning system** with Brandon, then demonstrated it: **5 builds** — the **Hall of Mirrors redesign** ("The Dispersion" — spectral-rail card-spine + live per-physics SVG vignettes; the cautionary-tale fix), **The Lighthouse** (Fresnel-lens optics bench, the Hall's 12th; focus to 9.99e-16 m, 5/5), **The Planimeter** (polar area-measuring instrument; Green's theorem in brass, pole-independence 9e-15, 7/7), **Fourier Epicycles** (draw any curve as rotating circles; from-scratch complex DFT, 6/6 + Parseval 3.25e-15), **The Brachistochrone** (cycloid wins the descent race + tautochrone spread 0.00e+0, 7/7) — plus **1 PLAN session** that fleshed the **Physics Lab** grand bet (→ `worklog/physics-lab-plan.md`), whose first benches *this* session built. (Behind that run: **Feats of Light** — each of the 9 Hall pieces drops a `ws:flag:earned-*` skill-flag feeding the Hall ribbon + the Survey's "The Optician" constellation [sky 41/41] + the hidden "Light Mixer" capstone [12th secret]; and the **planning-system** files — the mode gauge, [ROADMAP.md](ROADMAP.md) seedbed, [DESIGNING.md](DESIGNING.md) pipeline.) Full detail → `worklog/2026-06.md` (newest-first).
