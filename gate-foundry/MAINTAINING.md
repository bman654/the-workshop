...
is the gate's slow-upkeep lane, and it is deliberately **patient**:

- A **foundry TURN** comes due only every `foundryInterval` (12) cycles, and it sits **below garden-plan**
  in the decision ladder — so it can never starve the gardens or jump ahead of a bug or a swing.
...
- Foundry seeds **decay** like grounds seeds, but on their OWN clock (`foundryBuilt` contests lost, never
  mere waiting, never a grounds swing) — so a rep that keeps losing its turn eventually rotates out.

State lives in `seedbed/state.json` (`lastFoundry`, `foundryBuilt`, the `foundry*` tally, `fence.foundry`).
`node seedbed/gauge.mjs --status` shows the FOUNDRY line.
...
- **`[rep]`** — a bespoke front-gate **room-rep**. The gate's grounds show ONE rotating room's
  front-elevation; today only a handful are bespoke (Cairn, Cavern, Ripple Tank, Music Room) and every other
  room falls back to the glyph plinth. A `[rep]` grows that set. It names the room + the drawn object + the
  slot aspect + colors:
  ```
...
- **`[gate]`** — a gate **asset rework / polish**: re-soul or refine an existing gate asset (a building, the
  foliage, the grounds, the mist, a drifted rep) per its `the-gate/SPEC.md §4` row.

Both stamp `(sown #N · contest #M)` where M is `foundryBuilt` (the foundry contest counter, NOT the grounds
one). `sow.mjs` handles this automatically.
