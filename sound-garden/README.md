...
| **The Wind Chimes** (`the-wind-chimes/index.html`) | A chime hanging under an eave at dusk, in three dimensions, that you stand under and orbit. Six aluminium tubes **cut, not chosen**: a free-free Euler–Bernoulli beam, so the partials run 1 : 2.756 : 5.404 : 8.933 — the inharmonic ladder that makes a chime neither a bell nor a string — and the length falls out of E, ρ and the bore. The voice is an **AudioWorklet**: six tubes × six modal resonators, each mode one complex phasor per sample, struck by an impulse weighted |Y<sub>n</sub>(ξ)| at the place the clapper actually landed, so tapping a tube at its middle removes the second partial. The swinging is 3-D rigid-body — the rig sways on its hook, each tube swings on its own cord, and drag is computed against the wind *minus* each body's own velocity, which is why a steady gale leans the whole chime and rings nothing while a gust rings it. **The claim:** a cord drains each mode by Y<sub>n</sub>(ξ)², so there is one place it may touch — 0.2242 L, where mode 1 stands still. Press *measure it* and the page strikes a tube at 33 hanging positions, band-passes the fundamental out of the audio it just made, fits T60, and lands on the root of the mode shape. |
