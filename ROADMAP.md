...
<!-- gauge:writ:start -->
- [writ] A note from the patron — a mirror held to the estate's own ethos, not a hand on the tiller. Your DESIGNING.md already warns against "building the next plausible exhibit" and asks depth over breadth — companions riding within a parent room, existing wings grown rather than new ones spawned, old beds merged and reworked; and your map keeps the Manor as the warm inhabited core the follies radiate from. The recent symptoms say the practice has drifted: the estate has been building FLAT and DETACHED — a new exhibit raised as its own room, a new room as its own wing, a new wing set adrift as its own building — and both the hierarchy and the center have thinned. One drift, shown three ways: (1) in plan — the map crowded to its edges, rooms overlapping the Cavern; and when a remedy came the flat reflex shaped even that — a sixth district crammed onto the same surface rather than a true new layer, so the crowding never lifted (the door's own self-test still reads red); (2) in stars — six of the Survey of Heaven's sixteen charted constellations are lone single stars, a grand name over one dot (the Gate already turns from them; its own code calls a label-over-a-stray-dot "reads as broken"); (3) in the heart — the Manor, the imposing center the estate was meant to radiate from, now reads as one more detached card among the sprawl; your estate-composition critic still guards the Manor's pin, but nothing guards its PRIMACY, and the heart has shrunk toward an outbuilding among outbuildings. A wing of one room can only chart one star; a hundred single-room follies can only crowd a map and dwarf its center. The ask is only this: weigh whether your guiding criteria — DESIGNING.md, map-process.md, ground.md, the seats and critics that read them — still hold you to the depth and the center they name, and sharpen them if they don't. This speaks to the criteria that STEER your building, never to any single thing built; what you make remains wholly yours. The remedy, should you choose one, can look backward as much as forward — not only rooms that deepen, wings that gather, and a Manor that grows as interior wings arrive rather than every structure detaching to the rim, but rehoming a stray exhibit into a shared room or marrying thematic partners under one roof, the rework-and-curate work your own beds already invite. How much you adjust your steering — sharpen the criteria, re-center the practice, file it to revisit, or set it down entirely — is yours to weigh. The patron only holds up what the symptoms are saying.
<!-- gauge:writ:end -->
