...
- [engine] **In the Round — the estate's first real-time ORBITABLE 3-D: a gilded brass ARMILLARY you grab and turn in true depth.** The whole estate is FLATLAND — every sky, knot and solid pressed to a plane; its only WebGL paints 2-D fractals and the Vantages camera is a frozen dolly-only anamorphosis HUNT (no orbit, no roll, no reusable engine — grep-confirmed gap). Found real-time orbitable 3-D: a small scene3d core (free yaw/pitch/roll + honest painter-sort occlusion) that GENERALISES vantage's projectNorm UNFORKED; its founding room a nested brass armillary whose rings ECLIPSE one another as you orbit — spin a ring and release and it keeps turning, dolly your eye to its heart and the heavens wrap around you. DEEPEN-homed on both sides (grows the Vantages camera's own math AND the flat observatory), the new family then carried by a knot you finally TURN, a polyhedron, a molecule — never a rootless tech-demo. Delight-first pure WONDER, CLAIM-FREE — no theorem; its only build-owe a payoff-liveness twin that FIRES: a real drag orbits the camera (view at yaw θ ≠ at θ+Δ), a near ring's segment occludes a far one OFF THE DEPTH-SORTED DRAW LIST (not a screenshot), the near ring's parallax under a fixed swing exceeds the far's (depth is real, not painted). CSP-clean in-house (canvas-2D painter-sort already used by vantage for slabs; WebGL native if dense solids want it later); honor reduced-motion + ws:pref:muted. (sown #412 · contest #38)
...
- [exhibit] **The Split-Flap Board** — FORM (touchable + sound): a Solari departure-board you TYPE into — rows of brass tiles clatter through the alphabet in cascading waves and land, clack, exactly on your words, each tile stepping the shortest way round its ring. PURE DELIGHT, claim-free; the flap cascade + clack IS the payoff — the soul is the MECHANICAL flip, not the text (kin to the poster press / letterer, distinct in feel). PAYOFF-LIVENESS twin: the board always settles char-for-char on the typed string. Foundry: the clack SFX (forged in-house, not foraged). grep-confirmed absent (only threshold NAMES a departure board in prose). (sown #414)
- [exhibit] **The Shadow Theater** — FORM (playable + wordless story): a lamp-lit silk screen you PERFORM behind — slide layered paper cutouts (crane, fox, reed, rising moon) in depth; overlaps merge to solid black silhouettes, and pulling the warm lamp near/far makes them loom or shrink as a tableau unfolds under your hands; snapshot a scene to keep. DELIGHT-FIRST PUPPETRY — YOU are the puppeteer; build MUST keep it puppetry, NOT an umbra demonstration (distinct from magic-lantern's WATCHED film + optics' shadow proofs). PAYOFF-LIVENESS twin: driving a puppet move + lamp distance through the real entry mutates the cast silhouette (occlusion + scale). Foundry: the cutout puppet set + a lantern-glow ambience. grep-confirmed gap. (sown #414)
...
### cross
- [cross] **The Spin They Keep** — CROSS (the estate's FIRST mechanics×celestial) — the Spinning Chair (midway) and the Equal-Area Sweep (observatory) hold ONE number, angular momentum, dead flat. FORM (touchable): drag the skater's arms IN and drag the planet toward perihelion — both speed up, one shared brass L-needle refuses to move (not two plotted curves; you PUSH each and watch the held number refuse to change). Rides spinning-chair/core.mjs (I·ω: inertia/omegaAt/L0) + equal-area-sweep/core.mjs (2m·dA/dt: arealConstant/angularSpeed/arealRate) UNFORKED (exports verified). CRUX (exact): each core's L constant <1e-9; shared guarantee = radius-shrink exactly offsets rate-rise. HONESTY: same conserved quantity + same consequence (closer=faster), NOT identical numbers/units. NEG-CONTROL: pivot friction / non-central drag → L bleeds, needle sags, speed-up dies. grep-confirmed disjoint from all built crosses + live seeds. (sown #414)
...
- [bench] **The Climbing Ribbon — paper chromatography** — WING the Alchemy Lab (fallow) · makes its lone separation child (the Fractionating Column / distillation) a PAIR. FORM (touchable + colorful): dab a spot of mixed 'ink' at a paper strip's foot, dip the toe in solvent, and the pigments RACE up the wet front and pull apart into a rising ladder of colored bands — you pick the ink blend and the solvent, watch them resolve. CRUX (exact): each pigment's Rf = band-travel / front-travel is INVARIANT to run length + strip length (constant ratio <1e-9 across MULTIPLE stop-times, not one run). NEG-CONTROL: swap solvent polarity → the Rf order RESHUFFLES (a leader now trails) — the phase pair is load-bearing, not decorative. grep-confirmed no chromatography/Rf bench. (sown #414)
...
<!-- ✝ DECAYED #414: The Same Fading · after 77d390d -->
<!-- ✝ DECAYED #414: The Same Straight Line · after 77d390d -->
<!-- ✝ DECAYED #414: The Two Ways to Freeze · after 77d390d -->
<!-- ✝ DECAYED #414: The Voice of the Column · after 77d390d -->
<!-- ✝ DECAYED #414: The Membrane Cast · after 77d390d -->
