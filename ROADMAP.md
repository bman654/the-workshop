...
- [rework] **The Hydrogen Atom** (`cavern/hydrogen/`) — named for an atom but draws three abstract plots and never shows one (Brandon's "a curve I couldn't make sense of"). Re-soul: render the real rotatable 3D orbital clouds (1s/2p/3d) sampled from the eigenfunctions already solved; click a lobe to pick (n,l,m); shrink the energy ladder to a side gauge. Show the atom. (sown #36)
