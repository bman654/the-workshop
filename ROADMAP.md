...
<!-- gauge:bug:start -->
- [bug] **The front door's POI hover state machine drops the info card when you move between rooms inside the grace window — a stale hide-timer is never cancelled when the pointer enters a DIFFERENT POI than the one it left.** Moving the pointer in and out of POIs lights them up and shows the info card, but depending on speed/path you can reach two broken states: (1) the POI lights up (its hover styling) but the info card never appears / vanishes immediately; (2) more rarely, neither the light-up nor the card appears. REPRO for (1): hover a room, wait for the card to appear, then quickly move onto a SECOND room before the grace window expires — the card stops showing for the new room. ROOT CAUSE (confirmed in index.src.html, the hover-card grace logic ~line 2557): the card is shown by `openCard(r,a)` (bound to each POI's `mouseenter`/`focus` via `show`), and hidden by `hideCard()` which DEFERS the hide behind a single shared global timer: `_hideTimer = setTimeout(()=>{ if(!_overCard) card.classList.remove("show"); }, 90)`. The grace window exists so the pointer can travel from a POI onto the card itself to scroll a long popup (the card's own `mouseenter` clears `_hideTimer`). BUT `openCard`/`show` NEVER clears `_hideTimer`. So leaving POI A arms the 90 ms hide-timer; entering POI B within 90 ms runs `openCard(B)` (content swaps to B, `.show` re-added) yet the orphaned A-timer is still pending and fires, stripping `.show` from B's card — B is lit (CSS `:hover`) but its card is gone. The grace path only ever expected pointer→card, never pointer→another-POI. The shared `_overCard` flag can likewise be left stale by fast moves, which is the most likely source of state (2). FIX: make entering ANY POI cancel the previous POI's pending hide. In `openCard` (or `show`), at the very top, clear the grace timer (`if(_hideTimer){ clearTimeout(_hideTimer); _hideTimer = null; }`) and reset `_overCard=false` before showing the new card. Harden the machine so a stale timer can never hide the wrong room: track the id/element of the POI that currently OWNS the shown card (e.g. a `_cardOwner`), and have the deferred hide bail out if the owner has changed since the timer was armed (only hide if still the same owner and not `_overCard`). Net intended behaviour: moving directly from one POI to another instantly shows the new room's card (no fl. disappearance); moving from a POI onto the card keeps it (scroll grace preserved); leaving to empty space hides it after the grace. VERIFY WITH A REAL CURSOR (not synthetic dispatch — this is a timing/pointer-transition bug that synthetic events mask): (a) hover A → wait for card → quickly slide onto B before 90 ms → B's card must show; (b) sweep rapidly across a row of POIs → each shows its own card, none gets stuck lit-without-card or dead; (c) hover A → move onto the card → scroll → card stays; (d) hover A → leave to empty space → card hides after the grace.
