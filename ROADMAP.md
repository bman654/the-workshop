...
### exhibit
- [exhibit] **The Teacup Caustic** — the bright heart-shaped cusp of light that floats on coffee when a point light grazes the cup's inner wall (ripens the line-104 spark). FORM you OPERATE, not a plotted curve: a top-down porcelain cup; DRAG the lamp around the rim and the live caustic re-forms ON the dark coffee — lamp on the rim = a one-cusped CARDIOID, lamp pushed to infinity (a sun toggle) = the two-cusped NEPHROID. The curve IS the bright light, drawn by reflection. CRUX a Node twin proves (reusing cardioid/core.test.mjs's proven envelope-residual + cusp-count, NOT a new method): every reflected ray is TANGENT to the catacaustic E(t) (perp distance <1e-9), an independent neighbour-intersection envelope corroborates E to <1e-8, cusp-count = |E'(t)| minima (rim⇒1, infinity⇒2). NEG-CONTROL: a flat wall (curvature→0) ⇒ no cusp, no caustic. Distinct third origin from optics/Caustic + cardioid/ (reflection on a fluid, not a light-bench or string-art); the tying [cross] is separate future work. (sown #310)
...
- [bench] **The Plucked Reed** — the Sound Garden's first PHYSICALLY-MODELLED string (Karplus-Strong): you don't play a tone, you pluck a BURST OF NOISE into a tuned echo-loop and HEAR it become a string. Pluck → white-noise fills a delay line N=round(SR/f); each lap a two-tap averaging lowpass + feedback gain ~0.996 ⇒ the hiss self-organises into a pitched, decaying pluck, the brightness bleeding away as a real string does. PLAYED, not graphed: a horizontal reed you pluck at position p (near the end = bright/twangy, mid = mellow) + a DECAY knob (feedback) from dead thud to endless sustain + a BRIGHTNESS knob (loop-filter blend) + frets to retune N live. NO math claim — the soul is structure crystallising out of noise. GAP confirmed: monochord is additive-Fourier (0 delay-line/feedback grep hits), the-sidebands is FM, endless-staircase is Risset — KS delay-loop modeling is the one absent synthesis METHOD. Reuses pitch-core + mute/offline-render, the-overtone-rack standalone-core mold. (sown #310)
- [bench] **The Toads & Frogs Court** — grow the Numbers Room its FIRST PARTISAN game (every game it holds is impartial today). On a 1-D strip your green Toads step/leap only RIGHTWARD, the AI's red Frogs only LEFTWARD; slide into the empty square or leap one opposing piece, no captures, no-move LOSES (normal play). Because the two sides have different move-sets it's a true Conway-partisan game with surreal values, orthogonal to every impartial game (and to the seeded Sprouts). PLAYED: a brass strip you click piece-then-destination on, a flint minimax AI, a 'show the value' loupe naming the position's game value (0, ±1, ↑, ∗) and so who-wins-to-move. CRUX a twin proves anti-circularly: core's exact value-recursion {Left-opts | Right-opts} with a memoised simplest-number reduction, checked against a SEPARATE brute negamax oracle — value=0 ⇔ the mover LOSES across every board up to length 7, the canonical TTFF/T_FF outcomes match, the AI always lands on a 2nd-player-win child when one exists. Form = a played strip, not a plot. nimber-strip core/twin/forge mold; alt flavour blue-red Hackenbush if Toads reads thin. (sown #310)
