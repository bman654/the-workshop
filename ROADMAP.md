...
### exhibit
- [exhibit] **The Watched Coin — a measured pot never boils (Quantum Zeno)** — DEEPEN the Cavern/Quantum Drift, growing its measurement family (spin/Stern–Gerlach, the interaction-free Bomb, the Bell pair) — built under cavern/, own ws:seen:watched-coin, reached from the Cavern hub, NO new front-door plate, M unchanged. FORM (touchable, past the graph): a qubit Rabi-rotates from |0⟩ toward |1⟩ on a Bloch hemisphere; crank ONE dial N = how many strong 'peeks' you take during a single would-be flip, and watch each peek YANK it back to |0⟩ — the harder you watch, the more it FREEZES. CRUX (planter to pin, two derivations agree): survival after N equal projective peeks over a π rotation = cos^{2N}(π/2N) — N=1→0 (full flip), N=2→¼, N→∞→1, vs an independent Born-rule projector sim (rotate · project · renormalize · repeat); NEG-CONTROL never peek (N=0) ⇒ full flip, P=0 — watching freezes it, not the drive. Label the decoherence-free projective-measurement idealization as the modeled assumption; coordinate the Bloch picture with cavern/spin's core, don't fork its math. grep-confirmed ZERO zeno/Rabi bench (the 'zeno' hits are flavor text elsewhere). (sown #355)
