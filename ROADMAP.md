...
### exhibit
- [exhibit] **The Unstirring — smear a dye blob into a spiral, crank it back, and the blob REASSEMBLES (low-Reynolds reversibility).** A blue dye drop in clear corn-syrup between two glass cylinders: wind the dye into a thin invisible spiral — apparently mixed away — then crank back the SAME turns and it un-winds, gathering into the original blob as if time ran backward. FORM: a touchable annulus you crank both ways + a living dye trace, not a streamline plot. THE ONE KNOB — a Reynolds dial: at Re≈0 reversal restores the blob; slide toward high Re and inertial swirl scrambles it so backward-cranking can NEVER bring it home. CRUX (planter, machine-ε): in the Stokes (Re→0) limit the creeping flow is exactly time-reversible — advect every dye particle forward N turns then backward N and each returns to start within <1e-9; the reassembly error grows monotonically with Re as the neglected inertial term switches on (label honestly: kinematic reversal of a creeping-flow field, error-vs-Re, not a turbulence DNS). NEG-CONTROL: an inertia-ON toggle — forward-then-backward leaves the dye smeared, the blob gone. Deepens the Workbench (restocks the fluid-kinetics vein the decayed Chain Fountain + Leidenfrost vacated); contrast-kin to the Strange Garden's Marbling Bath (that makes swirl beautiful, this takes it back). grep-confirmed gap (no couette/reynolds/stokes-flow piece). (sown #390)
- [exhibit] **The Value of a Cut — Blue-Red Hackenbush, the game whose POSITION IS A NUMBER.** A ground line holds a tangle of coloured edges — bLue are yours, Red the opponent's; CLICK one of your edges and it (plus everything now in the air) falls; last to move wins. FORM: a game you PLAY that teaches a NUMBER — a single blue edge = +1, a blue-on-red stalk = ½, deeper stacks give ¼,⅛… (dyadic surreals, the first rung of Conway's numbers) — and that number ALONE decides the winner. Drag to build a tree/flower/grove, a live VALUE readout condenses beside it as you cut, a who-wins oracle lights before your move so you watch the NUMBER foretell the outcome. CRUX (planter, exact): the recursively-computed game value {L|R} (simplest-number rule) sign ⇔ optimal-play winner over random trees with zero mismatches; the Colon Principle (a blue-red stalk = its binary-fraction value) verified to machine-ε on all stalks ≤ depth 16. NEG-CONTROL: an ALL-GREEN Hackenbush is pure Nim — its value is a *nimber, NOT a number — the oracle deliberately mis-predicts it, showing only partizan blue/red colouring makes a position a NUMBER. Deepens the Numbers Room's Strategist constellation (its missing keystone: nimber games exist, none whose value is a fraction) — kin to Toads&Frogs + the Best Rational. grep-confirmed gap. (sown #390)
...
### curation
- [curate] **The Rewind Shelf — a themed walk of the pieces that FORGET how they began (and the rarer few that run backward clean).** THE TIE (a reveal, no math gate): gather the demos that relax to a state with no memory of their start — the Foundry's pour cooling, the Drift Jar going monochrome, the Sandpile, the Brazil-Nut Box, entropy's Source Dial — against their time-REVERSIBLE cousins (Two Costumes' LC-tank/pendulum, elastic Collisions, the Standing-Stones fold), anchored on the built Reversing-Room. FORM (a touchable walk, minimal re-implementation): a row of live low-fi thumbnails you nudge forward, then drag ONE shared rewind handle across the shelf — the reversible cluster runs cleanly back to its start while the irreversible cluster smears / refuses / snaps to disorder (an honest 'this one can't un-mix' verdict per thumbnail), each linking out to its full exhibit. The reveal: which rooms of the manor have an arrow of time, told as one gesture. Fills the EMPTY curation fence; gathers built pieces under the Reversing-Room's roof (no new footprint) — deepen-before-detach. BUILD NOTE: make the reversible/irreversible CONTRAST legible IN the thumbnail (clean return vs smear) or it slumps to a passive link-list; keep thumbnails low-fi greybox. grep-confirmed no walk/tour exhibit exists. (sown #390)
...
<!-- ✝ BLOOMED #387: The Long Chain — Dots and Boxes you actually play, won not by greed but… → the-long-chain/ · after 88c03f1 -->
<!-- ✝ BLOOMED #388: The Fractionating Column → alchemy/fractionating-column/ · after 2efcd45 -->
<!-- ✝ DECAYED #390: The Chain Fountain · after b823656 -->
<!-- ✝ DECAYED #390: The Leidenfrost Drop · after b823656 -->
