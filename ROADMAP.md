...
<!-- gauge:bug:start -->
- [bug] The front-door map has run out of room for new wings — newly-built rooms are now being placed on top of existing ones. The Infinite Overhand and the Keystone Arch are sitting directly over the Cavern, overlapping it. The estate keeps growing but the map's footprint hasn't: every outer slot is taken, so the placer is stacking new rooms onto occupied ground. Give the map more room to grow: add a new OUTER TIER — a ring beyond the current outermost tier — so fresh wings have their own space. If an outer tier won't fit the page, give the map a SECOND LEVEL instead — an upper floor or an underground level — and let wings spill onto it. Pick whichever scales cleanly; the point is to stop new rooms from landing atop the Cavern (or any other wing).
- [bug] **The Front Door Fails Its Own Legibility Self-Test (✗ 16/17)** — index.html shows a RED "door self-test ✗ 16/17" pill: one LIVE geometric label-placement claim fails (the revealed-loupe / guided-tour declutter on the actual plate), and `node tools/layout/smoke.cjs` exits 1 with 5 FOOT-OVERLAPs — rattleback×strange-garden, rattleback×weather-you-can-make, overhang×the-keystone-arch, overhang×physics-lab, the-keystone-arch×physics-lab. PRE-EXISTING (byte-identical on HEAD #333 — the pill reads the same ✗16/17 · LEGIBLE 0.13 · CROWDED 0.92 with no #334 add involved): the grounds-plate footprints (amusements/works) have grown crowded enough to collide. The CONSCIENCE is sound — legibility.test.cjs 29/29, sky 73/73, forge 106/106 current, partition 80/80 across 5 plates — it is the LIVE drawn geometry that overlaps. FIX = nudge the colliding footprints (rattleback/overhang/the-keystone-arch/physics-lab/strange-garden/weather-you-can-make) off one another in tools/layout/layout.js until `smoke.cjs` exits 0 and the door pill returns to ✓ 17/17.
