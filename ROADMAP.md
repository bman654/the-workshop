...
- [bug] **District captions should NOT counter-scale down as you zoom in — they must scale WITH the basemap, the way the sky's constellation/asterism names do.** The front door's `scaleCaption()` applies a 1/k counter-scale to every district zone-label so it holds a constant SCREEN size; but a district name reads as part of the basemap, so shrinking it relative to the plate as you zoom in is confusing. Fix: drop the 1/k counter-scale for `#zonelabels` so district captions grow with the map like the rest of the basemap.
- [bug] **On any zoom > 1 the THE MANOR HOUSE district caption becomes invisible — no other district caption does this.** Reads as a bug. Likely root cause (couples to the loupe-centering bug below): the loupe focus is pinned to the map centre, which sits on the manor, so on EVERY zoom the manor's rooms self-name and its caption fades by `op = ZONE_FULL − 0.16·n` down to the `ZONE_FLOOR` 0.06 (effectively invisible), while every other district — never reached by the centre-pinned loupe — keeps its full 0.7. Fixing the loupe centering should largely fix this; regardless, the manor caption must stay legible when zoomed.
- [bug] **Wing captions must become visible BEFORE room labels — a wing is a higher LOD tier than a room, but today room labels light the instant k > 1.001 while wing captions only begin at WING_LOD_IN = 1.25, inverting the hierarchy.** Suggested fix: WING_LOD_IN = 1, WING_LOD_FULL = 1.25, and gate room labels behind a NEW LABEL_LOD_IN = 1.25 so rooms only start naming themselves once the wing captions are at full LOD.
- [bug] **Wing captions should NOT scale with zoom either — same defect as the district captions.** Like district names, the wing sub-captions read as part of the basemap and must hold their size RELATIVE TO THE BASEMAP, not stay a constant screen size. Fix: drop the 1/k counter-scale on `#winglabels` so wing captions scale with the plate.
- [bug] **Room labeling is broken: the loupe is centred on the TOTAL map extent, not the currently-VISIBLE extent.** Zoom in and pan west to view the Observatory and you STILL see only the manor's labels (map centre) — the faintly-visible loupe ring stays pinned to the centre of the plate. Root cause: `centreFocus()` calls `svgPoint(innerWidth/2, innerHeight/2)`, and `svgPoint()` uses `sheet.getScreenCTM()` — the #sheet's CTM does NOT include the #viewport's translate/scale, so the screen-centre pixel always inverse-maps back to ~(720,450) regardless of how far you've panned. Fix: compute the focus in #viewport-local coordinates (use `viewport.getScreenCTM()`, or invert analytically: focus = (sheetPoint − {tx,ty}) / k) so the loupe follows the pan and names the rooms you are actually looking at.
- [bug] **A room detail popup with a lot of content grows past the viewport and gets clipped** — e.g. The Lodestone Hall's popup on a 619px-tall viewport. The `#card` panel needs a `max-height` (e.g. `calc(100vh − <margin>)`) plus `overflow-y: auto` so long content scrolls inside the card instead of spilling off the bottom of the screen.
...
- [room] **The Card Catalog — every exhibit in the estate, found by the room you flip through to reach it** — the estate has ~158 rooms on a growing map and no single place that holds them ALL; the front-door plate and the loupe keep any one district legible but never give a visitor a way to BROWSE or SEARCH the whole collection and jump straight to a piece. FORM past a list: a touchable antique card-catalog / rolodex / ledger-book — every state change (drill in, step back up, type a query) plays the physical device (drawer slides, the rolodex rolls, the page flips) and the new card/page shows the updated set. Browse the SAME catalog four ways — by map location (district › wing › room › exhibit, with a step-BACK-UP after drilling in), by subject, by order of entry into the estate, and alphabetically — plus keyword search over title+description; each card shows the exhibit's info and clicking it travels you straight there. HONORS LOCK-GATING: an exhibit still sealed by the unlock system (Undercroft, cavern drift, etc.) does not appear in browse or search until it is unlocked. DISCOVERABILITY HOOK: the embedded place/exhibit data is RE-PINNED every cycle by the one script guaranteed to run each cycle (collate.sh), the same convention ledger-bound rooms use — so the catalog can never go stale as the estate grows. New top-level room with its own POI/star; a navigation room whose FORM is the act of finding. (sown #230 · contest #20)
