...
- [room · GRAND · **OPEN & GROWING**] **Physics Lab** → **The Cavern** (`cavern/`) — **BLOOMED into a real front-door WING 2026-06-13**: a cave on the grounds, two drifts off a central shaft (Newtonian + Einsteinian) + a sealed Quantum back-passage. **First 3 benches shipped** — Light Clock (7/7), Newton's Cradle (6/6), Mercury's Precession → 42.98″/century (6/6). **STILL AHEAD (this seed stays live):** (a) **more benches per open drift** — the menu in [worklog/physics-lab-plan.md](worklog/physics-lab-plan.md) §2 (Newtonian: Three-Body Choreography, Gyroscope, Kepler-law-prover, Ballistics; Einsteinian: Black-Hole Lens, Stellar Collapse, Doppler, Gravitational-wave chirp); (b) the **hidden Quantum vein** — build Double Slit / Particle-in-a-Box / Tunnelling + the **"walked both drifts" unlock** (`ws:seen:cradle`∧… ∧ `ws:seen:light-clock`∧… → a `quantum-wing` SECRETS predicate; the benches already drop their breadcrumbs), per §3. A multi-session wing — extend incrementally like the Hall (9→12). (Brandon's seed; fleshed → opened 2026-06-13.)
...
| **The Physics Lab's quantum wing** | **teased, not yet built** — the Cavern wing is now OPEN (2026-06-13) with a *sealed* Quantum back-passage rendered on its index | A hidden sub-wing earned by walking both open drifts. The 3 shipped benches already drop `ws:seen:{light-clock,cradle,precession}`; building it = add the Q-benches (Double Slit / Particle-in-a-Box / Tunnelling) + a `quantum-wing` predicate in `tools/ws/ws.js` `WS.SECRETS` + open the index's sealed vein when unlocked. See `physics-lab-plan.md` §3. |
| **The Cavern feats / "Experimenter" constellation** | **not built — a consideration, not a mandate** | The Cavern *could* feed a new Survey-of-Heaven constellation (the way the Hall feeds "The Optician") once the wing fills out + the Quantum unlock gives a reason to reward exploration. Decide at a future build; don't bolt it on. |
