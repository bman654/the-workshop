...
<!-- gauge:bug:start -->
- [bug] **The Fairground Gate detach (#369) never actually FOLDS in the live page — the whole feature is non-functional in the browser though it shipped green on the modeled twins.** INTENT (from the feature's own docs): the `amusements` wing declares `detach:true`, so the engine should fold the WHOLE wing OUT of the parent plate into its own `child:amusements` layer — the 15 ride tiles VANISH from the parent and the engine leaves ONE synthetic GATE FACE tile in the hole; clicking the gate `descend()`s the camera into the child "midway" sheet where the rides fan out airily; this crowding relief is what flips door CLAIM C′ ✗16/17 → ✓17/17 (the detach-OFF neg-control stays red, proving DEPTH did it). ACTUAL (live, broken): the 15 amusement tiles are STILL on the parent plate (crowded), the gate arch is drawn OVER them (the fold never removed them), a REAL pointer click on the gate does nothing, and the live `#doortest` pill is RED 16/17 — i.e. C′ never flipped, because the fold isn't happening. THE THROUGH-LINE: the node twins (`fold.test.cjs`, `door.test.cjs`) report the fold works + C′ green, and the build log claims "VERIFIED in-browser, pill flips green" — yet the RENDERED page shows the fold did NOT happen + pill RED. Everything green is modeled/synthetic; the real render is broken (a #335-style false-green / the #337 blind spot). ROOT-CAUSE DIRECTION: the engine `plates()` computes `detached`/`childLayout`/`gates` correctly (the model proves it), but the PAGE (the platewalk module in `index.src.html`) isn't APPLYING that output to the live DOM — the amusement `.poi` tiles aren't removed from the parent (they should move into / hide as `child:amusements`), and/or `descend()` is never reachable because the gate face isn't truly clickable (SVG hit-tests only PAINTED pixels and `.gate-face` is a `<g>` of thin strokes with no filled hit-area → a synthetic `.click()` fires but a real click in the open arch misses; also check a z-order swallow — platebar precedent in `index.src.html`). FIX: make the fold ACTUALLY execute in the live render — the parent shows ONLY the gate (amusement tiles gone), a REAL click on the lit gate descends into the `child:amusements` midway where the rides live, and the live door pill reaches 17/17 GREEN. Plus close the verification gap that hid this: a real hit-tested click test + extend the door/fold twins to assert against the LIVE RENDERED DOM (getBBox), not just the model. VERIFY in a REAL browser (agent-browser): tiles gone from the parent · a real click descends · live `#doortest` green — never via a synthetic dispatch or the modeled twin alone. Same blind spot as the #337 door bug, now several times over.
