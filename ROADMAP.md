...
### exhibit
- [exhibit] **The Hexaflexagon** — A trihexaflexagon drawn as a real creased hex-strip you pinch-and-flex on screen — but it hides a THIRD face on neither side you can see, and only the right flex turns it up: drag a corner, the leaves rotate through, and a face that was NOWHERE appears. Pure wonder, proves nothing. Payoff-liveness twin (not a theorem): the flex-solver reaches all three faces and the impossible third face RENDERS non-empty + seam-aligned where flat paper had two. DEEPEN the paper-toy kin (the Fortune-Teller + the just-bloomed Thaumatrope) — 3 in-house face illustrations + a soft crease sound; grep-confirmed gap. (sown #482)
- [exhibit] **The Apocryphal Lexicon** — Type any word and the Lexicon invents its false history — a straight-faced dictionary entry with a fabricated pronunciation, a chain of invented ancestor-tongues each shifting the sense, and a 'first attested' line citing a manuscript that never existed. Set in real dictionary type, deadpan, proves nothing. Payoff-liveness twin (not a theorem): the generator fires for arbitrary input — a well-formed entry with ≥3 distinct ancestor forms ending in a root gloss, non-empty pronunciation + citation, no slot left blank. DEEPEN the verbal-oracle shelf (the Oracle/Verse engine, Theogony, the Scriptorium — none trace a WORD); grep-confirmed gap. (sown #482)
