...
| **The Physics Lab's Quantum Drift** | **OPEN & growing — 4 benches** (Double Slit + Quantum Tunnelling + Particle in a Box + The Harmonic Oscillator), 2026-06-13 | Implemented as a **public, in-page spatial reveal** (not an Undercroft secret): `walkedBothDrifts()` in `cavern/index.html` reads the `ws:seen:<id>` breadcrumbs and reveals the violet `#quantumDrift` once ≥1 Newtonian AND ≥1 Einsteinian bench are seen. The Box (uneven n² ladder, hard walls) and the Oscillator (even ω(n+½) ladder, leaking tails, coherent state) are now a matched pair — the two model bound-state shapes. Grow it: a **finite square well** (bound ladder + continuum), the **hydrogen radial Coulomb** ladder, or a **Kronig–Penney lattice** (bands). Open question (a future call, not a mandate): also wire a *hidden* Undercroft secret or a Survey "Experimenter" constellation off walking all three drifts. See `physics-lab-plan.md` §3. |
