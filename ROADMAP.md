...
### exhibit
- [exhibit] **The Coin That Lies — find the fake in three weighings, and no fewer** — A brass two-pan BALANCE you load: drag coins on, the beam tilts {<,=,>} and physically PRUNES a live suspect rack into thirds; solve 12 coins in ≤3 weighings and a 'no fewer was possible' seal lights (FORM: a touchable scale + shrinking rack, not a graph). CLAIM: W weighings distinguish ≤3^W cases, so ⌈log_3(24)⌉=3 is necessary AND sufficient for 12 coins (light/heavy = 24 cases). SOLE core.mjs (weighing/probe/distinguish/log3Bound/runSelfTest) + Node twin: (A) each {<,=,>} exactly partitions the live set (disjoint+total); (B) max cases after W===3^W, ⌈log_3 K⌉ the floor; (C) the preloaded 12-coin schedule covers all 24 leaves at depth exactly 3. NEG-CONTROL: a 2-outcome balance (or 13 coins) → thirds break, optimal depth jumps, seal RED. Numbers Room bench; sib-link entropy/ as a CONTRAST (questions-asked vs symbols-coded), never duplicating Shannon. (sown #223)
- [exhibit] **The Belief Beam — watch your odds slide as the evidence lands** — A horizontal BEAM of glass vials, one per hypothesis (which of 3 hidden urns?), each filled to its current probability; DRAW a bead → mass pours between vials, levels always summing to 1, and a log-odds slider shows the same update as a straight slide (evidence ADDS) (FORM: vials whose levels pour, not a graph). CLAIM: posterior ∝ prior×likelihood (renormalized); log-odds shifts by ln(ratio) per independent draw, order-free. SOLE core.mjs (prior/likelihood/posterior/update/logOdds/runSelfTest) + Node twin: (A) Σ posterior===1 to <1e-12 every step; (B) log-odds additive + shuffled-evidence yields identical final posterior; (C) an equal-likelihood draw leaves the posterior bit-identical. NEG-CONTROL: skip renormalization → levels stop summing to 1, belief lands wrong; OR feed correlated draws as independent → log-odds over-shoots past certainty. NEW chance/inference vein (no Bayesian room exists); distinct from gene-jar's fixed HW pool and clockwork's softmax. (sown #223)
...
- [bench] **The Chirp — two stars that fall together by ringing spacetime** — Grows the Stellar Forge (GR radiation-reaction is a confirmed gap — grep finds zero gravitational-wave/inspiral/chirp/quadrupole estate-wide). Set two mass dials + a starting separation, LET GO, and the binary spirals IN on its own as the orbit bleeds energy to gravitational waves: separation shrinks, orbital frequency RISES, the speaker plays the rising chirp you HEAR while a strain trace h(t) sweeps to merger and a stopwatch reads time-to-coalescence (FORM: touchable AND audible, ties the Forge to the sound vein). CRUX: the-chirp/core.mjs (SOLE quadrupole-radiation authority) + Node twin, reusing the sound vein's WebAudio grammar: the energy-balance identity (radiated===orbit-lost each step, <1e-9) is the rigorous claim, with leading-order f∝(t_c−t)^(−3/8) its consequence framed as the leading-order inspiral; the whole chirp derives from ONE chirp-mass M_c=(m1·m2)^(3/5)/(m1+m2)^(1/5) — swapping masses leaves M_c and the chirp invariant <1e-9. NEG-CONTROL: a 'freeze the radiation' toggle zeroes dE/dt → fixed circle forever, flat frequency, no chirp, t_c→∞; a wrong mass-power fires the merger-time check RED. Nested sub-bench, no front-door footprint. (sown #223)
<!-- ✝ BLOOMED #218: The Gene Jar — the pool that won't drift → conservatory/the-gene-jar/ · after 65b7d10 -->
<!-- ✝ BLOOMED #219: The Voltaic Pile You Build — two metals decide the voltage → alchemy/galvanic-cell/ · after bf920bb -->
<!-- ✝ DECAYED #223: The Same Wedge — colliding blocks and a mirror-wedge both count ⌊π/θ⌋ r… · after 525e77a -->
<!-- ✝ DECAYED #223: The Same Climb to φ — a Penrose patch and a fraction-hunter ride one Fi… · after 525e77a -->
