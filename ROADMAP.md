...
### exhibit
- [exhibit] **The Comb's Teeth** — add a steady tone (or pink noise) to a short-delayed copy of ITSELF and HEAR the jet-plane flange whoosh while you SEE the spectrum grow comb teeth — drag ONE brass delay slider (τ~0.2–12 ms), the teeth slide; a feedback knob deepens the notches toward cancellation, an LFO toggle auto-sweeps into the classic flange. FORM: a touchable instrument + a live spectrum the teeth ride (the drawn comb IS the heard timbre, the Sidebands idiom), NOT a transfer-curve plot. CRUX (machine-ε, core.mjs + OfflineAudioContext twin + audio-lens FFT): the feedforward comb |H(f)|=|1+g·e^(−j2πfτ)| has its first deep notch at 1/(2τ) and notch spacing 1/τ EXACTLY (pin value + tol; a swept render confirms the measured nulls land on (n+½)/τ); NEG-CONTROL τ=0 (or g=0) → flat spectrum, NO teeth — the DELAY arms the comb, not the gain. SHIP the FEEDFORWARD comb + clamp any optional feedback hard. grep-confirmed ZERO comb/flange/phaser in the Sound Garden. Small art (brass slider + glowing spectrum strip), no foundry pass. (sown #325)
...
<!-- ✝ BLOOMED #321: The Stopped Pipe → sound-garden/the-stopped-pipe/ · after 47e6915 -->
