...
<!-- gauge:bug:start -->
- [bug] **The front door's WEST GROUNDS renders three plates stacked on one lot — The Holonomy Walk, The Drawing Room and The Lodestone Plate pile into the same patch of the lower-west park, and the engraved wing labels "CURVED COUNTRY" and "DRAWING ENGINES" print almost on top of each other.** The door's own conscience already flags it: the fit-view header reads "FULL PLATE CROWDED (0.91)". ROOT CAUSE — two independent faults in the grounds wing budget (tools/layout/layout.js, the GROUNDS_WINGS table) plus a missing structural guard: (1) TWO GROUNDS-WING SUB-ANCHORS OCCUPY THE SAME GROUND — `GROUNDS_WINGS['curved-country'] = {x:170,y:520,w:150,h:160}` and `GROUNDS_WINGS['drawing-engines'] = {x:166,y:540,w:150,h:150}` overlap by ~91% (x 170–316, y 540–680 in common). curved-country holds The Holonomy Walk; drawing-engines holds The Drawing Room — so the two rooms' plates land on the same ground and stack, and the two wings' engraved labels collide. Each wing was placed independently into "the clear lower-west band" and each builder verified its rooms clear of STARS and other ROOM FOOTPRINTS via the live Layout.solve — but nothing checks one wing's region against another's, so the second wing dropped straight onto the first. (2) THE LODESTONE PLATE IS ORPHANED FROM ITS WING — room `iron-filings` (The Lodestone Plate) declares `district:"grounds"` with NO `wing:` field, while its own sibling `lodestone-hall` (and the rest of the induction bench) carry `wing:"induction"` (sub-region {x:600,y:690}). So The Lodestone Plate never joins The Lodestone Hall in the induction lot; it falls back into the crowded lower-west band and becomes the third plate in the pile. (3) NO STRUCTURAL GUARD — there is no assertion anywhere that two GROUNDS_WINGS regions are mutually disjoint (a grep finds none). The solver's own header promises "a footprint can never collide," but that invariant is enforced only for the auto-packed manor/shelf districts; the hand-budgeted grounds wing anchors have no mutual-overlap check, so a typo or a too-optimistic "clear band" silently stacks wings. FIX (root cause, not symptom): (a) move one of the two colliding wings to a genuinely clear band so curved-country and drawing-engines are disjoint — re-budget via the live Layout.solve, confirming clear of stars AND every other wing region, not just room footprints; (b) give `iron-filings` `wing:"induction"` so The Lodestone Plate sits with The Lodestone Hall in the induction lot, and confirm induction's region still fits its now-larger room set; (c) ADD a hard build-time check that asserts every pair of GROUNDS_WINGS regions is mutually disjoint (a HARD BUILD ERROR on overlap), wired into smoke.cjs / the solve self-test, so this whole class of wing-on-wing collision can never ship again. VERIFY by rendering the West Grounds in a real browser (fit-view AND zoomed): no two plates overlap, every wing label stands alone, the door header no longer reports a crowded plate, and the layout self-test runs green. SUGGESTION (forward-looking, once this fix lands and the plate is clean): RATCHET the cleared state so it cannot silently regress as new rooms and wings are added. Today the conscience treats crowding as a WARNING ONLY — the door still reads PASSABLE at "FULL PLATE CROWDED (0.91)" and smoke.cjs logs the full-plate density as "⚠ WARNING (intended · #103)". Harden it in two parts that respect the existing design: (i) PLATE/REGION OVERLAP — any two plates, room footprints, or wing regions sharing ground is unconditionally wrong, so make it a HARD BUILD ERROR (this generalises fix (c) above from wing-anchors to every plate pair); (ii) FULL-PLATE LABEL CROWDING (#103) — this density is deliberately tolerated because the visitor TRAVELS between plates rather than reading every label at once, so do NOT flip it to an absolute fail; instead capture the post-fix CLEAN composite as a BASELINE and FAIL the build if a later addition pushes crowding measurably above that baseline — a regression ratchet that keeps the #103 rationale intact while stopping the plate from drifting back toward crowded. Net intent: it is green now, so add the gate that keeps it green. (Implementation is the maker's call — absolute gate vs baseline ratchet vs a tightened threshold; the goal is that the next room which would re-crowd or overlap the plate trips a build error instead of shipping.)
