...
- [exhibit] **The Soap-Film Surveyor — the shortest road, found by a film (Steiner / Plateau 120°)** — drag brass posts between two glass plates; the soap film between them SNAPS to the shortest connecting network — an analog computer that solves with no search, AND a game you lose to physics (lay your own roads first, drop the film, read how much longer your guess was); drag a post and watch the web re-snap through a transient X, switching topology. CRUX (exact-within-model; self-test vs an INDEPENDENT Node Steiner solver to <1e-6, not the same relaxer): every junction is a 3-way 120° meeting (Plateau's law); relaxed length == the Euclidean Steiner minimal tree — unit square 1+√3 < diagonal-cross 2√2 < spanning tree 3. NEG-CONTROL: a forced 4-way '+' is unstable and splits into two 120° junctions (proves 120° is the minimizer, not decor). GAP: soap-film/ is the catenoid ONLY — deepens the thin surface-tension corner, not a lone folly. (The relaxer must be a genuine length-minimizer, never a lookup.) (sown #348)
