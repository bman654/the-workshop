...
<!-- gauge:bug:start -->
- [bug] **The descended child map is not an ISOLATED context — the whole parent estate lingers underneath it. Four coupled symptoms, one through-line.** Clicking the Fairground Gate now descends correctly (#378), but the layer you land in is half a map: it relays the 15 amusement tiles into the airy midway, yet leaves the entire estate present behind them. THE THROUGH-LINE: `descend()` reuses `enterTour()`'s PARENT-DISTRICT-TOUR semantics (dim-the-others, keep all chrome, declutter against the whole estate, drop-out-on-manual-pan) instead of entering a self-contained CHILD-MAP MODE. The four symptoms, each measured in the live render: (1) NON-CHILD POIs ARE DIMMED, NOT HIDDEN — 71 of 86 `.poi` tiles linger at `opacity:0.16` (only the 15 amusements show full); `enterTour` (~index.src.html L4689) sets every non-toured tile `opacity:0.16` rather than `display:none`. In a child layer the parent's POIs must be HIDDEN, not dimmed. (2) PARENT DISTRICT/WING CHROME STAYS DRAWN — 14 district boundaries + 31 wing-bound capsules still render behind the fairway; `enterTour` only touches `.poi` tiles, never the precinct/`.wing-bound`/wing-label chrome. In a child layer ALL parent chrome (districts, wings, the other gates) must be hidden. (3) CHILD LABELS ARE SPORADIC — only 5 of the 15 amusement POIs get a label (arcade, level-ride, standing-stones, murmuration-meter, brazil-nut-box; the other 10 are bare). The loupe is handed the child id-set (~L4706) but the declutter still competes for space against the 71 still-present (dimmed) PARENT labels + canonical chrome, so most child labels lose and are suppressed. The label declutter must be SCOPED TO THE CHILD SET ONLY when inside a child layer. (4) ANY MANUAL PAN/ZOOM EXITS TO THE ESTATE — `window.__panCamera.onManual = ()=>{ if(here) freeExplore(true); }` (~L4731) drops out to free-explore whenever `here` is set, INCLUDING a child layer. That is right for an estate-district tour (pan = "I want the whole estate back") but wrong for a child map: inside a child, pan/zoom should NAVIGATE WITHIN the child (clamp to the child frame), and ONLY the depth ribbon / Back / Esc should ascend. ROOT FIX (make it a real mode, and make it GENERAL so every future `detach:true` wing inherits it — this is meant to be the TEMPLATE for child maps): branch `enterTour`/`descend` on `isChild(pid)` into a CHILD-MAP MODE that — hides (display:none) every non-child plate's tiles AND all parent district/wing/gate chrome (restore them on `ascend()`); scopes the loupe/label declutter to the child set so every child POI that should be labeled is; and rebinds manual pan/zoom to stay inside the child (clamp to the child frame, suppress the free-explore drop-out) with the ribbon as the sole exit. VERIFY in a REAL browser with REAL input (agent-browser `click`/`scroll`, not dispatchEvent): once descended, the parent estate is GONE (0 visible non-child POIs, 0 parent district/wing bounds) — only the child's POIs + the midway ground + the ribbon; every child POI that warrants a label HAS one (declutter computed over the child set alone); a real wheel-zoom and a real drag PAN WITHIN the child without kicking out; only ribbon/Back/Esc returns to the estate; the `#doortest` pill stays green. Get this right and the next detached wing is free.
