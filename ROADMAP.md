...
### exhibit
- [exhibit] **The Equal-Area Sweep — Kepler's 2nd law you drag, not read** — Grab a planet and slide it around ONE fixed ellipse; a focus-anchored brass arm sweeps a filled pie-wedge and a row of fired wedges shows EQUAL areas in equal time — the hand drags heavy/fast at perihelion, feather-light/slow at aphelion (enact the speed as a real visible cue or cursor-resistance, NOT a caption). FORM: a drag-instrument, not the orrery's read-only plot; cross-link to orrery. CRUX: constant areal velocity dA/dt = L/2 (r²·dθ/dt = const) — every swept wedge equal to machine precision; neg-control = a circle (uniform) vs a high-e ellipse (lopsided). Grep-confirmed gap: orrery/aerodrome carry no areal code. (sown #247)
- [exhibit] **The Transit — find the planet by the star that blinks** — Pass a dark planet across a star disc; brightness dips a flat-bottomed notch while it's fully on the face, and the dip DEPTH alone reads the planet's radius. Playable round-game: a hidden planet crosses, you watch only the light, drag a size-caliper to match the dip, lock + score. FORM: a fun guessing game, not a plotted light-curve. CRUX: fractional dip ΔF/F = (R_planet/R_star)² (areal occultation) — grazing = shallow V, full transit = flat U; depth = radius-ratio² exact. Grep-confirmed gap: no transit/occultation/light-dip piece in the estate. (sown #247)
...
<!-- ✝ BLOOMED #243: The Wire That Jumps → lodestone-hall/the-wire-that-jumps/ · after 58c25bc -->
