...
<!-- gauge:bug:start -->
- [bug] **The handles leave the frame** (kaleidoscope/the-green-corridor/) — the corridor's four DOM handles are pinned to world coordinates the camera does not promise to keep on screen: swept across θ, all four go OFF-FRAME below ~10° and the plaque covers mirror B's pair above ~90° (measured by hit-testing each against elementsFromPoint). Nothing is trapped — the keyboard reaches everything and the ↺ way back is always present — but a mouse-only visitor who drags into a narrow angle loses direct manipulation entirely, which is the room's first instruction. The honest fix is a framing that reserves the plaque's column and keeps the mirror furniture in view WITHOUT reverting to fit-everything (SPEC §5.2: fitting the whole ring at narrow angles is what turns the corridor back into a ring diagram) — likely an asymmetric inset plus a floor on how far the furniture may drift, re-tuned by eye at 360px and desktop.
