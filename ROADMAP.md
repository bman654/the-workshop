...
### exhibit
- [exhibit] **The Mirage** — a hot road shimmers with a sky-puddle that is never there: drag a HEAT slider under the asphalt and watch a car's wheels dissolve into their own upside-down reflection, the blue puddle creep toward you, then stack warmer/cooler layers into a desert FATA MORGANA. FORM: a touchable roadside scene where rays VISIBLY CURVE (a smoothly bending beam, not the kinked Snell-interface of The Photon's Errand) — the false puddle and flipped reflection form live, not a ray diagram. CLAIM (self-test): in a continuous index gradient n(y) a ray obeys the eikonal n(y)·sinθ(y)=const; for a hot near-surface layer a ray below the critical grazing angle TURNS BACK before the road (inferior mirage), and the apparent-puddle horizon sits exactly at that turning condition. NEG-CONTROL: set dn/dy=0 (uniform air) — every ray goes dead straight, NO puddle, NO flipped image, turning-solver returns 'none'. Replaces the decayed Snell's Window in the same critical-angle vein, far more touchable. (sown #287)
- [exhibit] **The Pool That Dances** — the bright wobbling NET of light on a pool floor is a caustic, sunlight focused by the lens of every wavelet. Drag a finger across the surface and the net reorganises live: pinch a bump and a knot of light blooms under it; rake a wave-train and it combs into rolling bright ribbons; go flat and the floor washes even. FORM: the dancing caustic itself is the medium you touch — poke/rake the SURFACE, floor-light is the readout (no graph; distinct from the optics ray-bench and the cardioid envelope). CLAIM (self-test): caustics lie on the fold of the Snell-refracted surface→floor map — exactly the zero-set det J=0 of that map's Jacobian — and total light is conserved (∫ floor-brightness constant under any deformation, refraction only redistributes). NEG-CONTROL: a flat surface gives the identity map, J≡1, no zero-set → uniform floor (caustics ONLY where curvature exists); conserved-flux integral holds to <1e-6 across every poke. The 'caustics you sculpt'. (sown #287)
- [exhibit] **The Grain Mill** — tip a held tone into a brass mill-hopper and it crushes the sustained pitch into a swirling, VISIBLE cloud of overlapping grains you HEAR shimmer into mist. Two sliders — grain SIZE (recognisable pitch ↔ breathy texture) and DENSITY/scatter (pointillist plinks ↔ continuous wash) — dissolve a cello note into rain, then a glassy drone, then a stuttered echo. FORM: a touchable hopper + two sliders; a living drift of luminous grains breathing across the pane is the visual; the payload is the transformation you hear. Granular synthesis is the ONE major synthesis family the sound-garden lacks (pluck/FM/additive/formant/Shepard/beating all present, never grains). Owes NO proof — an instrument, engineered-pleasant (consonant source, master limiter, pooled/capped grain voices), lens-native (offline render → peak dBFS/clip). Disambiguate the name from the benford-mill / tone-mill pieces. (sown #287)
...
### bench
- [bench] **The Nimber Strip** (Numbers Room) — a felt strip over a subtraction game (take 1/2/3, last stone wins) you TAP against a perfect AI; each cell lights its own Grundy value g(n)=mex{g(n−1..n−3)} and you watch the nimbers fall into the period 0,1,2,3; a lever SPLITS into heaps, lights their XOR in Nim's binary place-lamps, and the best move ZEROES it — an unrelated game revealed as secretly one nimber. FORM: a playable board + lit Grundy beads, not a code comment. CLAIM (core.mjs+twin): g(n)=n mod 4 for n≤200 recomputed via mex (not hard-coded); a multi-heap position is a LOSS ⟺ XOR of component Grundy values=0; the AI's move always lands XOR=0 when one exists. NEG-CONTROL: switch to misère (last stone LOSES) and the XOR=0 rule mis-predicts one explicit endgame, proving the invariant is win-condition-specific. The sharper, playable re-pitch of the decayed 'Every Game Is Secretly Nim'; uses a NEW game so it isn't a Nim re-skin. Sow APART from Squaring Clock / Clock That Closes. (sown #287)
...
<!-- ✝ BLOOMED #285: The Vowel Throat → sound-garden/the-vowel-throat/ · after 61406e3 -->
<!-- ✝ BLOOMED #286: The Marbling Bath → strange-garden/pieces/the-marbling-bath.html · after 0746a00 -->
<!-- ✝ DECAYED #287: Snell's Window · after 9c49cb4 -->
<!-- ✝ DECAYED #287: Every Game Is Secretly Nim · after 9c49cb4 -->
