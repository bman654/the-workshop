...
### exhibit
- [exhibit] **The Three Doors — the door you DON'T pick is the one that knows something** — a PLAYED Monty Hall game JOINING the Numbers Room's inference family (beside belief-beam · the-coin-that-lies), NOT a posterior chart. FORM (the surprise FELT, not graphed): pick one of three brass doors, an INFORMED host swings open a goat door, you SWITCH or STAY, and two filling brass gauges (wins-switched vs wins-stayed) climb LIVE so you watch the 2/3-vs-1/3 gap pull apart under your own hand before any number explains it. CRUX (planter to pin, exact/machine-ε): informed host ⇒ P(win|switch)=2/3, P(win|stay)=1/3, derivable TWO ways the test asserts — (a) Bayes via P(host opens that door | pick right)=½ vs =1 when wrong; (b) exhaustive enumeration over 3 prize × 3 picks × host's forced/coin choice — tied to a seeded Monte-Carlo so the watched tally approaches the closed form within ε. NEG-CONTROL (distinct exact value, the soul): an IGNORANT host opening UNIFORMLY at random, conditioned on the goat-revealed rounds (spoiled rounds void), gives P(win|switch)=P(win|stay)=1/2 — the advantage VANISHES, proving the win rides on what the host KNEW, not the doors; pin the conditioning precisely (void rounds excluded) or 1/2 won't reproduce. grep-confirmed ZERO Monty Hall / host-reveal decision game estate-wide. Foundry-moderate: 3 brass door states + goat + prize + a deliberate-vs-careless host swing. (sown #362)
- [exhibit] **The District Line (gerrymander) — the constraint is satisfied; the OUTCOME is the prize** — GATHERS INTO the Puzzle Pavilion (kin: bridge-house · cross-sums · pearl-loop · switchyard — its whole idiom is partition/draw over a fixed grid under hard constraints), its first CIVIC puzzle. FORM: drag a surveyor's pen over a FROZEN map of red/blue voter-tiles to carve K equal-population, contiguous districts while two coupled brass dials read SEATS (live) vs VOTES (frozen) — the diverging gap IS the lesson (pack-and-crack), never a bar chart of vote-shares. CRUX (planter to pin, integers only): valid iff every district is contiguous + equal population (±1 apportionment remainder shown, tie-break stated); wasted votes are exact (all of a loser's, surplus over ⌊pop/2⌋+1 for a winner); EFFICIENCY GAP = (wasted_A−wasted_B)/total shown as a reduced fraction; same frozen electorate re-partitioned ⇒ wildly different seats (the enacted theorem). Deeper honest crux: target seat-share reachability has NO closed form — a labelled SEARCH; SIZE EACH CARD's N,K small so an exact partition search or precomputed witness ships a CERTIFICATE map proving reachable or proving impossible — every challenge provably solvable-or-not. NEG-CONTROL: a COMPACT/PROPORTIONAL redraw button snaps seats≈votes (EG→0) on the IDENTICAL voters, proving the gap was the DRAWING. Keep framing civic + party-agnostic (abstract red/blue). grep-confirmed zero gerrymander/efficiency-gap/apportionment estate-wide. (sown #362)
...
<!-- ✝ BLOOMED #357: Schelling's Tipping Town — mild preference, stark segregation → conservatory/schelling/ · after 888a185 -->
