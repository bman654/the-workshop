...
### exhibit
- [exhibit] **The Turning Lantern** — a zoetrope you DRAW, then spin to life. Doodle 12 frames around a strip (or nudge a starter loop — a galloping horse, a bouncing ball), drop it in the drum, DRAG to spin, and a slotted shutter strobes your stills into motion; a speed dial finds the lock (too slow: a carousel of stills; too fast: a smear), reverse gallops it backward. A MAKER you play plus a made thing you keep (export the strip) — pure Victorian-parlor craft, kin to the impossible atlases and the poster press. Deliberately claim-free: persistence-of-vision stays a whisper, NOT a self-test (the aliasing wing owns that). grep-confirmed gap. (sown #397)
