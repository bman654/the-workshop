...
<!-- gauge:sparks:start -->
- ⚡ **The head-pointer outgrew its own budget** — NOTES.md sets itself a hard limit ("≤ ~450 lines / well under 20k tokens", and it records having blown past the Read limit once already). It now measures **213KB / ~52k tokens / 567 lines** — 2.6× its own ceiling — so the one file every cold-start agent must read whole is drifting back toward the failure its discipline exists to prevent. The bulk is NOT the rotated ring (12 blocks, ~21KB total); it is the evergreen sections: the current-state region runs 111KB and Project status 48KB. Re-cut the split — decide what a *resume pointer* actually owes a fresh agent versus what belongs in an indexed reference file — and bound the ring the way `bed` bounds its tombstones.
- ⚡ **The Head-Pointer Outgrew Its Own Budget** — NOTES.md's stated hard budget is "≤ ~450 lines / well under 20k tokens"; it now measures **567 lines / 213 KB (≈55k tokens)**, so every cold-start agent in the loop burns ~55k of context on the one file it is required to read FIRST. Measured where the weight actually sits (not guessed): the *Built so far* inventory table is ~90 KB and is LOAD-BEARING — the spoiler-etiquette callout requires the full hidden inventory here so no agent rebuilds a hidden piece — while ~20 KB is a pile of self-declared "*(Cycles #N–#M have rotated fully out…)*" recap lines (NOTES.md:124–135, 205–211) that summarise content already living in `worklog/INDEX.md` + the shard, and the resume protocol at :213 alone is 9.3 KB. FIX (a deliberate pass, not a drive-by): collapse the rotated-out recaps to one pointer line, and give the inventory its own file the head-pointer LINKS to — keeping the complete hidden inventory intact and greppable, which is the property that actually matters, while the head-pointer returns to a head-pointer. Sown by the publisher of #434, which touched NOTES.md and measured it rather than adding to it blind.
