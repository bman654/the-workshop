...
### exhibit
- [exhibit] **The Taking-Apart Table** — the colophon invites you to "take anything apart to see how it works" and the estate has never once let you. A felt-topped table under a lamp with one of the estate's OWN instruments on it, seeded per visit (the Orrery, the Astrolabe, the Planimeter, the Volvelle — each panel a door back to its room): drag a piece and it lifts free and hangs; keep pulling and the whole thing blooms into a slow exploded view, every part named in copperplate, until a brass SETTLE lever lets it all fall gently home. PURE DELIGHT, proves nothing — this is anatomy for the beauty of it, and a builder must NOT bolt a kinematics claim on. PAYOFF-LIVENESS twin (headless): the real pulls reach fullyExploded with every part labelled, and settle() returns each to its exact origin pose, bit-equal across ten rounds and across an interrupted bloom. grep-confirmed absent; deepens four standing instrument rooms instead of detaching. (sown #430)
...
### rework
- [rework] **The Reaction Balancer** — a chemistry bench where the visitor never balances anything: `paintControls()` offers a row of reaction buttons and nothing else, `solve()` has already answered before you arrive, and the per-element gauges hide behind a toggle that starts OFF. It is a solver with a menu, not an instrument. RE-SOUL: put the equation on a BEAM you can tip — a hand-driven coefficient stepper per species, the beam leaning as atoms pile up on the wrong side, per-element gauges ALWAYS ON so you see which element is fighting you, the balance point FELT rather than fetched. Demote the exact nullspace solver to a quiet "show me" layer for when you give up, re-extraction sentinel parity intact and core.test.mjs untouched (47/47 green as of #430 — the math is sound; the mark is about buried play). (sown #430)
