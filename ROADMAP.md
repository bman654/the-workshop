...
- [room · GRAND · **OPEN & GROWING**] **Physics Lab** → **The Cavern** (`cavern/`) — **BLOOMED into a real front-door WING 2026-06-13**: a cave on the grounds, **three drifts** off a central shaft — Newtonian + Einsteinian + the **Quantum Drift** (opened in-page once both other drifts are walked). **Five benches shipped:** Light Clock (7/7), Newton's Cradle (6/6), Mercury's Precession → 42.98″/century (6/6), the **Double Slit** (8/8), and **Quantum Tunnelling** (7/7 — closed-form T == transfer-matrix solve 1.9e−14, R+T=1, resonant transparency). **STILL AHEAD (this seed stays live):** (a) **more benches per drift** — the menu in [worklog/physics-lab-plan.md](worklog/physics-lab-plan.md) §2 (Newtonian: Three-Body Choreography, Gyroscope, Kepler-law-prover, Ballistics; Einsteinian: Black-Hole Lens, Stellar Collapse, Doppler, Gravitational-wave chirp; **Quantum: a 3rd bench — Particle-in-a-Box**, the natural next one); (b) the Quantum Drift's reveal is currently a PUBLIC in-cave spatial unlock — a future call is whether to *also* feed a hidden Undercroft secret or a Survey "Experimenter" constellation off walking all three drifts (see metagame table). A multi-session wing — extend incrementally like the Hall (9→12). (Brandon's seed; fleshed → opened → growing 2026-06-13.)
...
| **The Physics Lab's Quantum Drift** | **OPEN & growing — 2 benches** (Double Slit + Quantum Tunnelling), 2026-06-13 | Implemented as a **public, in-page spatial reveal** (not an Undercroft secret): `walkedBothDrifts()` in `cavern/index.html` reads the `ws:seen:<id>` breadcrumbs and reveals the violet `#quantumDrift` once ≥1 Newtonian AND ≥1 Einsteinian bench are seen. Grow it: a 3rd Q-bench (Particle-in-a-Box). Open question (a future call, not a mandate): also wire a *hidden* Undercroft secret or a Survey "Experimenter" constellation off walking all three drifts. See `physics-lab-plan.md` §3. |
