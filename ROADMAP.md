...
<!-- gauge:bug:start -->
- [bug] **The manor is overcrowded — its front-gate POI hitboxes now overlap.** Playing the front gate, the manor's markers collide: their clickable hitboxes (dots + leaders + label boxes) sit on top of one another, so a POI is hard to pick and the seat of the estate reads as a cramped smudge. ROOT: the manor DISTRICT is PINNED to the historic shell box (tools/layout/layout.js:73, region {x:586 y:296 w:270 h:208}) and cannot grow, yet it has quietly become the densest precinct on the map — 20 rooms across 9 wings (studies·reckoning·east·archive·barrel-house·kinetics-sound·maker·sewing·arrow) packed into that tiny pinned rect with gutterFor=9. The packer crushes the interior cells until the POI hitboxes overlap. The full-plate crowd is already ACKNOWLEDGED (door.test.cjs "full plate STILL CROWDED 0.939 > 0.3, #103") — the focus-zoom declutter hides it for LABELS, but the POI markers themselves overlap at fit-view. The pinned shell ALSO crowds its neighbors vertically: the Processional court solves to y287, only ~9px above the manor top (y296), and the Undercroft/Reliquary cellar begins y514, only ~10px below the manor bottom (y504). RECOMMENDED FIX DIRECTION (Brandon's steer — treat the numbers as targets, finalize the exact envelope with the LIVE Layout.solve as every wing region was): (1) ENLARGE the manor footprint by claiming the open space WEST of the shell — below x574 the western band is largely candle-pool DECORATION, which is explicitly "no keep-out" (footprints may overlap it; the Deep Hearth already does). Aim for roughly ~3× the current width as a target. (2) REDUCE the height ~10–20% (208 → ~166–187) to relieve the vertical squeeze on the Processional court (north) and the Undercroft/Reliquary cellar (south). (3) Make the SILHOUETTE IRREGULAR — not a true rectangle: read it as a great house — a taller central block (the main house) flanked by LOWER left and right WINGS. Give the manor its own multi-lot partition (central + L/R wing sub-lots, analogous to GROUNDS_WINGS) so the packer spreads the 20 rooms + future growth across distinct sub-regions instead of one dense grid. Net = wider + shorter + irregular: more room to lay out POIs without collision, less vertical crowding, and a footprint that actually reads as a large manor. CONSTRAINTS TO HONOR (the pin exists for these — re-derive + re-verify, don't just widen): the candle-pool (x421 y150 600×600) and the frozen sky/coordinate envelope must stay valid — drawManorInterior (index.src.html:3427) and the candle-glow pooling read the SOLVED manor rect, so they follow the new footprint automatically, but the sky FOOTPRINTS mirror must be re-emitted (tools/layout/emit-mirror.cjs) and sky.test.cjs kept green in the SAME commit. Keep every footprint inside the star-clear FIELD (x162 y150 1116×668) and mutually DISJOINT from Processions, the Undercroft/Reliquary cellar, the Deep Hearth and every other wing (assertGroundsWingsDisjoint). Re-run and keep green: tools/layout/smoke.cjs, tools/layout/legibility.test.cjs, tools/layout/door.test.cjs (17/17), tools/sky/sky.test.cjs, and forge --check. If the whole swing (region + partition + irregular drawer + sky re-derive) is too big for one cycle, PASS THE BATON or decompose into required-fix seeds — but keep the manor a single coherent enlarged silhouette, not a patch that only nudges a couple of coordinates.
