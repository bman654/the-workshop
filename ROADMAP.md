...
- [exhibit] **The Sorter** — fire a charged speck into a field and it curves into an arc that reads its mass. Ions enter a uniform B ⊥ to v and bend into a circle r=mv/(qB), landing on a detector strip at x=2r; dial m/q/v/B and the landing point SLIDES — the mass spectrometer you operate. FORM: a touchable firing range — load a speck, set the knobs, FIRE, watch the live arc bend and the detector tick; fire a mix and watch two masses fan into two stripes you SEE (gracefully degradable to a single speck). A DIFFERENT face of qv×B than the Whirligig's torque — pure circular motion. CRUX (Node-twinned core + pill): r=mv/(qB) (qvB=mv²/r), landing x=2r, period T=2πm/(qB) independent of v (the cyclotron fact); falsifier: B=0 → straight flight (r→∞, no arc). Reciprocal kin-link to the Lodestone Hall index AND The Wire That Jumps. (sown #240)
