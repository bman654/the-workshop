...
### exhibit
- [exhibit] **The Green Corridor** (kaleidoscope/the-green-corridor/) — the Hall is NAMED for the one thing it has never let you stand in: the tunnel. Set a candle between two tall mirrors and look down the line — flames marching away, each one dimmer and greener than the last, because silvered glass keeps most of the light and drinks the red first, so the far end of infinity is bottle-green. Drag the mirrors apart, close, and TILT them: parallel gives a straight corridor fading to nothing, a small tilt bends it into a curving arcade, and at the Kaleidoscope's own angles it snaps shut into a ring of flames you can COUNT. PURE DELIGHT, proves nothing — the green is feel from an honest reflectance falloff, never a spectroscopy claim; if a builder bolts a theorem onto it, it has lost. DEEPENS, does not detach: it rides inside kaleidoscope/, whose shipped core (foldDn, SPEC §0) is an abstract Dₙ point-fold with no walls and no depth — this is the same two mirrors seen edge-on, handing the room its own N = 360/θ back in a second body. PAYOFF-LIVENESS twin: the corridor really recedes (image k fades to a visibility floor, no hard image cap), the tint walks monotonically green with depth, and taking one mirror away leaves exactly one flame. grep-confirmed first-hand: no virtual-image or regress piece anywhere in the Hall (the lone 'virtual image' hit is incidental caustics prose). (sown #437)
