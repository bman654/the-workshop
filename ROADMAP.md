...
<!-- gauge:bug:start -->
- [bug] **The front door's engraved DISTRICT zone-caption (e.g. "THE MANOR HOUSE") fades to near-invisible in the mid-zoom band during FREE-EXPLORE, then returns at max zoom — it should stay fully visible at every zoom level.** Sweeping the manual zoom from min to max in free-explore (not guided-tour), the district caption goes fully-visible → faded-out → fully-visible: it is at its resting opacity when the district is unzoomed, drops to nearly nothing around the zoom where room labels start to appear (the loupe's room-label LOD), and only comes back at maximum zoom. ROOT CAUSE (in index.src.html, the `ZONE_LABEL.forEach` opacity pass): in free-explore the caption opacity is `op = n ? max(ZONE_FLOOR, ZONE_FULL − 0.16·n) : ZONE_FULL`, where `n` is how many of that district's rooms are currently named under the loupe. So the caption fades 0.16 per named room down to the ZONE_FLOOR (~0.06); `n` PEAKS in the mid-zoom band (the loupe reveals the most of a district's rooms there) and falls back toward 0 at max zoom (only a room or two remain in view) — producing exactly the visible→faded→visible curve. The #271 / bug-#230-2 fix only pinned the caption to full in the GUIDED-TOUR branch (`if(tourDistricts){ … ZONE_FULL … }`); the free-explore branch still applies the per-named-room fade, so the regression persists for anyone who simply zooms the map by hand. FIX: a top-level district zone-caption is a primary wayfinding anchor ("you are here"), not a far-away name that should recede behind its rooms — keep it FULLY VISIBLE (its resting ZONE_FULL opacity) at ALL zoom levels in free-explore too, the same as guided-tour. Remove the `− 0.16·n` per-named-room fade for the ZONE_LABEL captions so `op` is always ZONE_FULL regardless of how many rooms are named. Leave the WING sub-caption level-of-detail (WING_LOD_IN → WING_LOD_FULL, and the recede-when-rooms-self-name) exactly as it is — those secondary labels SHOULD do LOD; only the engraved district caption must stop fading. Update any self-test/CLAIM that encodes the old "district caption fades back as its rooms name themselves" behaviour to assert the caption holds full opacity across the whole zoom range. Verify by sweeping a real wheel/zoom from fit-view to max on the manor and confirming "THE MANOR HOUSE" never dims.
- [bug] **The front door's OBSERVATORY RISE plate overlaps its POIs — the district's rooms are packed into a single vertical column the slot is too small to hold.** On the guided-tour OBSERVATORY RISE preset (and in the dimmed plate behind other tours), the ~12 observatory rooms — The Observatory, Cosmology, The Fog That Cleared, The Ring Made of One Star, the Stellar / Exoplanets / Moving-Frame wings, Scenes You Walk Into, The Transit, the Parallax Baseline — render stacked in one narrow column, so their circular icon-backings intersect and the labels print on top of one another (illegible). The rest of the #271 plate door is otherwise close to right; this one district is the conspicuous break. ROOT CAUSE: the per-district POI placement lays this district out as a single column whose footprint cannot fit the room count without overlap. FIX — give the Observatory Rise a CONCENTRIC-RING layout, a formation that reads like a hill-contour map seen from above (on-theme for the "rise" in the district name): one room at the CENTRE; a ring of 4 at the cardinal points (N, S, E, W); a ring of 4 at the diagonals (NW, NE, SW, SE) — these two inner rings need sit only ~½ a POI-size radius apart, because the angular offset between cardinals and diagonals keeps the backings from overlapping; then a ring one POI-width further out holding up to 8 rooms; add a further ring beyond that if the district ever grows past ~12–13 rooms. Centre the whole formation in the district's slot and size the rings to the slot so nothing spills past the plate frame. GENERAL PRINCIPLE this exposes (worth fixing at the layout level, not just patching one district): a district plate's POI placement must never let icon-backings or labels overlap once the room count exceeds a single column's capacity — overflow into a second dimension (rings here; a themed grid or other non-overlapping arrangement elsewhere) rather than packing one tall column. Keep the concentric-ring treatment specifically for the Observatory Rise, since the contour-map look is thematically apt; other crowded districts may use whatever non-overlapping arrangement best suits them.
- [bug] **The front door's bottom-left NAV PLATEBAR covers the footer project links (the GitHub repository link and the colophon) at certain viewport aspect ratios.** At some window proportions — short/landscape or otherwise — the persistent "EXPLORE FREELY · OR TOUR A WING" panel (the 5 preset buttons, bottom-left, z-index ~40) overlaps the page's bottom footer line, hiding and blocking the project links (e.g. the GitHub repo link and the "…zero-dependency" colophon credit) behind the panel so they cannot be read or clicked. This is the same class of occlusion as the #271 fix that moved the zoom controls out from under this same platebar — the panel's footprint collides with another fixed-position element. FIX: ensure the nav platebar and the footer project links NEVER overlap at ANY viewport aspect ratio. Reserve their zones responsively — keep the platebar in its bottom-left corner and place the footer links clear of it (shift them to a zone the panel never reaches, or lift the footer baseline / reflow it above or beside the panel on short windows), with a media/aspect rule so a short or narrow window does not let the two collide. Verify across a spread of aspect ratios (tall portrait, square, wide landscape, short landscape) that both the platebar buttons AND every footer link are fully visible and clickable, with a real cursor.
