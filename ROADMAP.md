...
### exhibit
- [exhibit] **The Selection Jar** — Conservatory. A jar of beetles with a heritable SHADE on a background you tint; each generation a predator eats the most-visible, survivors breed (offspring shade = parent ± variance), and the population mean CRAWLS toward the background — a creature you watch EVOLVE, not an allele table (distinct from the Gene Jar's drift). Grows the wing's missing selection-on-a-visible-phenotype. CRUX: response R = h²·S (breeder's equation — mean-trait shift per generation ≈ heritability × selection differential), asserted on the ensemble mean over many generations within tolerance, NOT a single noisy gen. NEG-CONTROL: set h²=0 (offspring shade independent of parent) and the mean does NOT track despite identical culling; turn selection OFF (random predation) and it only drifts by sampling noise, never directionally. Grep-confirmed ZERO breeder/selection hits in conservatory/. (sown #296)
...
### bench
- [bench] **The Counting-Out Ring** — Numbers Room. A ring of N lit seats; a rhyme counts every k-th seat out (k=2 default) until ONE survives — you WIN by claiming the surviving seat BEFORE the count, then watch the doomed seats dim one-by-one around the circle. The math IS the win: survivor = Josephus J(N,k); for k=2 it's N's binary with the leading 1 rotated to the back (41=101001→010011=seat 19), falling out of the elimination, never read off a dial. A CIRCLE you survive, NOT a residue-clock — re-souls the exact modular vein the 3 decayed clock-seeds kept botching. CRUX: live order ⇒ J(1,k)=0; J(i)=(J(i−1)+k) mod i over N≤200, k 2..6; k=2 ⇒ 2L+1 (N=2^m+L). NEG-CONTROL: claiming J+1 dies; a (J+k) mod (i+1) fence-post slip mis-calls N=5,k=2 (says 5, truth 3). Honest scope: the binary closed form is k=2 only; general k is a live recurrence. Grep-confirmed ZERO 'Josephus' hits estate-wide. (sown #296)
