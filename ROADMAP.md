...
- [exhibit] **The Pond You Can Fish Empty — steer a stock to its sustainable edge** — A pond of swimming fish breeding toward carrying capacity K; one HARVEST dial sets your catch rate and you watch the school thin or thrive in real time — net too hard and the pond goes EMPTY and stays empty (collapse), net at the sweet spot and it holds a steady haul forever. FORM that's ALIVE, not a yield curve. Crux: stable equilibrium N*(h) tracks the closed-form logistic-with-harvest root to <1e-9; sustained yield MAXIMISED at N*=K/2; h>h_crit=rK/4 drives N→0 while h just below holds N>0 forever; NEG-CONTROL zero-harvest rides to K byte-matching conservatory/logistic/core.mjs. Grep-confirmed gap: conservatory has growth, zero harvest. Grows the Conservatory. (sown #261)
...
### bench
- [bench] **The LC Tank — charge and current trading places forever** — A capacitor and coil in one edge-on loop: charge the plates, let go, and energy SLOSHES — blue plate-charge empties into amber coil-current B-glow and back, a pendulum made of electricity; two energy bars (½CV²⇄½LI²) seesaw to a constant sum, ring rate f=1/(2π√(LC)) read live off the L,C dials. FORM: the swap of two glowing reservoirs, not a scope trace. Crux: lossless energy conserved <1e-9 over a period, period=2π√(LC) exactly; a resistance NEG-CONTROL damps the bars, twin asserts the lossless case never decays. Lodestone Hall's changelog names LC tank as the EM wing's unbuilt OSCILLATOR; lives UNDER the Hall (no front-door footprint, like the-transformer). (sown #261)
