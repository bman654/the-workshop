...
<!-- gauge:bug:start -->
- [bug] **Front door — fit-view: per-room labels render at rest that shouldn't, and some are occluded.** The #212 fix is supposed to draw ZERO per-room labels at the resting fit-view (only the always-on district captions), yet several room labels still render at fit-view — e.g. the Observatory Rise sub-rooms (The Moving Frame, Scenes You Walk Into, The Stellar Wing) and Manor House rooms (East Wing, The Arrow Wing, The Sewing Room). They likely should NOT be visible at rest because they are not district names. Separately, other labels that do appear are covered by building footprints (e.g. the Living-Systems / cavern label sits under its own footprint). The resting fit-view is not the clean district-only state the fix claims.
- [bug] **Front door — hover reveals too many labels at once, and subtitles are near-black.** Hovering one room shows that room's label + info panel AND the labels of unrelated nearby rooms (e.g. hovering The Print Room also lights The Aerodrome, The Reckoning Cabinet, The Differential Gear, The Museum, The Engine Room) — it becomes confusing which label belongs to the room actually under the cursor. Also the room subtitles (e.g. "Compositor · Type Press") render in black, near-invisible against the dark map.
- [bug] **Front door — the hover info panel covers the room's own revealed label.** Hovering a room reveals BOTH its label and its info panel, but the panel is placed on top of the label so only one is readable at a time (e.g. hovering Threshold, the info panel covers the "The East Wing · Threshold" label). The info-panel placement should be routed away from the label that the same hover action reveals.
- [bug] **Front door — zoomed-in labels are oversized because they're drawn at fit-view scale.** When you zoom in, the view becomes crowded / hard to navigate again: the labels are rendered at fit-view scale instead of the zoomed-in scale, so they are far too large for the zoomed view.
- [bug] **Front door — zoom does not auto-reveal labels by LOD; it still requires hover.** When zoomed in, labels still require a mouse action (hovering over a room) to appear, instead of automatically fading in based on zoom level (level-of-detail) the way a standard map does.
- [bug] **Front door — dragging to pan selects the map's text.** Panning the view with the mouse selects text. Add user-select:none to the front door.
...
### exhibit
- [exhibit] **The Poisoned Bar — the win nobody can show you** — Numbers Room (chomp via tools/game/games/chomp.js + the shared Adversary engine, the Hexapawn precedent) — the impartial poset game (the arcade 'chomp' is a Pac-Man maze, proves nothing). FORM you BITE: a grid of brass chocolate squares, top-left a black POISON; TAP a square and eat it plus the whole down-right rectangle; forcing the opponent to eat the poison WINS, you play the perfect engine. The estate's RAREST idea: by strategy-stealing the first player is PROVEN to win every non-square board — yet the proof names a winner WITHOUT a winning move (non-constructive). The page makes that visible: a 'reveal the winner' lamp lights P1-WINS from solve(), but 'reveal the move' can only ever show ALL winning replies the search found, never a from-the-theorem formula. CRUX (chomp.js + Node twin): canonical staircase positions are tiny (4×6 ≈ 210 nodes, far under HARD_CAP) so solve() is exact; literatureValue 'WIN' for every non-1×1 board; a literatureBattery re-derives the strategy-steal (the top-right bite as steal witness) over a board-shape sweep. NEG-CONTROL: the 1×1 board (poison only) is the lone first-player LOSS; a single-row board collapses Chomp to one-heap Nim (tying back to Stone Heaps). BUILD CARE: key() must canonicalize the square board's transpose symmetry; keep adversary.js untouched, add only the def. (sown #214)
...
### bench
- [bench] **The Brass Whirligig — the current that pushes back** — Lodestone Hall (lodestone-hall/the-whirligig/) — the EM vein's MOTOR, reciprocal of the #211 induction bench (motion→current); here CURRENT→MOTION. FORM you SPIN: a wire loop on an axle between horseshoe-magnet poles; push a current dial → F=I·L×B on the two long sides gives a TORQUE and the loop turns (live torque-arrows + angle needle). The catch a visitor feels: plain DC dies at vertical and gets shoved back, so you FLIP the commutator toggle and it spins one way forever (the real motor trick). CRUX (the-whirligig/core.mjs reusing lodestone COIL/SCENE conventions + Node twin, page-inline byte-identical): τ(θ)=N·I·A·B·sinθ DERIVED by summing F=I L×B over the four sides, ===closed form to <1e-9 over θ+current sweeps; work/rev WITH commutator >0 and ===∮τ dθ to <1e-9; reverse B → τ sign flips exact. NEG-CONTROL (RED): commutator OFF → ∮τ dθ===0 exactly, the loop only rocks and nets zero work — the commutator, not the field, makes it a motor. No motor/torque bench exists anywhere (grep-confirmed); nests under lodestone-hall/, NOT a new front-door room. (sown #214)
