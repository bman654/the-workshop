...
<!-- gauge:bug:start -->
- [bug] **The descended child map's leader lines miss their tiles — each leader anchors on the relay SLOT box, not the relaid footprint (a regression from the child-label re-solve).** The label re-solve (childRelabel → applyPlacement, index.src.html ~L4635/4659) redraws each child leader from `relayView(meta, rf)` (~L4579), which forces `footprint:"box"` at the relay-foot box's own w/h (`cl.foot[id]`, a wide fan CELL). But relayChild draws the TILE at its CANONICAL footprint size, merely translated into that cell's top-left (`translate(rf.x−cx0, rf.y−cy0)`, ~L4499/4543 — a move, no scale). So the leader points at the CELL's edges while the smaller tile sits in the cell's upper-left. Measured (real browser, descended, framed scale): the offset is systematic and column-keyed — left-column leader foot-dots land (−7,+16) from the tile centre, right-column (+53,+16) (the cell's right edge, well past the tile); the page immediately before the re-solve has the leaders ATTACHED (mean offset ≈ (2,−1)) because it rigid-translated each label WITH its tile so the leader was unchanged. THE FIX: anchor each child leader's footprint box (the box handed to applyPlacement / footBBox) on the TILE's TRUE rendered footprint — the canonical foot box/type translated by the relay delta ({x:rf.x, y:rf.y, w:canonical-w, h:canonical-h} for box rooms; centre+r for tower rooms) — NOT the relay cell's w/h. VERIFY with REAL input (agent-browser click, not dispatchEvent): descend, and every leader's foot-dot lands ON its tile across BOTH fan columns (offset ≈ 0); the label END is unchanged; ascend leaves the canonical estate map untouched; the door pill is unaffected.
- [bug] **The front door's legibility pill flips ✗16/17 ⇄ ✓17/17 on WINDOW SHAPE alone — CLAIM C′'s tier-1 survivor count is screen-space dependent.** Resizing the window flips the door self-test red↔green with nothing else changed; the failing line is always CLAIM C′ ("the tour declutter is non-trivial — keeps ≥60% of tier-1 anchors"). Measured across viewports (fresh load each): 26/39 survivors (67%, green) at common aspects (~1.33–1.78, square, portrait), but 23/39 (~1100×950) and 22/39 (~2000×700, and tall high-DPI windows) = 56–59%, below the 60% gate → red. C′ counts how many tier-1 anchors survive the tour declutter, a SCREEN-SPACE pass (a pixel MIN_SCREEN_GAP) over the rendered scale; that scale is the fit of the fixed #sheet viewBox into the window, so the survivor count tracks the window's shape. The canonical SOLVED map, the layout, and the detached-child foot are all viewport-invariant — only this declutter step varies — and the best achievable count (26/39) clears the gate by only ~2 anchors, so a shape that costs two screen-space survivors tips an otherwise-sound door red. This is NOT from the recent child-map label work: the page immediately before that change gives byte-identical results at every viewport; C′ has behaved this way since the detach/childFoot machinery first introduced it. What CLAIM C′ ought to measure — and whether a window-shape-dependent ✗ is a fault to mend or the conscience faithfully reporting a tighter layer at that shape — is left OPEN: this entry is the faithful observation + repro + root cause, the verdict and remedy are the conscience's to make.
...
### exhibit
- [exhibit] **The Fractionating Column — boil a mixture clean, you never drew a curve.** The Alchemy Lab's 8th bench, under the existing roof (no front-door footprint): drag the flame up under a reboiler of 50/50 Light/Heavy (stated b.p.) and vapor climbs a glass stack of bubble-cap plates, each plate one separation step, so a STAIRCASE of two-colour composition bars drifts toward pure-light up the column (McCabe–Thiele walked up real plates, not plotted) and a receiver fills with near-pure distillate — too low never fills, too high floods back to 50/50. CRUX (core.mjs, machine-ε): the constant-α relation y=αx/(1+(α−1)x) stepped plate-by-plate must match Fenske's closed form at total reflux to <1e-9 over an α,N sweep; NEG-CONTROL — same b.p. ⇒ α=1 ⇒ y=x ⇒ the staircase collapses, the dead apparatus that proves the mechanism. Sibling to Le Chatelier's Vise (flame-driven vapor equilibrium); grep-clean of distillation/Fenske/McCabe estate-wide, distinct from Balance #41 & Titration #46. (sown #383)
...
<!-- ✝ BLOOMED #381: The Tippe Top — a top that flips over and stands on its stem, lifting i… → tippe-top/ · after c759a1b -->
