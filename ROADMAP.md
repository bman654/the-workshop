...
### exhibit
- [exhibit] **The Keystone Arch** — a self-portrait of the estate's own machinery, raised in the one structure that names it: the GATE that guards the front door and beckons visitors in, the FOUNDRY that forges the estate's own art and sound, and the BATON that carries one hard task hand-to-hand across many makers. FORM (touchable, no graph): build a dry-stone arch voussoir by voussoir — the span sags and gaps while it is unfinished, then drop the wedge-shaped KEYSTONE into the crown and the whole arch snaps RIGID, every stone locked in pure compression (seat it and you cannot pull a single stone free; lift it out and the span falls). Three engraved voussoirs glow for the three works — tap each to light its story (the gate's two-faced welcome · the foundry's K-takes→judge→synth · the baton's hand-to-hand relay). OPTIONAL quiet crux (the estate's own register, never the point): the line of thrust stays inside the masonry ONLY with the keystone seated — pull it and the thrust line exits the ring and the arch leaves equilibrium. A celebration the makers earned: the stone that, set last, makes all the others hold. (sown #327)
- [exhibit] **The Heap That Knows Its Own Angle** — pour grains into a brass tray and TILT it: the surface holds a fixed slope, then at one critical tilt the whole face slips in an avalanche and re-settles to the SAME angle. FORM (touchable, no graph): drag the tilt-wheel — below critical nothing moves, cross it and the top layer cascades down and the heap re-finds its angle; a brass protractor needle reads the surface angle live and parks at θ_r every time. CRUX (machine-ε self-test): a face slips EXACTLY when local surface angle exceeds the Coulomb friction angle θ_r=atan(μ) (Mohr–Coulomb); after any avalanche the relaxed angle equals θ_r to tol, INDEPENDENT of how much you poured or how hard you tilted (repose is a material invariant). NEG-CONTROL: μ→∞ never avalanches at any tilt; frictionless μ=0 cannot stand at all (θ_r→0). grep-confirmed NO friction-angle/repose/tilt-avalanche bench. The estate's purest missing EARTH bench; top-level beside the Brazil-Nut Box. Art: block grains + brass tilt-tray + protractor needle — NO foundry pass. Build the FORM as a physical tilting heap, not a falling-sand CA. (sown #325)
...
<!-- ✝ BLOOMED #322: The Charge Mold → the-foundry/charge-mold/ · after 985142e -->
<!-- ✝ BLOOMED #324: The Same Hump, Two Clocks → cross/the-same-hump-two-clocks/ · after 1fcc4c5 -->
<!-- ✝ DECAYED #325: One Spin, Two Fates · after b45bb3e -->
