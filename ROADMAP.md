...
### exhibit
- [exhibit] **The Spinning Chair — pull your arms in, spin faster** — WING dynamics. The estate has NO rigid-body L=Iω piece (precession is orbital point-mass h; the Rotor is centripetal friction — grep-confirmed distinct). Sit a rider on a free brass pivot holding two weights; ONE slider DRAWS THE WEIGHTS IN (radius a→b) and the chair SPEEDS UP on its own, no push — frictionless L=Iω fixed while I shrinks, felt as the body flung harder at small radius. CRUX (twin): point masses on massless arms I(r)=I₀+2m·r², L=I·ω invariant to <1e-9 across the pull, spin-up ratio EXACTLY I(a)/I(b); rotational KE rises by the exact work the arms do (½L²(1/I_b−1/I_a)>0). NEG-CONTROL: clamp the chair to a driven motor → pulling in changes nothing, L not conserved. State the point-mass honesty bound. (sown #254)
...
- [exhibit] **The Circle That Rolls Itself Straight — the Tusi couple** — WING spirograph/ bench. An exact straight line from pure rotation in a NEW form (a rolling disk, not the Straightedge's rod-linkage). A disk rolls without slipping INSIDE a ring of exactly TWICE its radius; pin a rim-point and CRANK — it rides a dead-straight diameter, the hypotrochoid degenerated to a line; drag the ratio off 2:1 and the line visibly OPENS into an ellipse. spirograph/ only ever self-tested d=0→circle and d=1→cusps, NEVER the R=2r line (grep-confirmed; share its no-slip core if clean). CRUX (twin): R=2r → best-line max-perp-dev <1e-12 over the full crank; ratio≠2 → a MEASURABLE non-zero deviation (an ellipse of nonzero minor axis) — the load-bearing neg-control. (sown #254)
