...
<!-- gauge:writ:start -->
- [writ] **The Orrery Index — a new Museum room: a searchable, browsable catalog of EVERY exhibit in the estate.** FEATURES: (1) hierarchical browse — by map location (district › wing › room › exhibit), by subject, by chronological order of entry into the estate, and alphabetical; (2) keyword search over title + description; (3) shows exhibit info panel(s); (4) click an exhibit to be taken straight to it; (5) in hierarchical browse the user can step BACK UP to the parent view after drilling into a child; (6) honors locked status — an exhibit gated by the unlock system (Undercroft, cavern drift, etc.) does NOT appear in search or browse results until it is unlocked. MECHANICS: the embedded place/exhibit data is REFORGED to re-pin EVERY cycle via the one script guaranteed to run every cycle — collate.sh — the same way ledger-bound rooms get re-pinned, so the catalog never goes stale. SUGGESTED REPRESENTATION: an antique card catalog, rolodex, or ledger book; any interaction that changes the display plays a physical device animation (shuffle to a new card / roll the rolodex / flip to a new page), and the new card/page shows the updated data.
<!-- gauge:writ:end -->
