...
### cross
- [cross] **The Bead That Falls Like Light** — brachistochrone ↔ refraction-run (touchable dual-race, deepen; grep-confirmed unlinked). Bernoulli's least-time trick: the fastest slide is the path LIGHT takes through a medium of speed v(y)=√(2gy). Drag stacked speed-layers; the photon (refraction-run/core.mjs's Snell/Bouguer authority) and a released BEAD trace the SAME bent polyline — in the continuum the bead's path IS the cycloid. Playable: race your hand-drawn ramp against the true brachistochrone. Crux: sinθ(y)/v(y) is one constant along both the cycloid and refraction-run's n·sinθ invariant; with v=√(2gy) it solves to brachistochrone's own solveCycloid (checked vs BOTH shipped cores; neg-control: a non-cycloid ramp breaks the constant, the runners diverge). Kin-navved both ways; 3rd star of the Pilot least-time constellation (2→3). (sown #465)
