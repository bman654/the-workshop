...
<!-- gauge:bug:start -->
- [bug] **The Fairground Gate detach layer (#369) passed synthetic/modeled self-tests but is broken in the real browser — 3 coupled symptoms, one through-line.** The estate's first detach-into-depth layer (eee0133, `the-fairground-gate`) shipped green yet is wrong in the live page: (1) CLICK — the lit in-map GATE FACE that should descend into the child wing does NOT respond to a REAL pointer click; (2) LAYOUT — its big arch is drawn OVER the AMUSEMENTS exhibit column (a visual mess; the detach layer was meant to RELIEVE crowding, not stack an arch on the tiles); (3) SELF-TEST — the live `#doortest` pill reads RED 16/17, yet the node twin (`tools/layout/door.test.cjs`) reports GREEN 17/17. THE THROUGH-LINE (the important part): everything that PASSED is SYNTHETIC or MODELED; everything that's BROKEN is REAL/RENDERED. For (1): a synthetic `.click()`/dispatch on the gate `<g>` fires the handler, but a real pointer click misses — SVG only hit-tests PAINTED pixels and `.gate-face` is a `<g>` of thin strokes with no filled hit area, so the open arch interior isn't clickable; add a transparent hit-rect or `pointer-events:bounding-box`/`all` over the whole interactive zone (and check Z-ORDER too: an exhibit tile / platebar / labelgroup painted above may be swallowing the event — precedent in index.src.html: the platebar "used to OVERLAP and swallow clicks"). For (3): the node twin passes 17/17 because it does NOT model the new gate geometry, so it is BLIND to the overlap the LIVE pill catches over real getBBox — the live red is FAITHFUL (it is catching symptom 2; read the live pill to see which claim, likely CLAIM B "zero rendered overlaps" or CLAIM A "resting layer legible"). FIX ALL THREE: a real click anywhere on the lit gate face descends into the child wing; lift the arch CLEAR of the AMUSEMENTS tiles so the live pill returns to 17/17 green; and EXTEND the twin to model the detach-layer/gate elements so node-side verification reproduces the live pill (no more model-green-while-live-red). VERIFY in a REAL browser (agent-browser): a real hit-tested click on the gate descends, and the live `#doortest` pill reads green — NOT via a synthetic dispatch or the modeled twin alone. Same blind spot as the #337 door bug, now twice over.
