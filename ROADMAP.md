...
- [engine] **The Toy Theatre — the Shadow Theater learns to PLAY a play** — tailored from the ⚡ 'make it a playhouse' spark: shadow-theater/ is a lovely SANDBOX that never performs on its own (proscenium.js + puppets.js BUILT, penumbra faked at index.html:1245) — the soul it lacks is a curtain going up. Forge a reusable cue-timeline STAGING [engine]: a play is DATA (ordered {at, do:<Stage-seam entry call>} cues); a director advances a clock, fires cues in order, and the existing critically-damped springs ease every target into one flowing gesture — the company/playbill grow by adding data, not code. FORM: a paper PLAYBILL pinned to the proscenium — tap a title and the stage stages it (puppets glide, lamp dollies itself, the backdrop washes dawn→dusk via a new drawBackdrop(phase), foreground FLATS slide in on grooves as offstage cutouts in the existing Puppets contour system, cues fire to a closing TABLEAU, curtain falls; intermission returns to free-play — grab a puppet mid-scene and the play absorbs it). DEEPEN not detach: gathers WITHIN shadow-theater/magic-lantern, NO new front door. Right-size: engine + 3 short wordless plays (crane-takes-the-moon / fox-at-the-reed-bank / willow's-secret) + 2 proscenium verbs. PURE DELIGHT, no theorem. Payoff-LIVENESS twin (headless, not a theorem): every play PLAYS — cue-index advances monotone through ALL cues, each cue's effect lands (getState/measureCast at boundaries), the closing tableau + curtain-fall flag are reached; green desktop+mobile. (sown #472 · contest #44)
...
- [rep] **The Maker's Shed rep** — a low sawhorse-trestle workbench, a vise clamped at one end, a hand-plane, saw and mallet across the top, curls of shaving beneath · aspect:horizontal · room:workbench · accent:#b07a3c — 2nd-oldest un-repped room (entry 164, outbuilding); the estate's literal maker-table and a textbook wide-low silhouette. (sown #469 · contest #12)
- [rep] **The Hall of Mirrors rep** — a tall gilt-framed standing mirror holding an infinite receding corridor of ever-smaller, ever-dimmer frames, one cool glint travelling to the vanishing point · aspect:vertical · room:hall-of-mirrors · accent:#8fd9ff — 3rd-oldest un-repped (entry 227, opticks); the most alluring name in the backlog, an unmistakable tall silhouette; slab-accent blue adds cool relief to a gold-heavy shelf. (sown #469 · contest #12)
...
### exhibit
- [exhibit] **The Game That Can't Be Drawn (Sim)** — a proof-register arcade game (deepen via tools/game/adversary.js, never fork it): six brass studs in a ring; you and the machine alternately claim a chord in YOUR colour, and whoever is forced to CLOSE a same-colour triangle LOSES — it can NEVER draw, because R(3,3)=6 means every 2-colouring of K6's 15 edges hides a mono triangle (second-player solved). CRUX: a headless twin proves all 2^15 K6 colourings contain a mono triangle (0 draw-boards) AND the solver returns a P2-win; NEG-CONTROL — K5 has a triangle-free colouring, so the 5-stud version CAN draw (6 is the exact threshold). Payoff-liveness: the closing triangle flashes and the LOSS reveal fires. Deepen the arcade. (sown #474)
- [exhibit] **The Fortune-Teller** — a nostalgic folk-ORACLE you fold and operate (NOT a perception guess-game): a real paper cootie-catcher — pick a colour, pinch it open/shut spelling the word, lift a numbered flap for your fortune; write your OWN four fortunes and KEEP the paper (reload-restores, Raked-Garden style). Kin to Kirigami's folded paper + the verse oracle's voice — DEEPENS a two-toy folded-paper pair beside Kirigami, no lone front door. Claim-free delight, no crux. Payoff-liveness twin: the pinch/spell count deterministically selects the lifted flap → its authored fortune, and save→reload restores the exact custom fortunes — the payoff FIRES. (sown #474)
...
### cross
- [cross] **The Gas That Pushes Back (PV=NkT — the M-B gas grows a piston)** — the ripest named cross (PROJECT-STATUS flags M-B × Engine Room as THE next): put a draggable PISTON on the Cavern's Maxwell-Boltzmann hard-disc gas — squeeze it and watch the SAME visible discs hammer the wall harder, so macroscopic PV=NkT emerges from the collisions the engines only ever assumed. Halve V → P doubles (isotherm); heat the walls → P climbs at fixed V. CRUX: time-averaged wall-impulse pressure (real disc-wall momentum tally) == NkT/V to statistical tolerance; PV=const on an isotherm; P∝T at fixed V — a headless twin samples the impulse stream and asserts the lock. Imports the M-B core BYTE-IDENTICAL (no re-derivation; add only piston + tally); makes the two thermodynamics wings shake hands. Deepen both roofs. (sown #474)
...
<!-- ✝ BLOOMED #470: As Hangs the Chain, So Stands the Arch → hung-line/ · after 57b1890b -->
<!-- ✝ DECAYED #474: The Mensuration Barrel · after 69e0af43 -->
<!-- ✝ DECAYED #474: One Nimber, Two Games · after 69e0af43 -->
