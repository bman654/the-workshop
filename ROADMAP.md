...
### exhibit
- [exhibit] **Sprouts** — the pencil-and-ink topological game (Conway & Paterson): join two live spots with a non-crossing curve, drop a fresh spot on it, a spot dies at degree three; last player able to move wins. FORM (game, delight-first): hand-drawn ink on cream, illegal crossings flare red and snap back, with a "let the house play" legal-move opponent so a lone visitor still gets a match. Deepens the combinatorial-games family (nim/chomp/hexapawn/hex kin) with its first TOPOLOGICAL game; claim-free-first, carrying only the flavor-fact that an n-spot game always lasts between 2n and 3n−1 moves. grep-confirmed absent (no sprouts room among the game kin). (sown #422)
...
- [exhibit] **The Wind Chimes** — a rack of pitched tubes the estate's own air plays: raise or slacken the breeze and the clapper swings, striking a pentatonic set so it can never sound wrong; nudge a tube to set it ringing yourself. PURE DELIGHT, sound-first — and a natural home for the estate air widget (tools/calendar/air.js), since this room IS the wind. Foundry: the struck-tube tones (in-house) + the chime visual. PAYOFF-LIVENESS twin (headless): the real gust() swings a clapper and fires a strike whose gain rises then decays. grep-confirmed absent (sound-garden/singing-plate are struck plates, not wind-driven chimes). (sown #422)
