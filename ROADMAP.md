...
### exhibit
- [exhibit] **The Cartesian Diver** — SQUEEZE the bottle and a little glass diver sinks; ease off and it rises — hunt the exact pressure where it HOVERS and feel it refuse to hold (a hair deeper and it runs to the floor). PURE-DELIGHT TOY you press/drag (bobbing diver, bubble-refraction glint, optional muted glug); grep-confirmed GAP — no buoyancy/Archimedes/Boyle room exists (the-aquarium is ambient fish, pool is surface waves; the Trammel & Rayleigh-Bénard only borrow the word). Honest layer, no chart: the neutral point is DYNAMICALLY UNSTABLE — sink a hair → more compression → less lift → sink more — so the hover is a knife-edge you can't keep by hand; the instability IS the play. Liveness twin (headless, has a payoff): past-threshold squeeze ⇒ monotone sink to floor, release ⇒ monotone rise, neutral un-holdable without correction, restored session resumes at correct depth. Gathers as water-kin beside the-aquarium/pool (no new wing over one dot). (sown #444)
- [exhibit] **The Crystal Garden** — drop metal-salt seeds into a jar of water glass; osmotic pressure ruptures a gel membrane and colorful mineral tubes climb and branch — cobalt blue, iron rust, copper green. PURE DELIGHT, no claim: pick the salts, watch a silent garden grow. FORM: a generative living-growth exhibit. DEEPEN (host: The Alchemy Lab) — the 'grow-it-in-a-jar' companion the wing lacks, kin to the Periodic Table (which metal → which color) and The Climbing Ribbon (chromatography). grep-confirmed GAP — no silicate / chemical-garden / precipitation-growth piece anywhere. Liveness twin (claim-free, has a payoff): the tubes actually grow and branch over the visit (height and branch-count strictly increase from a seeded start). (sown #444)
...
### cross
- [cross] **The Spiral Meadow** — three lattice species locked rock-paper-scissors, each eating one neighbor and losing to another: well-mixed they crash to a lone survivor, but on the lattice they self-organize into eternal rotating SPIRAL WAVES and all three live forever. FORM: a living, paintable ecology-sim with a well-mixed/spatial toggle that stages the extinction-vs-forever flip. DEEPEN (host: The Conservatory) — the spatial third that unifies Predator-Prey (2-species population dynamics) and The Replicator (which builds RPS only WELL-MIXED on the 2-simplex and proves it neutrally stable). CRUX (falsifiable): well-mixed → fixation/extinction; spatial lattice → persistent 3-species coexistence, all three surviving long-run (measurable). grep-confirmed unbuilt (no spatial cyclic-dominance / spiral-wave sim anywhere). (sown #444)
