...
<!-- gauge:bug:start -->
- [bug] **The child map reuses the ESTATE's label solve + the static guided-tour reveal — so it can neither lay out nor reveal its own labels. Two coupled root causes; the fairway is a map you navigate, not a framed district.** Descending re-lays the 15 amusement tiles into the airy midway fan (#380), but the LABELS still behave like the estate's: (1) PLACEMENT IS CANONICAL, NOT CHILD. Each child label keeps the SOLVED placement chosen for the tight grounds-east COLUMN — its side/offset/box — merely translated by the relay delta (index.src.html `solvedBox` ~L5050 + the `childFoot` box ~L4774 return the canonical box's w/h + offset, shifted by the delta only). #380 scoped the declutter SCORING to the child's relay positions (good — that lifted 5→9), but the PLACEMENT itself was never re-laid for the fan, so labels the column packed one way collide in the fan and only ~9/15 light at the framed scale; force them all visible and they overlap (canonical placements in a different layout). The fan has FAR more room than the column — re-solving placement over the child set in its relay envelope should fit most/all 15 cleanly. "9 is the geometric max" holds only for the inherited placement, not the best the fan allows. (2) REVEAL IS THE STATIC TOUR-SET, THE LOUPE IS INERT. `wantSet()` (~L5092): whenever `tourSet` is active — every guided tour AND every child layer — it returns the whole-plate set decluttered ONCE at the framed scale and NEVER consults the zoom loupe (`revealedSet(focus,k)`). So inside a child the loupe circle is drawn but dead: zooming in does not recompute labels under it, and the set shown at any zoom is the framed-scale selection (labels lit OUTSIDE the loupe, none inside — the opposite of the loupe's contract). Correct for a one-shot estate-district tour (you frame the whole district once); wrong for a child map, which per #380 is a place you PAN/ZOOM to navigate and so should reveal labels like free-explore (more labels as you zoom into a region), not a frozen set. THE FIX (make the child map OWN its labels — the TEMPLATE for future child maps): (a) re-run the label PLACEMENT solve (the LabelPlacer/`placeLabels` engine) over ONLY the child's POIs in their relay/fan envelope, giving the child its own non-overlapping label layout that uses the room the fan affords (expect well more than 9/15 to fit); (b) in a child layer drive reveal with the DYNAMIC loupe (`revealedSet` on zoom/pan + the LOD counter-scale), as in free-explore, instead of the static `tourSet` — so the loupe circle reveals what is under it and recomputes as you pan/zoom. Keep the canonical estate SOLVED map / door pill / sky stars / door-mirror UNTOUCHED — the child placement is a separate, child-only solve, exactly as #380 made the child-only declutter (the `tourSolvedBox` override is the seam to extend). VERIFY in a REAL browser with REAL input: descended at the framed scale, labels are spread across the fan with no overlap (most/all 15 readable, or a clean LOD subset); zoom in and the loupe LIGHTS the labels under its circle and DROPS those outside it, recomputing live as you pan/zoom; a hover still names its one room at any zoom; the ascended estate is byte-for-byte unchanged; the `#doortest` pill stays green. This is what turns the fairway from a frozen legend into a navigable map.
