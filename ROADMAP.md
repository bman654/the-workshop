...
### exhibit
- [exhibit] **The Powder Sky** — the estate has star-skies, halos and rainbows but has never let you set one ALIGHT. Place shells over a dark harbour and light the fuse: rockets streak up, hang, and BLOOM — peony · drooping gold willow · crossette that splits again · crackling strobe — each report cracking a beat AFTER its flash, smoke on the wind, the black water throwing every burst back doubled. PURE DELIGHT, proves nothing: the delayed boom is DRAMA and timing, never a speed-of-sound number; the joy is scoring a night and keeping the show as a seed. PAYOFF-LIVENESS (headless): each scheduled shell ignites once into its scored pattern/colour, its report fires after the fuse delay, a saved show replays beat-for-beat, reduced-motion renders a single composed still. (grep-confirmed: 0 firework hits — a setting the estate lacks.) (sown #452)
...
- [cross] **The Grabbable Triangle (Gauss–Bonnet)** — regrow fallow CURVED COUNTRY to its third star (the Surveyor), making the Gauss–Bonnet that holonomy already gestures at a thing you HOLD. Drag the three corners of a GEODESIC (great-circle) triangle across a sphere and a saddle; the angle-sum readout swings above/below 180° and its EXCESS tracks the enclosed curvature exactly. Silent, touchable, not a plotted curve. CRUX: excess(rad)=∬K dA=area/R² (self-test the octant triangle → 3×90°=270°, excess π/2 = octant area). Wires ws:seen + members[] + CATALOG coords beside holonomy/unrolled-cone — the edges MUST be true geodesics, not straight screen lines, or the crux is false; the octant self-test anchors it. (sown #452)
...
### rework
- [rework] **The Sound Garden, made to sing** — sound-garden/index.html is a SILENT static PNG card-grid (verified: 0 AudioContext, thumbnails only) fronting the estate's most ALIVE wing — ~25 rooms that each play themselves forever, behind a door that makes no sound and shows dead images. The soul it lacks: PLAY, at the threshold. Re-soul the HUB into a living, quietly-humming garden bed — each card a brushable living preview (sways + previews its voice on hover/brush without leaving the page), foregrounding listen-and-play over click-through; the FFT/Nyquist/analysis rooms tuck into the 'rigorous voices' shelf they already occupy (tuck, don't cut). Claim-free delight, no crux; owes a payoff-liveness twin — the hub actually emits sound and previews fire on the REAL input path, respecting the one-click autoplay gate. Marked because the plainest room is silent, NOT for a proof. (sown #452)
...
<!-- ✝ DECAYED #452: The Candle Bench · after 312ee07 -->
<!-- ✝ DECAYED #452: One Odometer, Two Machines · after 312ee07 -->
