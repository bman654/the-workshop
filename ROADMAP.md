...
### exhibit
- [exhibit] **The Drift Jar — the current that won't drift, finally does** — [Conservatory companion: the Gene Jar ('the current that won't drift', infinite pop) and the Selection Jar ('won't drift; this one does') both NAME drift as what they are NOT — the wing has no drift bench.] FORM (living, touchable, no chart): a FINITE jar of gold/slate beads; each generation resample N with replacement from the current pool (Wright–Fisher) — no selection, no mutation — and the gold fraction RANDOM-WALKS until the jar goes monochrome; a grid of parallel jars shows the fraction fixing GOLD pile up to exactly the starting p. CRUX (machine-ε, EXACT Markov absorption not Monte-Carlo): solve the (N+1)-state tridiagonal Wright–Fisher matrix → fixation prob = initial frequency p to <1e-12 (the martingale E[p_{t+1}|p_t]=p_t); heterozygosity decays H_t=H_0·(1−1/2N)^t (pin the exact per-generation factor). NEG-CONTROL: N→∞ (the Gene Jar's own infinite limit) NEVER fixes — H stays pinned at 2pq forever; drift is purely a finite-N effect. grep-confirmed ZERO wright-fisher/genetic-drift/fixation; cross-link both jars so the three read as one family. Foundry-light (beads + a monochrome-fill jar grid). (sown #341)
- [exhibit] **The Unstamped Bag — why I can't tell dog-bites-man from man-bites-dog** — [Clockwork Automata bench — the wing's first new bench in a long while; the AI-self-fact its 9 benches lack: bare self-attention is order-BLIND.] FORM (touchable, drag-to-reorder tiles, no chart): a row of token tiles; with the position-stamp OFF, scramble them and the attention output + next-word die come back byte-identical up to the same shuffle — the model literally can't tell 'dog bites man' from 'man bites dog'; flip the sinusoidal position-stamp ON (a sin/cos barcode striping each tile) and the same scramble changes everything. CRUX (machine-ε on a tiny HAND-BUILT fixed Q/K/V block): permuting input rows by π permutes softmax(QKᵀ)·V output rows by EXACTLY π (permutation-equivariance <1e-12 over many random π); a mean-pool readout is permutation-INVARIANT (byte-identical). NEG-CONTROL: add the positional encoding and the same π changes the output — the position stamp is the SOLE thing that breaks order-blindness. grep-confirmed ZERO positional-encoding/permutation-equivariance/bag-of-words; distinct from Spotlight/Tokenizer/Context. Reuse the wing's existing softmax-die idiom (Partition / Next Word) for kinship. Foundry-light. (sown #341)
...
- [exhibit] **The Homicidal Chauffeur — Dodge the Automaton** — FORM (played, not graphed): a top-down brass arena — YOU are an agile pedestrian (drag to steer, slow but pivots instantly); a wound automaton-CAR charges at constant speed but can only turn inside a fixed radius (no pivot in place), a capture ring on its nose. WIN by JINKING — sidestep the instant the car commits and it overshoots in a wide arc; flee in a straight line and it runs you down / reach the far gate. A continuous, TWO-SIDED differential-pursuit game — a structurally-opposite form beside the wing's combinatorial boards (nim/chomp/hex/wythoff/toads&frogs), and the two-sided upgrade of the just-decayed one-sided 'Lamplighter's Chase' (supersedes it). CRUX (a SCRIPTED-law twin of the free-play game, both paths in ONE code-disjoint core): model the car as a Dubins vehicle (speed v_p, min-turn-radius R) — pin its optimal route to a fixed target to the analytic Dubins shortest-path length (vs a reference, not the realtime sim), a SCRIPTED radially-fleeing evader is caught in bounded time for v_p/v_e>1, and a SCRIPTED optimal-jink evader survives ≥T. NEG-CONTROL: R→0 (turn-on-a-dime) ⇒ Dubins length collapses to the straight-line distance and NO jink escapes a faster pursuer — the TURNING CONSTRAINT, not raw speed, is the only thing the evader exploits. grep-confirmed ZERO pursuit/chauffeur/Dubins/differential-game. SVG/canvas, foundry-light (a brass clockwork car + heat-tail + capture ring). (sown #341)
...
### cross
- [cross] **The Unrolled Cone — the Surveyor's second star (deficit angle)** — FORM (touchable, no graph): a paper cone you fold and unroll — cut a wedge δ from a flat disc, roll the rest up, and the missing wedge IS all the cone's curvature crammed to a point at the apex while everywhere else stays FLAT (it unrolls without stretching). Drag the wedge to sharpen/fatten; carry a transported arrow around any loop enclosing the tip and it comes home rotated by exactly δ — holonomy with zero smooth curvature on the path. Curved Country's singular-curvature companion to the holonomy room's smooth-sphere Gauss–Bonnet (already built there). CRUX (machine-ε): deficit δ = 2π(1−sin α) for half-angle α, and the holonomy of ANY simple loop around the apex equals δ regardless of loop shape (discrete Gauss–Bonnet ∮κ_g ds + δ = 2π), pinned over a family of loops and α to tol. NEG-CONTROL: a loop NOT enclosing the apex returns the arrow unrotated — all the deficit lives at the one point, nowhere on the flat. grep-confirmed ZERO cone-deficit room (only the sky.js FEATS comment that promises this sibling). Build wires the member into the 'surveyor' FEATS group (sky.js) + a ws:seen crumb so the asterism completes. Enacts the standing 'Gather the singleton constellations' curation. (sown #341)
...
### bench
<!-- ✝ DECAYED #341: The Lamplighter's Chase · after 2b5d03f -->
