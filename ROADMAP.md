...
- [metagame] **The Marquee — the Arcade's house scoreboard** — the Arcade's 23 neon cabinets each keep a high score alone in the dark (grep-confirmed: no marquee/par/champion layer anywhere in arcade/; the Undercroft's Constellation-of-Mastery is SEALED, not arcade-native). FORM = a metagame LAYER, not a chart: a grand illuminated marquee mounts above the rack — each cabinet a BULB that ignites the instant your PERSISTED best crosses a fair HOUSE PAR (a neon flicker + chime), a live 'X of 23 lit' reads across the top, and lighting them all runs a finale sweep + engraves a 'Champion of the House' plate. Lights on MASTERY not per-play — nothing to grind, a house to conquer cabinet by cabinet — and it is the visible ON-RAMP the sealed Mastery trophies have always lacked. DEEPEN not detach: appends to arcade/index.html + a new arcade/marquee/ (marquee.js + an authored key/par table), reads each cabinet's OWN existing best-score localStorage key, touches NO game, mints no new slug or front-door dot. Delight-first, claim-free (the only 'math' is best>=par) — NO theorem; payoff-LIVENESS twin: a seeded best above par reads LIT + the counter increments, all-above fires the finale/Champion plate, a best below par reads UNLIT (the neg-control — the board can't be lit by wishing). BUILD coupling: the high-score keys are INCONSISTENT (ws:best:<slug> vs <slug>_high, e.g. asteroids_high/snake_high) — author the key/par map by READING each cabinet, never by slug convention, read-only on ws:best:* so the hidden trophies stay undisturbed. Foundry: neon bulb dark-glass vs ignited-with-bloom, an ignite flicker + chime and a board-wide finale chase (in-house WebAudio honoring ws:pref:muted), the engraved champion plate. (sown #459 · contest #42)
...
### exhibit
- [exhibit] **The Raked Garden** — a karesansui sand garden as a generative surface you COMB: drag a wide wooden rake and parallel grooves follow your stroke, bending around stones you place and drag into the estate's concentric rings; sweep flat and begin again, or keep a garden that repaints on return. PURE claim-free craft — kin to the Marbling Bath / poster press, no readout, no theorem. Payoff-liveness (its only proof): a kept garden — stones + grooves — restores on reload. grep-confirmed gap (0 karesansui/raked-sand/zen-garden hits); distinct from the strange/sound/elementary gardens. The slate's DELIGHT-FIRST anchor. (sown #458)
- [exhibit] **The Left-Handed Bench** (alchemy · chirality) — the Alchemy wing's FIRST manipulable 3-D object: two ball-and-stick molecules that are perfect mirror images — a chiral pair, like your two hands. Grab one, tumble it every which way; it never lands on its twin. DELIGHT-FIRST — the payoff is the bodily frustration of hands that won't stack. DEEPEN not detach: rides the existing scene3d core (in-the-round/scene.mjs, UNFORKED), one card on alchemy/index beside The Crystal Garden, no new map dot. Light claim + built-in neg-control: best-fit RMSD over ALL rotations stays > ε for the chiral pair, drops to 0 for an achiral control. Keep molecules low-atom greybox; confirm at build it doesn't overlap knot-tabulator's handedness framing. (sown #458)
...
- [exhibit] **A Message, Cast to the Tide** — the estate's first SEA. Stand on a night shore, write a line, cork it in a bottle and THROW: it arcs, plops, bobs on the swell, shrinks to a speck over the horizon. Come back later — real wall-clock minutes or a day — and the tide has carried one IN: weathered, barnacle-flecked, stamped with the days it drifted, your own words returned or a stranger's line in the estate's OWN hand (a baked drift-pool, single-user, no network). PURE DELIGHT / story — kin to the verse oracle and the decayed Candle Bench's real-minutes register. PAYOFF-LIVENESS (headless): a cast persists+timestamps the bottle; advancing the clock lands a return whose weathering matches the elapsed drift; a restored session resumes a voyage mid-drift; a graceful empty-tide state shows before any bottle is due. (grep-confirmed: 0 sea/tide hits; the-aquarium is a sealed deep tank, not a shore.) (sown #452)
...
### cross
- [cross] **The Strobe Mill** — the Sirenist's sky.js comment names a free stroboscope as promised kin (members[]=2); ship the SEEN twin of its two HEARD instruments. A hand-cranked marked disk under a tunable flash: match flash-rate to spin-rate and the wheel FREEZES; nudge it and it crawls forward, then backward (the wagon-wheel effect). Same rate-arithmetic as the Tone Mill, new sense — stillness you SEE, not pitch you hear. DELIGHT-FIRST, playable; new bench under Kinetics & Sound (kin to tone-mill, no fork). Sets ws:seen:the-strobe-mill, joins sirenist members[] (2→3), earns CATALOG coords, re-run derive-sky + bijection. Payoff-liveness: freeze fires when flash-rate = spin-rate; apparent-crawl speed = (flash − spin). (sown #458)
