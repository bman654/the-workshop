...
### exhibit
- [exhibit] **The Sultan's Suitors** — a line of veiled suitors files past your dais ONE at a time; lift each veil (you see only "beats all so far?", never a rank), then marry-or-banish on the spot, no going back. SLIDE a brass "look-don't-pick" band to set how many you let pass before saying yes; the 37% rule surfaces in your own crown-rate, never a curve. The estate's first OPTIMAL-STOPPING piece (grep-confirmed: no secretary problem anywhere). Re-soul guards: keep the procession SLOW + irreversible (the agony is the soul), the cutoff band the PRIMARY control, and "best so far" a RELATIVE comparison. Optional crux (secretary/core.mjs + Node twin): P(k,n)=(k/n)·Σ_{i>k}1/(i−1) maxes at k≈n/e→1/e; enumerate all n! for small n + a k=0 neg-control winning exactly 1/n. (sown #280)
