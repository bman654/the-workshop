# THE TEN-FOLD GLASS — plate `fern` (−1 decade)

**The subject:** a single fern frond, upright, against the dark.

**Its anchor** is `[0.10, -0.22]` — one pinnule near the tip of the frond sits exactly there and grows out of it as
the wheel turns inward. This plate is at 10^−1 m on the ladder; the plate
...
It should look ALIVE — a slight lean, the taper from base to tip, pinnae that get smaller and tighter toward the crown, spore-cases showing on some undersides. At blow-up the leaf's own cellular texture must appear (it is what the next plate down IS). Green kept muted, closer to old botanical-plate green than to a bright leaf.
...
The winner replaces the placeholder `PlateArt.fern` in
`ten-fold/plates.js` (or is installed as `ten-fold/plates/fern.js` and
