# THE TEN-FOLD GLASS — plate `cell` (−5 decade)

**The subject:** one cell of a fern leaf, seen through a good microscope.

**Its anchor** is `[0.14, -0.10]` — one chloroplast among the many adrift in the cell sits exactly there and grows out of it as
the wheel turns inward. This plate is at 10^−5 m on the ladder; the plate
...
A plant cell: the thick wall a soft brick, the vacuole's pressure pushing everything to the edges, chloroplasts crowded on the cytoplasmic stream, one pale nucleus. It should look SEEN rather than diagrammed — uneven, slightly out of focus at the edges, the way a real slide does. Texture: the granular cytoplasm.
...
The winner replaces the placeholder `PlateArt.cell` in
`ten-fold/plates.js` (or is installed as `ten-fold/plates/cell.js` and
