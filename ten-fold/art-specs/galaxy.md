# THE TEN-FOLD GLASS — plate `galaxy` (21 decade)

**The subject:** a barred spiral galaxy, seen face-on and slightly tilted.

**Its anchor** is `[-0.19, -0.14]` — one stretch of one spiral arm sits exactly there and grows out of it as
the wheel turns inward. This plate is at 10^21 m on the ladder; the plate
...
Stipple, not gradient: the arms built of hundreds of thousands of individual points so that magnifying the plate resolves it into stars rather than blurring it. Dust lanes as DARK structure winding inside the arms — those are what read at blow-up. A dense warm core, cool blue in the young arms, a faint halo. The single most-photographed subject in the batch: make it look drawn.
...
```js
PlateArt.galaxy = function (g, u, a, t, anchor) { /* ... */ };
```
...
The winner replaces the placeholder `PlateArt.galaxy` in
`ten-fold/plates.js` (or is installed as `ten-fold/plates/galaxy.js` and
