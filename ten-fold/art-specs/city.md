# THE TEN-FOLD GLASS — plate `city` (4 decade)

**The subject:** a whole city from the air at night.

**Its anchor** is `[-0.12, -0.09]` — one lit street among thousands sits exactly there and grows out of it as
the wheel turns inward. This plate is at 10^4 m on the ladder; the plate
...
The lit grid, warped by geography — a river cutting it, a dark park, the density falling off at the edges into scattered outlying lights. The gold of sodium lamps against a cold dark. At blow-up the individual lights should stipple apart into a field rather than smear. Aerial, not skyline.
...
```js
PlateArt.city = function (g, u, a, t, anchor) { /* ... */ };
```
...
The winner replaces the placeholder `PlateArt.city` in
`ten-fold/plates.js` (or is installed as `ten-fold/plates/city.js` and
