# THE TEN-FOLD GLASS — plate `earth` (7 decade)

**The subject:** the Earth, whole, from far enough out to see it as a ball.

**Its anchor** is `[-0.09, -0.16]` — one stretch of coast at night sits exactly there and grows out of it as
the wheel turns inward. This plate is at 10^7 m on the ladder; the plate
...
The blue marble as an INKED plate, not a photo: ocean depth grading, land in olive and umber, cloud in white swirls with real cyclonic form, the terminator soft, a thin lit rim of atmosphere. The anchor must land on a night-side coast where the child's town-lights will emerge. At blow-up: cloud texture and ocean grain.
...
The winner replaces the placeholder `PlateArt.earth` in
`ten-fold/plates.js` (or is installed as `ten-fold/plates/earth.js` and
