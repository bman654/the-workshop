# THE TEN-FOLD GLASS — plate `atom` (−10 decade)

**The subject:** a single carbon atom — a hard speck of nucleus inside a vast stippled probability cloud.

**Its anchor** is `[-0.04, 0.05]` — the atom's empty room, deeper in sits exactly there and grows out of it as
the wheel turns inward. This plate is at 10^−10 m on the ladder; the plate
...
The honest picture of an atom: almost entirely NOTHING, denser toward the middle, with no hard shell and no cartoon orbits (a hairline suggestion of shells at most). It is a stipple problem end to end — the cloud IS the texture, so it should survive magnification naturally if the octaves are right. Cool blue-white against the warm dark.
...
The winner replaces the placeholder `PlateArt.atom` in
`ten-fold/plates.js` (or is installed as `ten-fold/plates/atom.js` and
