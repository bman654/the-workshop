...
- **Done (browser play-tested PASS, 60fps, clean consoles) — 21 cabinets:**
  ...
  - the-climb.html ✅ — Donkey-Kong-lineage **single-screen girder-climb**, the rack's first **fixed-arc jump over a hazard you must time** (mechanically distinct from Lode Runner's dig and Dig Dug's excavation — here you *ascend* a slanted-girder lattice while rolling barrels follow the ramps down at you). A neon single screen: **run** the girders (←/→), **climb/descend** ladders (↑/↓), and **JUMP** (Space/Z) the fixed parabolic arc to hop a rolling **barrel** for points — clear the lattice to the **lit top platform** to win, then a hotter second floor. A thrower at the top rolls barrels that **follow each slanted girder** and **roll-on-or-tumble** at the ladders/junctions. The whole sim is a separate **deterministic core** (`the-climb.core.js`, forge-inlined into the page with its module guard stripped, required raw by a Node twin with the guard intact): a **pure DOM-free** char-grid level grammar (2 levels sharing a winnable spine, slopes, DROP junctions) + a single fixed jump arc (`JUMP_VY`/`GRAV`), an absolute-lattice px/py figure, **sub-cell box collision evaluated clearance-BEFORE-kill**, and a **SWEPT barrel path** (samples the barrel's pre→post movement in half-body increments so a fast barrel can't tunnel through you) — advanced in a **fixed timestep**, same input ⇒ a byte-identical `hashWorld`/replay, forever. Muted-by-default Web Audio (M toggle, shared `ws:pref:muted`), full juice (animated BONUS countdown / SCORE / BEST / 3-life pips / LEVEL tag, attract mode, particles/shake/flash), Space-or-tap start (Enter alias). Ships a **headless 5-claim self-test** the page shows as a green ✓ chip and a `the-climb.test.cjs` Node twin re-runs (5 core claims + 5 harness checks): (1) **WINNABLE** — a scripted-solver run climbs the ladders to the top platform (`won=true ticks=73`); (2) **BARREL FOLLOWS RAMP** — rolls then DROPs only at a junction, with a **NEG CONTROL** (a no-gap girder never drops); (3) **JUMP-CLEARANCE (IFF) via a DOUBLE negative control** — a well-timed jump clears a barrel (POS), NEG-A a never-jump grounded overlap collides, NEG-B a too-late jump (fired when the barrel is already on you) still collides (so a jump saves you IFF it is well-timed); (4) **WIN IFF TOP** — standing on the top platform wins, NEG CONTROL one row below is NOT a win; (5) **REPLAY DETERMINISM** (identical per-tick hash twice + unchanged across a busy-wait). Plus both levels winnable, fixed-timestep frame count, **no-tunnel** (a barrel faster than a body-width per tick still collides), and **hop scores exactly once** harness cross-checks. The **hop scores on horizontal-overlap + airborne-feet-above-the-barrel-top** (the shadow the arc casts — scores when you arc *over* a barrel even without a box overlap) while the **kill is a full box overlap** (this is what makes the IFF claim 3 pass). Fairness-tuned: L1 softened to spawnEvery=120 / barrelSpeed=2 (was 96 in the prototype, visibly over-dense), L2 stays hotter (78 / 3); both re-proven winnable, and the BFS solver wins via the right-side ladder column without needing to hop (a human has a clear safe route up). Verified in a real browser (agent-browser, served origin, named session): **green ✓ self-test 5/5 chip**, `window.__CLIMB__.selftest === true`, **0 console errors/page-errors**, the **21-card rack renders the cabinet with its real-playfield thumb** (0 nested anchors, no overflow @1280 or @390); the **live in-page world driven to a genuine win** via `_start()`+`_solve()` (`won=true` reached goal cell [1,1] in 73 ticks); Node twin `the-climb.test.cjs` = **10/10 PASS** (5 core claims + 5 harness, chip === twin). `ws:best:the-climb` = best **score** (raise-only). Accent `#ff7a3d` (warm orange — distinct from Lode Runner's gold).
- **Nav:** every game has a click-only `← arcade` back-link (NO key binding — games use keys).
- **Gallery:** index.html (neon "cabinet rack"), games.js manifest (21), README ✅
