...
import { build, evaluate, normalizeForCompare, ROOMS_FLOOR, PIECES_FLOOR } from './manifest.mjs';
...
{ const { ok: gok, failures } = evaluate(live, committedJson);
...
{ const planted = build({ extraDirs: [PLANT] });
  ...
  const { ok: gok, failures } = evaluate(planted, committedJson);
...
{ const synth = { manifest: deepClone(live.manifest), unclaimed: [], doubleClaimed: ['some-dir <- [room, exhibit:numbers-room]'] };
  const { ok: gok, failures } = evaluate(synth, committedJson);
...
{ const shortM = { manifest: deepClone(live.manifest), unclaimed: [], doubleClaimed: [] };
  ...
  const { ok: gok, failures } = evaluate(shortM, committedJson);
...
{ const shortR = { manifest: deepClone(live.manifest), unclaimed: [], doubleClaimed: [] };
  ...
  const { ok: gok, failures } = evaluate(shortR, committedJson);
...
{ // a fresh emit compared against itself is NOT stale
  ...
  const r1 = evaluate(live, selfJson);
  ...
  const r2 = evaluate(live, JSON.stringify(onlyGen, null, 2));
  ...
  const r3 = evaluate(live, JSON.stringify(changed, null, 2));
