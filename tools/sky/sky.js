...
(function (root) {
  ...
  var CATALOG = {
    ...
    "ten-fold": { x: 377.3, y: 1396.3, mag: 2 },
    ...
    "the-sluice-gate": { x: 2501.4, y: 1515.7, mag: 3 },
    ...
    "differential-gear": { x: 2573.8, y: 2201.7, mag: 1 },
    "ball-and-disk": { x: 2601.8, y: 2225.7, mag: 2 },
    "planimeter": { x: 2573.8, y: 2249.7, mag: 3 },
    "pick-and-wheel": { x: 2601.8, y: 2273.7, mag: 3 },
    "slipstick": { x: 2573.8, y: 2297.7, mag: 2 },
    "tone-mill": { x: 275.6, y: 1799, mag: 1 },
    "singing-plate": { x: 275.6, y: 1748, mag: 2 },
    "strobe-mill": { x: 275.6, y: 1850, mag: 3 },
    ...
    "magic-lantern": { x: 2659.6, y: 907.8, mag: 1 },
  ...
  var STAR_META = {
    ...
    "ten-fold": {"name":"The Ten-Fold Glass","href":"ten-fold/index.html"},
    ...
    "planimeter": {"name":"The Planimeter","href":"planimeter/index.html"},
    "pick-and-wheel": {"name":"The Pegboard Planimeter","href":"pick-and-wheel/index.html"},
    "slipstick": {"name":"Slipstick","href":"slipstick/index.html"},
    "tone-mill": {"name":"The Tone Mill","href":"tone-mill/index.html"},
    "singing-plate": {"name":"The Singing Plate","href":"singing-plate/index.html"},
    "strobe-mill": {"name":"The Strobe Mill","href":"strobe-mill/index.html"},
    ...
    "magic-lantern": {"name":"The Magic Lantern","href":"magic-lantern/index.html"},
  ...
  var FEATS = [
    ...
    // The Reckoner — the Reckoning Cabinet's reward constellation: the estate's kin that
    // reckon by MEASURING A SHAPE, not by calculating. Founded with The Differential Gear
    // (the adder) and The Ball-and-Disk Integrator (the integrating organ), now grown to
    // FIVE by gathering the Cabinet's own self-crumbing siblings that already stood
    // unclaimed by the name over them: The Planimeter (area by rolling the boundary —
    // Green's theorem on a wheel), The Pick-and-Wheel (area by lattice count — Pick's
    // theorem), and The Slipstick (products by log-length — log a + log b = log a·b). All
    // five drop plain `ws:seen:<id>` room crumbs; the engraved name now sits over FIVE
    // stars and its tally fires only when every one is seen. Like every feat-group it is
    // ADDITIVE — it NEVER feeds the wings-only all-skies capstone. Room to keep growing as
    // more reckon-by-measuring instruments (a nomograph, a soroban) ship.
    { id: 'reckoner', name: 'The Reckoner', myth: 'Reckons by measuring a shape; the mean, the area, the product — each measured, never counted.',
      members: ['differential-gear', 'ball-and-disk', 'planimeter', 'pick-and-wheel', 'slipstick'] },
    // The Drover — the Arcade's herding reward constellation. Founded by The Shepherd
    // (`ws:seen:the-shepherd`, hold the stick + drive a flock live), grown by The Standing Stones
    // (`ws:seen:the-standing-stones`, set the stick down — place fixed stones, then release), and now
    // The Sluice-Gate (`ws:seen:the-sluice-gate`, slide the walls themselves — sweep the flock through a
    // timed gate), the SAME verified flock law in three verbs. The engraved name now sits over THREE dots.
    // Like every feat-group it is ADDITIVE — it NEVER feeds the wings-only all-skies capstone. Sized to
    // keep growing as the amusements' steer-the-living-system kin ship.
    { id: 'drover', name: 'The Drover', myth: 'Never pushes the flock; only chooses where to stand — and when to open the gate.',
      members: ['the-shepherd', 'the-standing-stones', 'the-sluice-gate'] },
    ...
    // The Sirenist — the Kinetics & Sound wing's reward constellation. Now THREE stars:
    // The Tone Mill (`ws:seen:tone-mill`), The Singing Plate (`ws:seen:singing-plate`, the
    // driven Chladni plate that SINGS its eigenmode), and The Strobe Mill
    // (`ws:seen:strobe-mill`, the promised free STROBOSCOPE — the same strobe-sample rate,
    // SEEN as a wheel that freezes instead of heard as a pitch). One law in three senses.
    // Like every feat-group it is ADDITIVE — it NEVER feeds the wings-only all-skies
    // capstone. Room to keep growing (a seen-and-heard tuning-fork beat) as its kin ship.
    { id: 'sirenist', name: 'The Sirenist', myth: 'Spins the rate you watch into the pitch you hear.',
      members: ['tone-mill', 'singing-plate', 'strobe-mill'] },
