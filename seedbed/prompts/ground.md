...
- **A silent room may borrow the estate's air.** `tools/calendar/air.js` mounts as a widget — `Air.mount(el)`
  puts a dark chip (the self-test-chip idiom) wherever you want it, whose hover card explains itself and
  carries the listener's preferences. Optional, never expected: a page that already sings should not wear it,
  and silence is a legitimate choice. It is there so that giving a room the estate's hum costs one line. The
  builder prompt has the wiring.
- **The estate is a HIERARCHY WITH A WARM CENTER — not a flat, detached sprawl.** Three habits hold this: **(1) Deepen before you detach** — grow by *deepening what stands* (a companion within a room, kin gathered under one roof, a wing grown a second hall) at least as much as by raising new structures; a new thing that could ride *within* one that stands should gather there, and you detach a lone new structure only for a genuine new family with no honest home. **(2) The Manor is the inhabited center** — the grand warm core the grounds, observatory, outbuildings and cavern orbit; an enclosed/interior/instrument/document room belongs *in* it (growing it), open-air/landscape/working/amusement themes orbit it outside. (Its centrality is now a geometric given — the Manor is the pole, tier-0 by construction — so the old "keep it the dominant mass" mandate retires; the soul-routing rule above is the live one.) **(3) No grand name over one dot** — a wing, a constellation, a family earns its engraved name only at TWO; until then it joins existing kin or arrives holding a named sibling's hand. **The placement cascade** ties these together — growth is bottom-up: a new piece first seeks an existing room that fits it in theme and function; a new room, an existing wing with credible kin; a new wing, a district whose theme and seats can hold it — found a new place only when no existing one honestly fits, and found WITHOUT APOLOGY when it truly doesn't: a piece with no honest kin deserves its own front door. The estate stays balanced by deepening before it widens — and balance cuts both ways. (Full statements: DESIGNING.md + tools/layout/map-process.md.)
