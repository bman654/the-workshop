...
import { makeTomb, removeByTitle, addTomb, restampByTitle, gc, FENCES } from './bed.mjs'
...
// ── addTomb: bare tombstone, no seed removed ──
{
  const before = parseBed(FIXTURE)
  const { text } = addTomb(FIXTURE, { fence: 'writ', reason: 'SERVED', cycle: 109, title: 'a served writ', after: 'ccc3333' })
  eq(parseBed(text).writs, before.writs, 'addTomb removes no live seed (writ count unchanged)')
  ok(tombs(text, 'writ').length === 1, 'a writ tombstone was added')
}
