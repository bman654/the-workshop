...
(function () {
  ...
  var FRAME_SETTLE_MS = 400; /* settle beat after the last frame `load` (ENGINE §8 step 2) */
  ...
  var state = {
    ...
    /* ?record arming (T6.2) */
    framesTotal: 0,         /* stack frames created at boot (preflight denominator) */
    framesLoaded: 0,        /* frames that have fired `load` */
    framesSettledAt: 0,     /* perf.now() when framesLoaded first reached framesTotal */
    arming: false,          /* the arm gesture fired; count-in running */
    pfTimer: 0              /* the preflight poll timer (record mode) */
  };
  ...
  function buildStack() {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      /* track load for the ?record preflight (ENGINE §8 step 2 — every frame
         `load`ed + a settle beat). Attached BEFORE src so no early load is missed. */
  ...
  function onFrameLoad() {
    state.framesLoaded++;
    if (state.framesLoaded >= state.framesTotal && !state.framesSettledAt) {
      state.framesSettledAt = (window.performance && performance.now) ? performance.now() : Date.now();
    }
  }
  ...
  /* ── ?record preflight (ENGINE §8 step 2 — the OBS contract) ────────────────
     Every red is a VISIBLE refusal to arm, never a degraded take. Returns
     { ok, checks:[{ name, ok, detail }] } — pure read-only measurement, run on a
     poll before the arm click and RE-RUN inside armRecord (so a condition that
     flips red between poll and click still refuses). T6.2. */
  function preflightRecord() {
    var checks = [];
    var now = (window.performance && performance.now) ? performance.now() : Date.now();

    /* 1. all stack frames loaded + a settle beat */
    var framesOk = state.framesTotal > 0 && state.framesLoaded >= state.framesTotal &&
                   state.framesSettledAt && (now - state.framesSettledAt) >= FRAME_SETTLE_MS;
    checks.push({ name: 'stack frames loaded', ok: !!framesOk,
      detail: state.framesLoaded + '/' + state.framesTotal + (framesOk ? ' · settled' : ' · settling…') });

    /* 2. all audio readyState ≥ 2 (HAVE_CURRENT_DATA) — bed + every VO segment */
    var audioOk = true, ad = [];
    var bedRS = bed ? bed.readyState : 0;
    if (bedRS < 2) audioOk = false;
    ad.push('bed=' + bedRS);
    for (var id in segEls) {
      var rs = segEls[id] ? segEls[id].readyState : 0;
      if (rs < 2) audioOk = false;
      ad.push(id + '=' + rs);
    }
    checks.push({ name: 'audio buffered (readyState ≥ 2)', ok: audioOk, detail: ad.join(' ') });

    /* 3. Reduce Motion OFF (the estate freezes its motion under RM) */
    var rmOn = false;
    ...
    /* 4. estate unmuted (ws:pref:muted ≠ '1' — the colophon voice + Gate audio depend on it) */
    var m = muted();
    checks.push({ name: 'estate unmuted', ok: !m, detail: m ? "ws:pref:muted='1'" : 'unmuted' });

    /* 5. viewport ≥ 1920×1080 */
    var vw = window.innerWidth, vh = window.innerHeight;
    var vpOk = vw >= 1920 && vh >= 1080;
    checks.push({ name: 'viewport ≥ 1920×1080', ok: vpOk, detail: vw + '×' + vh });

    /* 6. fresh-state assertion — the diary reports Page 1 AWAITING + exactly two
       SEALED stamps (same-origin DOM count). Any other state = a non-fresh profile
       (invariant 7: the take is served from a dedicated fresh record port). */
    var diaryOk = false, dd = 'diary frame not reachable';
    try {
      var dfe = frameEls[DIARY_KEY], dw = dfe && dfe.contentWindow, doc = dw && dw.document;
      if (doc) {
        var aw = doc.querySelectorAll('.card.awaiting').length;
        var se = doc.querySelectorAll('.card.sealed').length;
        diaryOk = (aw === 1 && se === 2);
        dd = aw + ' awaiting / ' + se + ' sealed';
      }
    } catch (e) { dd = 'diary cross-read failed'; }
    checks.push({ name: 'diary fresh (1 awaiting / 2 sealed)', ok: diaryOk, detail: dd });
  ...
  /* Paint the preflight panel on the record gate (the arm surface). */
  function renderPreflight(pf) {
    if (!els.pfList) return;
    var html = '';
    ...
    els.pfList.innerHTML = html;
    if (els.pfStatus) {
      els.pfStatus.textContent = pf.ok
        ? 'READY — click anywhere to arm the take'
        : 'NOT READY — any red row above blocks the take (fix it, do not record)';
      els.pfStatus.className = 'pf-status ' + (pf.ok ? 'ok' : 'bad');
    }
  }

  /* ── the record arm (ENGINE §8 step 3) — the ONE real blessing gesture ───────
     Re-runs preflight (refuses on any red), performs the FULL reach-in under the
     real gesture, DOM-removes the last chrome, then a silent ≥ 2 s held-black
     count-in before bed t=0. T6.2. */
  function armRecord() {
    ...
    reachIn(); /* unlock bed+VO, weave the colophon, unlock+suspend the Gate storm */

    /* held black BEFORE removing the gate → no flash of the woven page */
    if (els.heldblack) els.heldblack.classList.add('on');
    if (els.startgate && els.startgate.parentNode) els.startgate.parentNode.removeChild(els.startgate);
    ...
    setTimeout(function () {
      if (els.heldblack) els.heldblack.classList.remove('on');
      if (bed) { try { bed.currentTime = 0; } catch (e) {} }
      state.started = true;
      startClock();
      play();
      log('▸ record: bed t=0 — the film runs');
    }, COUNT_IN_MS);
  }

  /* The reach-in: everything that needs the real gesture, done once at the arm
     click (a synthetic .click() unlocks no audio — verified). ENGINE §8 step 3. */
  function reachIn() {
    /* (a) unlock bed + every VO element (play-paused once, muted through the
       unlock so no blip escapes before the count-in) */
    unlockEl(bed);
    for (var id in segEls) unlockEl(segEls[id]);
    /* (b) the colophon cold open — weave-and-hold off the master clock. REPO-WINS
       (T6.1 turn-2 rework supersedes ENGINE §3's colophonBeginAt/colophonCut path):
       primeColdOpen() = colophonHold + colophonSeek; colophonHold mutes the page's
  ...
  function boot() {
    ...
    els = {
      ...
      preflight: $('preflight'), pfList: $('pf-list'), pfStatus: $('pf-status')
    };
    bed = $('bed');
    ...
    /* the ONE blessing gesture (real input — a synthetic .click() unlocks no
       audio). In ?record the whole gate is the OBS "Interact" arm target and runs
       the full preflight + reach-in (T6.2); in authoring the start button plays. */
    if (RECORD) setupRecordArm();
    else if (els.startbtn) els.startbtn.addEventListener('click', start);
  ...
  /* Wire the ?record arm surface (ENGINE §8): the whole gate is clickable (OBS
     Interact clicks anywhere), and a poll paints the preflight panel until READY.
     The poll keeps running so a condition that flips (RM toggled, a frame that
     finishes loading) is reflected live before the operator clicks. T6.2. */
  function setupRecordArm() {
    if (els.startgate) els.startgate.addEventListener('click', armRecord);
    (function poll() {
      if (state.started || state.arming) return;
  ...
  /* Cold-open primer (SP-B items 1/2/3, T6.1): weave-and-hold the colophon at
     COLO_START_MS (the paragraph before "I am Claude") via colophonHold — which
     mutes the page's own narration (the rendered S0 segment is the voice) and
     reaches state="play" SYNCHRONOUSLY (no dependence on the colophon mp3's own
     playback/load, whose race was Brandon's "static, dead colophon" + no-voice
     pause). The reveal is then driven deterministically by driveColdOpen()
     scrubbing colophonSeek() off the master clock. Called by the authoring start()
     AND by the ?record reach-in (T6.2) — both under a real gesture; identical weave. */
  ...
  window.__trailer = {
    ...
    /* ?record contract surface (T6.2) — read-only preflight + the arm gesture, for
       the OBS operator's checks and the rehearsal harness. */
