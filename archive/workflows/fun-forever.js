...
const DIRECTOR_SCHEMA = {
  ...
  properties: {
    ...
    basicDesign: { type: 'string', description: 'The skeleton: the one idea, the FORM it takes (what real thing you SHOW / how it is touched or played / the delight it delivers — reach past a plain graph), and — IF it makes a math claim — the claim its self-test proves (conditional, not required; a pure-delight piece makes none — do not invent one). For a REWORK: which existing piece + the soul it lacks + the re-soul direction. For a bug: what is broken + the fix.' },
...
const EXPLORER_SCHEMA = {
  ...
  properties: {
    ...
    candidateSeeds: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['type', 'title', 'sketch'], properties: { type: { type: 'string', description: 'garden: exhibit | cross | curation | rework · grounds: room | engine | metagame | map · or spark' }, title: { type: 'string' }, sketch: { type: 'string', description: 'the ≤3-line provocation + the FORM it wants (touchable / living / game / sound / pure delight with no claim at all — not just a graph) + (IF it makes a math claim) its falsifiable crux (claim-free is first-class — never bolt one on); grep-confirmed a genuine gap. For a rework: name the existing piece + the soul it lacks + the re-soul direction.' } } }, description: 'PLAN scouts only.' },
  },
}
